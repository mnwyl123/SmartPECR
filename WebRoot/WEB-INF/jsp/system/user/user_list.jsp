							...
							<form action="user/listUsers.do" method="post" name="userForm" id="userForm">
								...
								<fieldset class="hidden-xs " id="fieldset1">
									<div class="row style_m">
										...
										<div class="col-xs-4">
											<select dict-name="ClientType" default-value="${pd.DEPARTMENT_ID }"  name="DEPARTMENT_ID" id="DEPARTMENT_ID"  >
												<option value="">请选择机构</option>
												<c:forEach items="${orgList}" var="org">
													<option value="${org.orgId }" <c:if test="${org.orgId==pd.DEPARTMENT_ID}">selected</c:if>> ${org.name}</option>
												</c:forEach>
											</select>
										</div>
									</div>
									<div id="fieldset" hidden="">
										...
										<div class="row style_m">
											...
											<div class="col-xs-3">
												<select dict-name="TempType" name="ROLE_ID" id="ROLE_ID"  default-value="${pd.ROLE_ID }" >
											...
											<td class="text-left jianju" style="margin-left:10px;">
												 <c:if test="${QX.sendSms == 1 }">
													<div class="btn-group jianju" >
														<a title="批量发送站内信" class="btn btn-mini" onclick="makeAll('确定要给选中的用户发送站内信吗?');">
															<i style="font-size: 18px;" class="iconfont icon-zhanneixin"></i>
														</a>
													</div>
												</c:if>
												...
												<c:if test="${QX.downLoad == 1 }">
													<div class="btn-group jianju2">
														<a class="btn btn-mini" onclick="toExcel();" title="导出到EXCEL">
															<i style="font-size: 18px;" class="iconfont icon-xiazai"></i>
														</a>
													</div>
												</c:if>
												<c:if test="${QX.upLoad == 1 }">
													<div class="btn-group jianju2">
														<a class="btn btn-mini" onclick="fromExcel();" title="从EXCEL导入">
															<i style="font-size: 18px;" class="iconfont icon-msnui-cloud-upload"></i>
														</a>
													</div>
												</c:if>
												<c:if test="${QX.add == 1 }">
													<div class="btn-group jianju2" >
														<a class="btn btn-mini" onclick="add();" title="新建用户">
												...
												<c:if test="${QX.batchDetele == 1 }">
													<div class="btn-group jianju2" >
														<a title="批量删除" class="btn btn-mini" onclick="makeAll('确定要删除选中的数据吗?');" >
													...
													<c:if test="${QX.query == 1 }">
														<c:forEach items="${userList}" var="user" varStatus="vs">	
															<tr>
																<td class='center'>
																	<label>
																		<input type='checkbox' name='ids' value="${user.USER_ID }" id="${user.EMAIL }" alt="${user.PHONE }" title="${user.USERNAME }" class="ace"/>
																		<span class="lbl"></span>
																	</label>
																</td>
																<td class='center'>${vs.index+1}</td>
																<td class="center">
																	<a onclick="viewUser('${user.USER_ID}')" style="cursor:pointer;">${user.USERNAME }</a>
																</td>
																	...
																	<c:if test="${user.USERNAME == pd.curUser || user.USERNAME != pd.curComp}">
																		...
																		<div class="hidden-sm hidden-xs action-buttons">
																			<c:if test="${QX.unLock == 1 }">
																				<a > 
																					<i class="iconfont icon-quanxianguanli green1"  title='解锁重置密码' onclick="unLock('${user.USER_ID }');"></i>
																				</a>
																			</c:if>
																			<c:if test="${QX.authorg == 1 }">
																				<a > 
																					<i class="iconfont icon-quanxian blue2"  title='机构授权' onclick="authUser('${user.USER_ID }','org');"></i>
																				</a>
																			</c:if>
																			<c:if test="${QX.authrole == 1 }">
																				<a >
																					<i class="iconfont icon-icon purple2" title="角色授权" onclick="authUser('${user.USER_ID }','role');"></i>
																				</a>
																			</c:if>
																			<c:if test="${QX.edit == 1 }">
																					...
																					<c:if test="${QX.authorg == 1 }">
																						<li>
																							<a  onclick="authUser('${user.USER_ID }');" class="tooltip-info" data-rel="tooltip" title="授权机构">
																								<span class="blue2"><i class="iconfont icon-quanxian "></i></span>
																							</a>
																						</li>
																					</c:if>
																					<c:if test="${QX.authrole == 1 }">
																						<li>
																							<a  onclick="authUser('${user.USER_ID }');" class="tooltip-info" data-rel="tooltip" title="授权角色">
																								<span class="purple2"><i class="iconfont icon-icon "></i></span>
																							</a>
																						</li>
																					</c:if>
																					<c:if test="${QX.edit == 1 }">
																						<li>
																							<a  onclick="editUser('${user.USER_ID}');" class="tooltip-success" data-rel="tooltip" title="编辑">
																								<span><i class="iconfont icon-bianji red1"></i></span>
																							</a>
																						</li>
																					</c:if>
																					<c:if test="${QX.delete == 1 }">
																						<li>
																							<a  onclick="delUser('${user.USER_ID }','${user.USERNAME }');" class="tooltip-error" data-rel="tooltip" title="删除">
																								<span><i class="iconfont icon-shape7 green2"></i></span>
																							</a>
																						</li>
																					</c:if>
																				</ul>
...
function add(){
	 ...
	 <%-- 适应单点登录SSO maff 修改时间：2017/7/6
	 diag.URL = '<%=basePath%>user/goAddUSSO.do'; --%>
...
function toExcel(){
	...
	window.location.href='<%=basePath%>user/excel.do?keywords='+keywords+'&lastLoginStart='+lastLoginStart+'&lastLoginEnd='+lastLoginEnd+'&IDS='+str;
	}
}
