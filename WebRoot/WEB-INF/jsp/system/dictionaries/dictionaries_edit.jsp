						...
						<table id="table_report" class="table table-striped "><!-- table-bordered table-hover -->
							...
							<tr>
								...
								<c:if test="${null != (pd.DICT_CODE)}">
									<td><input type="text" name="DICT_CODE" id="DICT_CODE" value="${pd.DICT_CODE}" maxlength="10" placeholder="这里输入编码" title="编码" class="width-75" readonly="readonly"/></td>
								</c:if>
								<c:if test="${null == (pd.DICT_CODE)}">
									<td><input type="text" name="DICT_CODE" id="DICT_CODE" value="${pd.DICT_CODE}" maxlength="10" placeholder="这里输入编码" title="编码" class="width-75" onblur="hasBianma('${pd.PARENT_ID }')"/></td>
								</c:if>
							</tr>
							<tr>
								<td class="align-right">排序:<span class="red ">*</span></td>
								<td><input type="number" name="ORDER_BY" id="ORDER_BY" value="${pd.ORDER_BY}" maxlength="10" placeholder="这里输入排序" title="排序" class="width-75"/></td>
							</tr>
							<tr>
								<td class="align-right">备注:</td>
								<td>
									<textarea rows="3" cols="46" name="BZ" id="BZ" maxlength="100" placeholder="这里输入备注" title="备注"  class="width-75">${pd.BZ}</textarea>
								</td>
							</tr>
							<tr>
								<td class="align-right">排查表:</td>
								<td><input type="text" name="TBSNAME" id="TBSNAME" value="${pd.TBSNAME}" maxlength="50" placeholder="这里输入表名, 多个用逗号分隔(非必录)" title="排查表" class="width-75"/></td>
							</tr>
							<tr>
