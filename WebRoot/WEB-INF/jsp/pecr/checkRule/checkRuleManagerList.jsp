					...
					<div class="layui-form-item" style="margin:1px">
						<div class="layui-inline">
							...
							<div class="layui-input-inline" style='padding:10px 5px 5px 5px;'>
								<input type="text" name="ERRNO" id="ERRNO" placeholder="请输入规则编码" 
...
<script type="text/html" id="toolbarTal">
		...
		<button type="button" class="layui-btn layui-btn-xs layui-btn-danger" lay-event="delSelectAll">批量删除 
...
<script type="text/javascript">
	//初始化下拉字典
	initDict('BUSINESSTYPE', staticDict.checkRuleType);
	initDict('VALIDTYPE', staticDict.validType);
	initDict('ISINUSE', staticDict.isUse);
	//弹出层id
	var popIndex;
	...
	layui.use(['form', 'laydate', 'element', 'table'], function(){
		...
		table.render({
			...
			url:'checkRule/dataList.do', //url
			cols: [[ 
				{type:'numbers'},	//列每行序号
				{type:'checkbox'}, //列复选框
				{field: 'ERRNO', title: '规则编码', align:'center', sort: true},
				{field: 'BUSINESSTYPE', title: '业务类型', align:'center',templet:function(d){return renderColumnDict(d, 'BUSINESSTYPE', staticDict.checkRuleType)}},
				{field: 'ERRDESCRIBE', title: '规则描述', align:'center'},
				{field: 'VALIDEXPRESSION', title: '校验方法', align:'center'},
				{field: 'CHECKTANLENAME', title: '校验表名', align:'center'},
				{field: 'CHECKCOLNAME', title: '校验字段', align:'center'},
		...
		table.on('toolbar(checkRuleRow)', function(obj){
			if(obj.event === 'addBtn') {
			...
			}else if(obj.event === 'delSelectAll'){
				//获得表格CheckBox已经选中的行的信息
				var checkStatus = table.checkStatus('checkRule'),
				//返回行的value
				...
				layer.confirm('确定要删除选中的数据吗？', {icon: 3, title: '提示信息'}, function (index){
					$.ajax({
						type : "POST",  //请求方式
						url : "checkRule/deleteALL.do", //请求地址
		...
		*/
		...
		form.on('submit(exampleSubmit)', function(data){
			$.ajax({
				type : "POST",  //请求方式
				url  : "checkRule/addOrUpdate.do?addOrUpdate=" + addOrUpdate,
