...
//展示已知数据
  table.render({
	...
	,url:'indvLoanAcctInfoSeo/dataHistListForUpdate.do?ACCT_TYPE=' + dataObj.ACCT_TYPE+"&ACCT_CODE="+dataObj.ACCT_CODE //url
    ,cols: [[ 
    	{type:'numbers'},	//列每行序号
		{type:'checkbox'},  //列复选框
...
//		{field: 'mngmtOrgCode', align:'center', title: '所属机构'},
       ...
       // {field: 'idType', align:'center', title: '身份标识类别', templet:function(d){return renderColumnDict(d,'idType',staticDict.idType)}},
		...
		{field: 'mdfcSgmtCode', align:'center', title: '更正段段标',templet:function(d){return renderColumnDict(d,'mdfcSgmtCode',staticDict.perLoanUpdInfRecType)}},
		{field: 'month', align:'center', title: '月度表现月份'},
			...
			layer.open({
        	  ...
        	  addOrUpdate = 'detail';
	  			layer.open({
	  				...
	  				anim: 2,
	  				resize: false,
	  				content: 'indvLoanAcctInfoSeo/loanAcctInfoEdit.do?addOrUpdate=' + addOrUpdate+ "&pbaAcctBsSgmtId=" + data.pbaAcctBsSgmtId + "&queryType=indvLoanUpdateBySegment",
	  				end: function(){
	  					tableReload(); 
	  				}
	  			});
    	...
    	if(flag === 'new'&& businessStates == '50'){
    		layer.confirm('是否确认回退报文(该数据所在报文的全部数据状态为审核成功)？', function(index){
    			$.ajax({
   		   	       ...
   		   	       type : "POST",
   		   	       //请求地址
   		   	       url : "indvLoanAcctInfoSeo/backdate.do?addOrUpdate="+addOrUpdate,
   		   	       //数据，json字符串
   		   	       data : data,
   		   	       //请求成功
