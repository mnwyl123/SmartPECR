...
//展示已知数据
  table.render({
	...
	,url:'indvBasicsInfoSeo/dataHistList.do?ID_TYPE='+ dataObj.ID_TYPE + "&ID_NUM=" + dataObj.ID_NUM //url
    ,cols: [[ 
    	{type:'numbers'},	//列每行序号
		{type:'checkbox'},  //列复选框
		{field: 'paaBsSmgtId', title: '主键', hide: true},
//		{field: 'orgCode', align:'center', title: '所属机构'},
		...
		{field: 'reportMessageId', align:'center', title: '报告名称'},
    	...
    	if(flag === 'new'){
        	...
        	$("body").data("dataObj",data);
  			...
  			layer.open({
  				...
  				anim: 2,
  				resize: false,
  				content: 'indvBasicsInfoSeo/indvBasicsInfoDetail.do?addOrUpdate=' + addOrUpdate
  				         + '&paaBsSmgtId=' + data.paaBsSmgtId,
  				end: function(){
  					tableReload();
  				}
    	...
    	if(flag === 'new' && businessStates == '50'){
    		layer.confirm('是否确认回退报文(该数据所在报文的全部数据状态为审核成功)？', function(index){
		   	    $.ajax({
		   	       ...
		   	       type : "POST",
		   	       //请求地址
		   	       url : "indvBasicsInfoSeo/backdate.do?addOrUpdate="+addOrUpdate,
		   	       //数据，json字符串
		   	       data : data,
		   	       //请求成功
