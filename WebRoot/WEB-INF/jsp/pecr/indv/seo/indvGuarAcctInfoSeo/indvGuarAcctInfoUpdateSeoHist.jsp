...
//展示已知数据
  table.render({
	...
	,url:'indvGuarAcctInfoSeo/dataHistListForUpdate.do?ACCT_TYPE=' + dataObj.ACCT_TYPE+"&ACCT_CODE="+dataObj.ACCT_CODE //url
    ,cols: [[ 
    	{type:'numbers'},	//列每行序号
		{type:'checkbox'},  //列复选框
...
//		{field: 'mngmtOrgCode', align:'center', title: '所属机构'},
		...
		{field: 'acctType', align:'center', title: '账户类型', templet:function(d){return renderColumnDict(d,'acctType',staticDict.perGuarAcctType)}},
		...
		{field: 'mdfcSgmtCode', align:'center', title: '更正段段标', templet:function(d){return renderColumnDict(d,'mdfcSgmtCode',staticDict.perGuarUpdInfRecType)}},
		{field: 'rptDate', align:'center', title: '信息报告日期'},
		{field: 'reportMessageId', align:'center', title: '报文名'},
    	...
    	if(flag === 'new'){
			...
			layer.open({
  			...
  			layer.open({
  				...
  				anim: 2,
  				resize: false,
  				content: 'indvGuarAcctInfoSeo/guarAcctInfoEdit.do?addOrUpdate=' + addOrUpdate
  				         + '&pdaGuarAcctBsSgmtId=' + data.pdaGuarAcctBsSgmtId
  				       	 + "&queryType=indvGuarUpdateBySegment",
  				end: function(){
  					tableReload(); 
  				}
  			});
    	...
    	if(flag === 'new'&& businessStates == '50'){
    		layer.confirm('是否确认回退报文(该数据所在报文的全部数据状态为审核成功)？', function(index){
		   	     $.ajax({
		   	       ...
		   	       type : "POST",
		   	       //请求地址
		   	       url : "indvGuarAcctInfoSeo/backdate.do?addOrUpdate="+addOrUpdate,
		   	       //数据，json字符串
		   	       data : data,
		   	       //请求成功
