			...
			<div class="layui-form-item">
				<div class="layui-inline">
				  ...
				  <div class="layui-input-inline">
					<input type="text" name="NW_BNES_CODE" id="NW_BNES_CODE" autocomplete="off"  class="layui-input"/>
				  </div>
				</div>
				<div class="layui-inline">
				  <label class="layui-form-label">原业务标识码</label>
				  <div class="layui-input-inline">
					<input type="text" name="OD_BNES_CODE" id="OD_BNES_CODE" autocomplete="off"  class="layui-input"/>
				  </div>
				</div>
			</div>
		  </div>						  
...
//展示已知数据
  table.render({
	...
	,url:'accountchangeSeo/dataList.do' //url
    ,cols: [[ 
      {type:'numbers'}	//列每行序号
      ,{type:'checkbox'} //列复选框
      ,{align : "center",field: 'NW_BNES_CODE', title: '新业务标识码'}
      ,{align : "center",field: 'OD_BNES_CODE', title: '原业务标识码'}
  ...
  $("#queryBtn").on('click', function(){
	  table.reload('tableDemo',{
		  method : 'post',
			where: {
				OD_BNES_CODE   :   $('#OD_BNES_CODE').val(), 
				NW_BNES_CODE   :   $('#NW_BNES_CODE').val()
				}
	  }); 
	  return false;
  });
  ...
  table.on('tool(tableDemo)', function(obj){
    ...
    layer.open({
		...
		anim: 2,
		resize: false,
		content: 'accountchangeSeo/listHistPage.do?NW_BNES_CODE=' + data.NW_BNES_CODE+'&OD_BNES_CODE='+data.OD_BNES_CODE,
		end: function(){
			table.reload('tableDemo', {
				method : 'post',
				where: {
