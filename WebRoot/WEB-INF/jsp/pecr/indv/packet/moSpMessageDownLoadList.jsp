...
<body>
		...
		<div class="layui-form layui-form-pane" id="queryPanel">
			<div class="layui-colla-item">
				<h3 class="layui-colla-title">
					数据检索
					<button
						class="layui-btn layui-btn-xs layui-btn-danger layui-btn-radius pane-title-btn"
						id="queryBtn">
					...
					<div class="layui-form-item">
						<div class="layui-inline">
							...
							<div class="layui-input-inline">
								<input type="text" class="layui-input" id="rptDate"
									name="rptDate" placeholder=" - ">
							</div>
						</div>
						<div class="layui-inline">
	...
	<button class="layui-btn layui-btn-xs layui-btn-radius" lay-event="generateUploadView"  id="generateUploadView">回执上传 <i class="layui-icon layui-icon-upload-circle"></i></button>
	<button class="layui-btn layui-btn-xs layui-btn-radius" lay-event="showFeedback"  id="showFeedback">反馈详情下载 <i class="layui-icon layui-icon-file-b"></i></button>
...
//初始化性别下拉字典
initDict('infRecType', staticDict.motgaInfrectype);
//弹出层id
var popIndex;
 ...
 layui.use(['form', 'laydate', 'element', 'layer', 'table', 'laypage','laytpl'], function(){
		...
		require(['static/js/pecr/module/base'],function(base){
		    ...
		    var initEnumerationValue = function(){
				var array = ['INDV_INFRECTYPE','REPORT_STATUS'];
				var selectData = [];
				...
				$.ajax({  
					...
					success:function(data){
						for(var i=0;i<data.result.length;i++){
							if (data.result[i].INDV_INFRECTYPE){
								base.renderTpl({
									tplId : "enumerationValueTpl",
									arr : {"list" : data.result[i].INDV_INFRECTYPE},
...
//展示已知数据  
  table.render({
	...
	,url:'indvMessageDownLoad/dataList.do?type=moSp' //url
    ,cols: [[ 
      ...
      ,{field: 'REPORT_MESSAGE_TYPE', align:'center' , title: '报送报文类型',templet: function(d){return renderColumnDict(d, 'REPORT_MESSAGE_TYPE', staticDict.motgaInfrectype)}}
...
//      ,{field: 'create_success_time', align:'center', title: '创建完成时间'}
	  ...
	  ,{fixed: 'REPORT_MESSAGE_STATUS', align:'center', width:200 , title:'报文状态',fixed: 'right', templet: function(d){return renderColumnDict(d, 'REPORT_MESSAGE_STATUS', staticDict.reportMessageStatus)}} //行上的工具栏
    ]]
    ,even: true  //行斑马线
	,page: true //是否分页
	,limit: 10 //初始每页显示条数
			...
			case 'generateEncMessage' :
					...
					for(var i = 0; i < data.length; i++){
						...
						if(checkStatus.data[i].REPORT_MESSAGE_STATUS != '60'){
							layer.msg("存在报文状态为：预处理失败 的数据记录，请重新选择！");
							return ;
						}
					}
					str = str.substr(0, str.length - 1);
			...
			case 'showFeedback':
				 ...
				 if(data.length==0){
					...
					return ;
				 	...
				 	if(str === '30'){
				 		layer.alert('该报文反馈成功!');
				 	}else if(str === '40' || str === '50' || str === '70' || str === '80'){//50-反馈部分出错 70-预处理全失败   80-预处理部分失败
				 		str = checkStatus.data[0].FEEDBACK_MESSAGE_FILE_PATH;
						window.location.href='<%=basePath%>indvMessageDownLoad/download.do?DATA_IDS='+str; 	
				 	}else{
				 		layer.alert('该报文未反馈!');
				 	}
