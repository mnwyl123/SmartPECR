...
//	                d1.resolve("渲染完成");
	...
	table.render({
	  elem: '#tableDemo' //表格所固定的div
		...
		,url:'indvAbnormalMessage/dataList.do' //url
	  ,cols: [[ 
	    {type:'numbers', fixed: 'left'}	//列每行序号
	    ,{type:'checkbox', align:'center'} //列复选框
	    ,{field: 'RPTDATE', align:'center', title: '报送日期'}
	    ,{field: 'INFRECTYPE', align:'center' , title: '信息记录类型',templet: function(d){return renderColumnDict(d, 'INFRECTYPE', staticDict.INFOTYPE)}}
...
//查询按钮点击事件
	...
	table.on('toolbar(tableDemo)', function(obj){
		...
		//获取选中的行信息
		 ...
		 var normalOrAbnormal ="indvabnormal";
		 if(data.length==0){
		...
		for(var i = 0; i < data.length; i++){
			str+=checkStatus.data[i].INFRECTYPE+","+checkStatus.data[i].COMPANY+","+checkStatus.data[i].RPTDATE+";"
		}
