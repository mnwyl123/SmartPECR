...
initDict('id_Type', staticDict.idType);
//弹出层id
var popIndex;
 ...
 // form.render('select');
  ...
  //展示已知数据
    table.render({
  	...
  	,url:'inIDEfctInf/List.do' //url
      ,cols: [[ 
        {type:'numbers'}	//列每行序号
        ,{type:'checkbox'} //列复选框
        ,{field: 'pakInIdEfctInfId', hide: true,title: '主键', sort: true}
  ...
  table.on('toolbar(tableDemo)', function (obj) {
  		switch (obj.event) {
  			case 'addBtn':
  				...
  				$("#close").show();
  				 ...
  				 form.val('exampleForm', {'pakInIdEfctInfId':'','name':'', 'idType':'', 'idNum':'', 'infSurcCode':'', 'idEfctDate':'', 'idDueDate':'', 
  						'idOrgName':'', 'idDist':'', 'cimoc':'', 'rptDate':'','feedBackError':''});
  					$.ajax({
  						type : "POST",  //请求方式
  						url  : "generateUUID/id.do",
  						success : function(data) {
  							if(data.success == true){
  								$("#pakInIdEfctInfId").val(data.result);
  							}else{
  			...
  			case 'submitBtn':
  				...
  				for(var i = 0; i < data.length; i++){
  					str+=checkStatus.data[i].pakInIdEfctInfId+","
  				}
    ...
    table.on('tool(tableDemo)', function(obj){
  	  form.val('exampleForm', {'pakInIdEfctInfId':'','name':'', 'idType':'', 'idNum':'', 'infSurcCode':'', 'idEfctDate':'', 'idDueDate':'', 
  			'idOrgName':'', 'idDist':'', 'cimoc':'', 'rptDate':'','feedBackError':''});
      var data = obj.data;
      var PK= data.pakInIdEfctInfId;
      var tableName = 'I_IN_ID_EFCT_INF';
      if(obj.event === 'more'){
      	addOrUpdate = "update";
      	 $.ajax({
  	  	       //请求方式
      	...
      	//将该行数据在form中回显
      	form.val('exampleForm', data);
  	   	//verify.dataValidation('save','I_IN_ID_EFCT_INF',$("#pakInIdEfctInfId").val(),'form',false,false); //数据校验
  	   	form.render('select');
