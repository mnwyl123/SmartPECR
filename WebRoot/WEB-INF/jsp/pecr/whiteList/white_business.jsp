				...
				<div class="layui-form-item">
					...
					<div class="layui-inline">
					  ...
					  <div class="layui-input-inline">
						<select  lay-search="" id="business_type" name="business_type">
	...
	<form class="layui-form layui-form-pane" lay-filter="exampleForm" id="exampleForm">	
		<div class="layui-form-item" style="margin-left:30px;">
			...
			<div class="layui-inline">
			  <label class="layui-form-label">业务类型</label>
			  <div class="layui-input-inline">
				<select  lay-search="" readonly name="business_type"id="business_type1">
				  <option value="">请选择</option>										 
				</select>
			  </div>
			</div>
			...
			<div class="layui-inline">
			  <label class="layui-form-label">业务主键标识码</label>
			  <div class="layui-input-inline">
				<input type="text" name="business_code" id="business_code1" autocomplete="off" class="layui-input" maxlength="60">
			  </div>
			</div>
...
//展示已知数据
  table.render({
	...
	,url:'whiteMsgMaintenance/product_list.do?type=business' //url
    ,cols: [[ 
      {type:'numbers'}	//列每行序号
      ,{type:'checkbox'} //列复选框
      ...
      ,{align : "center",field: 'PRODUCT_CODE', title: '业务主键标识码'}
  ...
  $("#queryBtn").on('click', function(){
	  table.reload('tableDemo', {'where':{'business_type':$('#business_type').val(),'business_code':$('#business_code').val() }}); 
	  return false;
  });
	...
	$('#addBtn').on('click', function(){
		 ...
		 form.val('exampleForm', {'uuid':'','business_type':'','business_code':''});
		 $("#add").show();
	  	  $("#save").hide();
		 popIndex = layer.open({
			type :'1',
  ...
  form.on('submit(add)', function(data){
	  var addOrSave = "add";
	  if(data.field.business_type == null || data.field.business_type == ""||
			  data.field.business_code == null || data.field.business_code == ""){
		  
		  layer.alert('业务类型与业务标识码信息必填!');
		  return false;
	  }else{
		  $("#add").show();
	  	  $("#save").hide();
	     ...
	     $.ajax({
	       ...
	       type : "POST",
	       //请求地址
	       url : "whiteMsgMaintenance/business_save.do?addOrSave="+addOrSave,
	       //数据，json字符串
	       data : data.field,
	       //请求成功
    ...
    if(obj.event === 'update'){
    	delOrUpdate="update";
    	 $.ajax({
  	       ...
  	       type : "POST",
  	       //请求地址
  	       url : "whiteMsgMaintenance/business_editdel.do?delOrUpdate="+delOrUpdate,
  	       //数据，json字符串
  	       data : data,
  	       //请求成功
  	       success : function(result) {
