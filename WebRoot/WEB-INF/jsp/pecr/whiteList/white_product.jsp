	...
	<form class="layui-form layui-form-pane" lay-filter="exampleForm" id="exampleForm">	
		<div class="layui-form-item" style="margin-left:30px;">
			...
			<div class="layui-inline">
			  <label class="layui-form-label">产品代码</label>
			  <div class="layui-input-inline">
				<input type="text" name="product_code" id="product_code1" autocomplete="off" class="layui-input" maxlength="40">
			  </div>
			</div>
...
//展示已知数据
  table.render({
	...
	,url:'whiteMsgMaintenance/product_list.do?type=product' //url
    ,cols: [[ 
      {type:'numbers'}	//列每行序号
      ,{type:'checkbox'} //列复选框
  ...
  $("#queryBtn").on('click', function(){
	...
	$('#addBtn').on('click', function(){
		 ...
		 form.val('exampleForm', {'product_code':'','uuid':''});
		 $("#add").show();
	  	  $("#save").hide();
		 popIndex = layer.open({
			type :'1',
  ...
  form.on('submit(add)', function(data){
	  ...
	  if(data.field.product_code == null || data.field.product_code == ""){
	     ...
	     $.ajax({
	       ...
	       type : "POST",
	       //请求地址
	       url : "whiteMsgMaintenance/product_save.do?addOrSave="+addOrSave,
	       //数据，json字符串
	       data : data.field,
	       //请求成功
	       success : function(result) {
	    	   if(result.success == true){
	     		  ...
	     		  table.reload('tableDemo', {'where':{'product_code':$('#product_code').val()}}); 
    ...
    if(obj.event === 'update'){
    	delOrUpdate="update";
    	 $.ajax({
  	       ...
  	       type : "POST",
  	       //请求地址
  	       url : "whiteMsgMaintenance/product_editdel.do?delOrUpdate="+delOrUpdate,
  	       //数据，json字符串
  	       data : data,
  	       //请求成功
  	       success : function(result) {
    	...
    	delOrUpdate="delete";
    	layer.confirm('确定删除该产品 ?', function(index){
		     $.ajax({
		       //请求方式
		       type : "POST",
		       //请求地址
