		...
		<div class="layui-colla-item">
		  ...
		  <div class="layui-colla-content layui-show">
			<div class="layui-form-item">
				...
				<div class="layui-inline">
				  <label class="layui-form-label">企业标识号码</label>
				  <div class="layui-input-inline">
...
//展示已知数据
  table.render({
	...
	,url:'entLoanAcctInfoSeo/dataHistListForUpdate.do?ACCT_TYPE=' + dataObj.ACCT_TYPE+"&ACCT_CODE="+dataObj.ACCT_CODE //url
    ,cols: [[ 
    	{type:'numbers'},	//列每行序号
		{type:'checkbox'},  //列复选框
		...
		{field: 'mdfcSgmtCode', align:'center', title: '待更正段段标',templet: function(d){return renderColumnDict(d, 'mdfcSgmtCode', enterDict.entLoanInfRecType)}},
		{fixed: 'right',align:'center',  title:'操作', toolbar: '#opraBarRow', width:250} //行上的工具栏
    ]]
    ,even: true  //行斑马线
	,page: true //是否分页
	,limit: 10 //初始每页显示条数
			...
			layer.open({
        	  ...
        	  $("body").data("dataObj",data);
	  			...
	  			layer.open({
	  				...
	  				anim: 2,
	  				resize: false,
	  				content: 'entLoanAcctInfoSeo/loanAcctInfoEdit.do?addOrUpdate=' + addOrUpdate 
	  				         + "&edaAcctBsSgmtId=" + data.edaAcctBsSgmtId
	  				       	 + "&queryType=entLoanUpdateBySegment",
	  				end: function(){
	  					tableReload(); 
	  				}
	  			});
    	...
    	if(flag === 'new'&& businessStates == '50'){
    		layer.confirm('是否确认回退报文(该数据所在报文的全部数据状态为审核成功)？', function(index){
		   	     $.ajax({
		   	       ...
		   	       type : "POST",
		   	       //请求地址
		   	       url : "entLoanAcctInfoSeo/backdate.do?addOrUpdate="+addOrUpdate,
		   	       //数据，json字符串
		   	       data : data,
		   	       //请求成功
