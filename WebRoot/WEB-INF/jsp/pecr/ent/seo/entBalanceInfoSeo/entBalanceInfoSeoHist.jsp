...
//展示已知数据
  table.render({
	...
	,url:'entBalanceInfoSeo/dataHistList.do?ENT_CERT_TYPE=' + dataObj.ENT_CERT_TYPE+'&ENT_CERT_NUM='+dataObj.ENT_CERT_NUM //url
    ,cols: [[ 
    	{type:'checkbox'},  //列复选框
    	...
    	if(flag === 'new'){
			...
			layer.open({
  			...
  			layer.open({
  				...
  				anim: 2,
  				resize: false,
  				content: 'entBalanceInfoSeo/indvBasicsInfoDetail.do?addOrUpdate=' + addOrUpdate
  				         + '&ehaBalanceSheetBsSgmtId=' + data.ehaBalanceSheetBsSgmtId,
  				end: function(){
  					tableReload(); 
  				}
    	...
    	if(flag === 'new'&& businessStates == '50'){
    		layer.confirm('是否确认回退报文(该数据所在报文的全部数据状态为审核成功)？', function(index){
		   	     $.ajax({
		   	       ...
		   	       type : "POST",
		   	       //请求地址
		   	       url : "entBalanceInfoSeo/backdate.do?addOrUpdate="+addOrUpdate,
		   	       //数据，json字符串
		   	       data : data,
		   	       //请求成功
