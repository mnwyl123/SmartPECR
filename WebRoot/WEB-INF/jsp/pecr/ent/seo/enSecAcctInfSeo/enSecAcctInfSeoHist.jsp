...
//展示已知数据
  table.render({
	...
	,url:'enSecAcctInfSeo/dataHistList.do?ACCT_TYPE=' + dataObj.ACCT_TYPE+"&ACCT_CODE="+dataObj.ACCT_CODE //url
    ,cols: [[ 
    	{type:'numbers'},	//列每行序号
		{type:'checkbox'},  //列复选框
		{field: 'eeaGuarAcctBsSgmtId', title: '主键', hide: true},
//		{field: 'mngmtOrgCode', align:'center', title: '所属机构'},
		{field: 'rptDateCode', align:'center', title: '报告时点说明',templet: function(d){return renderColumnDict(d,'rptDateCode',enterDict.entGuarRptDateCode)}},
		{field: 'name', align:'center', title: '姓名'},
        {field: 'idType', align:'center', title: '债务人标识类别',templet: function(d){return renderColumnDict(d,'idType',enterDict.entCertMainType)}},
		{field: 'idNum', align:'center', title: '债务人标识号码'},
    ...
    if(obj.event === 'more'){
			...
			layer.open({
        	  ...
        	  $("body").data("dataObj",data);
	  			...
	  			layer.open({
	  				...
	  				anim: 2,
	  				resize: false,
	  				content: 'enSecAcctInfSeo/enSecAcctInfEdit.do?addOrUpdate=' + addOrUpdate
			         + '&eeaGuarAcctBsSgmtId=' + data.eeaGuarAcctBsSgmtId,
	  				end: function(){
	  					tableReload(); 
	  				}
    	...
    	if(flag === 'new' && (businessStates == '20' || businessStates == '11' || businessStates == '10' || businessStates == '40' || businessStates == '80' || businessStates == '110')){
			...
			layer.open({
				...
				anim: 2,
				resize: false,
				content: 'enSecAcctInf/enSecAcctInfEdit.do?addOrUpdate=' + addOrUpdate + '&eeaGuarAcctBsSgmtId=' + data.eeaGuarAcctBsSgmtId,
				end: function(){
					tableReload(); 
				}
			});
    	...
    	if(flag === 'new'&& businessStates == '50'){
    		layer.confirm('是否确认回退报文(该数据所在报文的全部数据状态为审核成功)？', function(index){
		   	     $.ajax({
		   	       ...
		   	       type : "POST",
		   	       //请求地址
		   	       url : "enSecAcctInfSeo/backdate.do?addOrUpdate="+addOrUpdate,
		   	       //数据，json字符串
		   	       data : data,
		   	       //请求成功
