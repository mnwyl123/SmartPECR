...
//展示已知数据
  table.render({
	...
	,url:'enCtrctInfoSeo/dataHistListForUpdate.do?CONTRACT_CODE=' + dataObj.CONTRACT_CODE //url
    ,cols: [[ 
    	{type:'numbers'},	//列每行序号
		{type:'checkbox'},  //列复选框
		...
		{field: 'mdfcSgmtCode', align:'center', title: '待更正段段标', templet:function(d){return renderColumnDict(d,'mdfcSgmtCode',enterDict.entCreditUpInfRecType)}},
    	...
    	if(flag === 'new'){
			...
			layer.open({
  			...
  			layer.open({
  				...
  				anim: 2,
  				resize: false,
  				content: 'enCtrctInfoSeo/enCtrctInfoEdit.do?addOrUpdate=' + addOrUpdate
  						 + '&egaCtrctBsSgmtId=' + data.egaCtrctBsSgmtId
  						 + "&queryType=entCtrctUpdateBySegment",
  				end: function(){
  					tableReload(); 
  				}
  			});
    	...
    	if(flag === 'new'&& businessStates == '50'){
    		layer.confirm('是否确认回退报文(该数据所在报文的全部数据状态为审核成功)？', function(index){
		   	     $.ajax({
		   	       ...
		   	       type : "POST",
		   	       //请求地址
		   	       url : "enCtrctInfoSeo/backdate.do?addOrUpdate="+addOrUpdate,
		   	       //数据，json字符串
		   	       data : data,
		   	       //请求成功
