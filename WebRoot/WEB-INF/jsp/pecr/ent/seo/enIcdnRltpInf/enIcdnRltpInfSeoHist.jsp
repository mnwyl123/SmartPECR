...
//展示已知数据
  table.render({
	...
	,url:'enIcdnRltpInfSeo/dataHistList.do?ENT_CERT_TYPE=' + dataObj.ENT_CERT_TYPE+'&ENT_CERT_NUM='+dataObj.ENT_CERT_NUM //url
    ,cols: [[ 
    	{type:'numbers'},	//列每行序号
		{type:'checkbox'},  //列复选框
		{field: 'ecaEnIcdnRltpInfId', title: '主键', hide: true},
		{field: 'entName', align:'center', title: 'A企业名称'},
		{field: 'assoEntName', align:'center', title: 'B企业名称'},
    ...
    if(obj.event === 'more'){
    	if(flag === 'new'){
			...
			layer.open({
  			...
  			layer.open({
  				...
  				anim: 2,
  				resize: false,
  				content: 'enIcdnRltpInfSeo/basicsInfoDetail.do?addOrUpdate=' + addOrUpdate
  				         + '&ecaEnIcdnRltpInfId=' + data.ecaEnIcdnRltpInfId,
  				end: function(){
  					tableReload();
  				}
    	...
    	if(flag === 'new' && (businessStates == '20' || businessStates == '11' || businessStates == '10' || businessStates == '40' || businessStates == '80' || businessStates == '110')){
			...
			layer.open({
				...
				anim: 2,
				resize: false,
				content: 'enIcdnRltpInf/enIcdnRltpInfEdit.do?addOrUpdate=' + addOrUpdate+ "&ecaEnIcdnRltpInfId=" + data.ecaEnIcdnRltpInfId,
				end: function(){
					tableReload();
				}
			});
    	...
    	if(flag === 'new'&& businessStates == '50'){
    		layer.confirm('是否确认回退报文(该数据所在报文的全部数据状态为审核成功)？', function(index){
		   	     $.ajax({
		   	       ...
		   	       type : "POST",
		   	       //请求地址
		   	       url : "enIcdnRltpInfSeo/backdate.do?addOrUpdate="+addOrUpdate,
		   	       //数据，json字符串
		   	       data : data,
		   	       //请求成功
