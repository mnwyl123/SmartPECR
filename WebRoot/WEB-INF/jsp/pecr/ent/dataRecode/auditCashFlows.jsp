...
//展示已知数据
  table.render({
	...
	,url:'auditing/dataCashFlows.do' //url
    ,cols: [[ 
      {type:'numbers'}	//列每行序号
      ,{type:'checkbox'} //列复选框
			...
			case 'auditingBtn':
				//获取选中的行信息
				 ...
				 for(var i = 0; i < data.length; i++){
					 strIds+=checkStatus.data[i].ejaCashFlowsBsSgmtId+",";
				}
	...
	var popIndex  = layer.open({
						...
						yes: function (index,layero){
							...
							$.ajax({ 
								...
								data     : {state:'30',strIds:strIds,auditOpinion:auditOpinion,tableName:'E_CASH_FLOWS_BS',tablePk:'EJA_CASH_FLOWS_BS_SGMT_ID'},
								type     : 'post',  
								cache    : false,  
								dataType : 'json',  
					  	...
					  	btn2: function (index,layero){
							...
							$.ajax({ 
								url      : 'auditing/isAuditing.do', 
								data     : {state:'40',strIds:strIds,auditOpinion:auditOpinion,tableName:'E_CASH_FLOWS_BS',tablePk:'EJA_CASH_FLOWS_BS_SGMT_ID'},
								type     : 'post',  
								cache    : false,  
								dataType : 'json',  
								success:function(result){ 
