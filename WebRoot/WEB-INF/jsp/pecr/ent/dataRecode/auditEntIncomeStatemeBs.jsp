			...
			<div class="layui-colla-content layui-show" style="margin:1px 1px 0px 0px">
				<div class="layui-form-item">
					...
					<div class="layui-input-inline">
						<input type="text" name="entName" id="entName" autocomplete="off" class="layui-input"/>
					</div>
					</div>
					<div class="layui-inline">
					<label class="layui-form-label">企业身份标识号码</label>
					<div class="layui-input-inline">
						<input type="text" name="entCertNum" id="entCertNum" autocomplete="off" class="layui-input">
					</div>
					</div>
				</div>
			</div>
...
//展示已知数据
  table.render({
	...
	,url:'auditing/dataEntIncomeStatemeBs.do' //url
    ,cols: [[ 
      {type:'numbers'}	//列每行序号
      ,{type:'checkbox'} //列复选框
		...
		var layEvent = obj.event;
		 if (layEvent === 'more'){
			...
			layer.open({
				...
				anim: 2,
				resize: false,
				content: 'ecdtreportinstie/entIncomeAndExpenseStatemeBsEdit.do?addOrUpdate=' + addOrUpdate
		...
		switch (obj.event) {
			case 'auditingBtn':
				 ...
				 for(var i = 0; i < data.length; i++){
					 strIds+=checkStatus.data[i].elaIncomeAndExpenseStatId+",";
				}
				 ...
				 var popIndex = layer.open({
						...
						yes: function (index,layero){
							...
							$.ajax({ 
								...
								data     : {state:'30',strIds:strIds,auditOpinion:auditOpinion,tableName:'E_INCOME_EXPENSE_bs',tablePk:'ELA_INCOME_AND_EXPENSE_STAT_ID'},
								type     : 'post',  
								cache    : false,  
								dataType : 'json',  
					  	...
					  	btn2: function (index,layero){
							...
							$.ajax({ 
								url      : 'auditing/isAuditing.do', 
								data     : {state:'40',strIds:strIds,auditOpinion:auditOpinion,tableName:'E_INCOME_EXPENSE_bs',tablePk:'ELA_INCOME_AND_EXPENSE_STAT_ID'},
								type     : 'post',  
								cache    : false,  
								dataType : 'json',  
								success:function(result){ 
