...
//展示已知数据
  table.render({
	...
	,url:'auditing/dataEnBasInf.do' //url
    ,cols: [[ 
      {type:'numbers'}	//列每行序号
      ,{type:'checkbox'} //列复选框
		...
		var layEvent = obj.event;
		 if (layEvent === 'more'){
			...
			layer.open({
				...
				anim: 2,
				resize: false,
				content: 'eccustbasis/enBasInfEdit.do?addOrUpdate=' + addOrUpdate 
						+ "&eaaBsSgmtId=" + data.eaaBsSgmtId,
				end: function(){
		...
		switch (obj.event) {
			case 'auditingBtn':
				 ...
				 for(var i = 0; i < data.length; i++){
					 strIds+=checkStatus.data[i].eaaBsSgmtId+",";
				}
				 ...
				 var popIndex  = layer.open({
						...
						yes: function (index,layero){
							...
							$.ajax({ 
								...
								data     : {state:'30',strIds:strIds,auditOpinion:auditOpinion,tableName:'E_BS',tablePk:'EAA_BS_SGMT_ID'},
								type     : 'post',  
								cache    : false,  
								dataType : 'json',  
					  	...
					  	btn2: function (index,layero){
							...
							$.ajax({ 
								url      : 'auditing/isAuditing.do', 
								data     : {state:'40',strIds:strIds,auditOpinion:auditOpinion,tableName:'E_BS',tablePk:'EAA_BS_SGMT_ID'},
								type     : 'post',  
								cache    : false,  
								dataType : 'json',  
								success:function(result){ 
