    ...
    <form class="layui-form layui-form-pane" lay-filter="editForm" id="editForm">
        <div class="layui-form-item" style="margin-left:30px;">
            ...
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <select lay-search="" name="objectionType" id="objectionType" lay-filter="objectionType">
                        <option value="">请选择</option>
                    </select>
                </div>
            </div>
            ...
            <div class="layui-inline" id="objectionStatusDiv">
...
<script type="text/html" id="toolbarDemo">
    ...
    <button class="layui-btn layui-btn-xs layui-btn-normal layui-btn-radius" id="submitBtn" lay-event="submitBtn">提交
...
<script type="text/javascript">
    ...
    layui.use(['form', 'laydate', 'element', 'layer', 'table', 'laypage', 'laytpl'], function () {
        ...
        var loadDataOS = "10";//用来判断加载处于那个状态的异议列表-已登记待提交
        var objType = "APPLY";
        ...
        form.on('select(objectionType)', function (data) {
            $('#objectionSegment').empty();//清空下拉
            var obTValue = data.value;
            if (obTValue === '1') {
                initDict('objectionSegment', staticDict.objectionSegment.slice(0, 9));
            } else if (obTValue === '2') {
            ...
            form.render('select');//渲染--不渲染，没有效果
        });
        ...
        table.on('toolbar(tableDemo)', function (obj) {
            switch (obj.event) {
                case 'addBtn':
                    ...
                    //新增时取消异议状态的输入
                    $('#objectionStatusDiv').attr("style", "display:none;");
                    //显示取消和保存按钮
        ...
        table.on('tool(tableDemo)', function (obj) {
            ...
            if (obj.event === 'more') {
                ...
                layer.confirm('确认要删除该条数据?', {icon: 3, title: '提示'}, function (index) {
                    $.ajax({
                        //请求方式
                        type: "POST",
                        //请求地址
                        url: "IndvObjection/deleteByObjectionById.do",
                        //数据，json字符串
                        dataType: "json",
                        data: data,
                        //请求成功
