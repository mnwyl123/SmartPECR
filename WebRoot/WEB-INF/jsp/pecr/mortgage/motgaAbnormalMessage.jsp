...
table.render({
  elem: '#tableDemo' //表格所固定的div
	...
	,url:'motgaAbnormalMessage/dataList.do' //url
  ,cols: [[ 
    {type:'numbers', fixed: 'left'}	//列每行序号
    ,{type:'checkbox', align:'center'} //列复选框
    ,{field: 'RPTDATE', align:'center', title: '报送日期'}
    ,{field: 'INFRECTYPE', align:'center' , title: '信息记录类型',templet: function(d){return renderColumnDict(d, 'INFRECTYPE', staticDict.motgaInfrectype)}}
    ,{field: 'RECODENUM', align:'center', title: '记录数'}
    ,{field: 'RIGHTRECODENUM', align:'center', title: '正确记录数'}
    ,{field: 'WRONGRECODENUM', align:'center', title: '错误记录数'}
  ]]
  ,even: true  //行斑马线
	,page: true //是否分页
	,limit: 10 //初始每页显示条数
 ...
 table.on('toolbar(tableDemo)', function (obj) {
		switch (obj.event) {
			case 'generateMessage':
				 ...
				 var normalOrAbnormal ="mccabnormal";
				 if(data.length==0){
