...
<!--  base href="<%=basePath%>" -->
<title>报表视图</title>
<link rel="stylesheet" type="text/css"	href="<%=basePath%>css/knacss.css" />
<link rel="stylesheet" type="text/css"	href="<%=basePath%>css/base/hideInput/page.css" />


<!--  link rel="stylesheet" type="text/css"
	href="<%=basePath%>css/styles.css" -->

<link rel="stylesheet" type="text/css"	href="<%=basePath%>css/base/hideInput/operateArea.css" />
<link rel="stylesheet" type="text/css"	href="<%=basePath%>css/base/hideInput/regularReportGrid.css" />

<!-- link rel="stylesheet" type="text/css"
	href="<%=basePath%>css/base/dataGridStyles.css" -->
<link rel="stylesheet" type="text/css"	href="<%=basePath%>css/base/hideInput/inputPanel.css" />
...
function drawTopLeftCell() {
	initLayout($("body"));
	$(".topLeft2bottomRightLine").each(function(td){
	    var width =  $(this).outerWidth() + parseInt($(this).css("padding-left")) + parseInt($(this).css("padding-right"));
		var height = $(this).outerHeight();
		var rgbColor = $(this).css("border-top-color");
		if(/^#([0-9a-fA-f]{3}|[0-9a-fA-f]{6})$/.test(rgbColor)){
			//如果是一个hex值则直接返回
		}
		else if(/^(rgb|RGB)/.test(rgbColor)){
			var rgb = rgbColor.match(/\d+/g);
			function hex(x) {return ("0" + parseInt(x).toString(16)).slice(-2);}
			rgbColor= "#" + hex(rgb[0]) + hex(rgb[1]) + hex(rgb[2]);
		}
		else{
			rgbColor="#000000";  //其它方式缺省为黑色
		}
	    var url = "?topLeft2bottomRightLine=true&width=" + width + "&height=" + height + "&_rgbColor=" + rgbColor.substr(1);
		var idx = window.location.href.indexOf("?");
		var urlStart = window.location.href.substring(0,idx);
		url = urlStart + url; 
        //alert(url);
...
.download .hl .progress_bar {
  ...
  text-align:center;
  font-size: 10px;
  border-bottom:2px solid #39AB26;
}
...
-->
	...
	<div id="commentsPan"
		...
		<div class="download pdf">
			<div class="hl">
			   <div style="width: 100%; text-align:left;">PDF下载</div>
               <div class="progress_bar"></div>
            </div>
			<div class="detail">...</div>
			<div class="downloadop" style="text-align:center;">
		...
		<div class="download xls">
			<div class="hl">
			   <div style="width: 100%; text-align:left;">Excel下载</div>
               <div class="progress_bar"></div>
            </div>
			<div class="detail">...</div>
			<div class="downloadop" style="text-align:center;">
