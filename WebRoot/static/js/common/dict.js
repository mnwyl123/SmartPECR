	...
	DicMain.prototype = {
		...
		formartParamType: function(type) {  //方法名标准化
			...
			var funcName = {};
			 ...
			 if(type == 'div'){
			 	funcName.paramFuncName= 'formartParamDiv';
			 	funcName.dataFuncName= 'formartDataDiv';
			 	funcName.buildFuncName = 'buildDictDiv';
			 	funcName.async = false;
			 }
		...
		formartParamDiv: function() {  //格式化表格参数
			var self = this;
	        var list = [];
		  	$.each(self.config.ids,function(i,item){
				var map ={};
	    ...
	    formartDataDiv:function(data){  //格式化数据
	    	return data;
	    },
	    ...
	    buildDictDiv:function(arr){  //构建下拉框
	    	var self = this;
	    	$.each(arr,function(i,item){
	    		$.each(item.dict,function(j,dict){
	    			if(dict.key == $("#"+item.id).attr("default-value")){
	    				$("#"+item.id).html(dict.value);
	    			}
		    	})
	    	})
