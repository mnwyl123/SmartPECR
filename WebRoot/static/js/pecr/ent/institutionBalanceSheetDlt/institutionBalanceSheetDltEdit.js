		...
		*/
        ...
        $.when(d0,d1).done(function (d0,d1) {
            ...
            form.on('submit(baseSaveBtn)', function(data){
				...
				//提交
				$.ajax({  
					url      : 'institutionBalanceSheetDlt/saveAcctBsSgmt.do', 
					...
					success:function(data){ 
						if (data.success == true){
							...
							baseExistFlag = 'Y';
							dataObj = para; //存储新数据
			                verify.dataValidation('base','E_INSTITU_BALANCE_BS',$this.data("id"),'form',true,false); //数据校验
            ...
            $("#baseDelBtn").on("click",function () {
                ...
                layer.confirm('删除基础段相当于删除该页面所有段信息，是否确认删除', {anim: 6}, function(index){
					...
					//提交
					$.ajax({ 
						url      : 'institutionBalanceSheetDlt/delAcctBsSgmt.do', 
