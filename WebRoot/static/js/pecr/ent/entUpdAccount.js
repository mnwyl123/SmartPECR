...
layui.use(['form', 'laydate','layer', 'element', 'table','laypage','laytpl'], function()
{
	  ...
	  require(['static/js/pecr/module/base'],function (base){
			...
			form.render();//每个页面都有
			 ...
			 var initEnumerationValue = function () {
					var array = ['ENT_INFRECTYPE_UPDATE'];
					var selectData = [];
					...
					$.ajax({  
						...
						success:function(data){
								for(var i=0;i<data.result.length;i++){
									if (data.result[i].ENT_INFRECTYPE_UPDATE){
										selectAdd(data.result[i].ENT_INFRECTYPE_UPDATE,'infRecType');
										selectAdd(data.result[i].ENT_INFRECTYPE_UPDATE,'infRecType1');
								}	
								form.render('select');
							}
							}
	...
	,url:'entUpDaccountData/dataList.do' //url 
    ,cols: [[ 
      {type:'numbers'}	//列每行序号
      ,{type:'checkbox'} //列复选框   
  ...
  if(obj.event === 'del'){
    layer.confirm('真的删除行么', function(index){
 	  $.ajax({
 	       ...
 	       type : "POST",
 	       //请求地址
 	       url : "entUpDaccountData/delete.do",
 	       //数据，json字符串
 	       data : data,
 	       //请求成功
...
form.on('submit(exampleSubmit)', function(data){	
	   $.ajax({
	     ...
	     type : "POST",
	     //请求地址
	     url : "entUpDaccountData/addOrUpdateUser.do?addOrUpdate="+addOrUpdate,
	     //数据，json字符串
	     data : data.field,
	     //请求成功
	     success : function(result) {
