		...
		*/
		...
		$.when(d0,d1).done(function (d0,d1) {
			...
			form.on('submit(saveCommonModelBtn)', function(data){
				var egbCtrctCertRelSgmtId = $("#saveCommonModelBtn").data("id");
				$.ajax({
					type : "POST",  //请求方式
					url  : "enCtrctInfo/saveCtrctCertRelSgmt.do",
					data : {rltData  :  JSON.stringify(data.field),
							egaCtrctBsSgmtId : dataObj.egaCtrctBsSgmtId,
							egbCtrctCertRelSgmtId : egbCtrctCertRelSgmtId
							},
					dataType : 'json',
					success : function(result) {
						if(result.success == true){
							layer.msg('操作成功', {time:3000});
							var newcommInfoUrl = 'enCtrctInfo/getRltInfList.do?egaCtrctBsSgmtId='+ dataObj.egaCtrctBsSgmtId + '&pageType=edit';
							console.log("save       "+$("#saveCommonModelBtn").data("id"));
