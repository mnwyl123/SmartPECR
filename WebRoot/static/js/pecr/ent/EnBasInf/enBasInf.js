...
//                d1.resolve("渲染完成");
	...
	//展示已知数据
	table.render({
		elem: '#enBasInfTab', //表格所固定的div
		toolbar: '#toolbarHead',  //表格顶部工具栏
		url:'eccustbasis/enBasInfList.do',
    	cols: [[
			{type:'numbers'},	//列每行序号
			{type:'checkbox'},  //列复选框
		...
		page: true, //是否分页
		limit: 10, //初始每页显示条数
		height: $(document).height() - $('#enBasInfTab').offset().top - $('#queryPanel').height() - 25,
		limits: [10, 20, 50],
	...
	table.on('toolbar(enBasInfTab)', function(obj){
		if(obj.event === 'addBtn') {
            ...
            layer.open({
			 ...
			 //获取选中的行信息
			 var checkStatus = table.checkStatus('enBasInfTab');
			 //获取选中行的数据
			 ...
			 if(data.length==0 && rptDate.length==0){
				...
				for(var i = 0; i < data.length; i++){
					str+=checkStatus.data[i].eaaBsSgmtId+","
				}
	...
	table.on('tool(enBasInfTab)', function(obj){
		var data = obj.data;
		...
		if(layEvent === 'deleteInfo'){
			layer.confirm('确认要删除该条信息？', function(index){
				$.ajax({
					type : "POST",
					url : "eccustbasis/delBasicsInfo.do",
					data : data,
