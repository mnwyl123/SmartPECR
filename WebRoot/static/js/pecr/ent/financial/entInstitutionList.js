...
//	                d1.resolve("渲染完成");
	...
	//展示已知数据
	table.render({
		elem: '#entInstitutionTab', //表格所固定的div
		toolbar: '#toolbarHead',  //表格顶部工具栏
		url:'entInstitutionInfo/balanceSheetList.do',
		//method : 'post',
		where : {time:new Date()},
    	cols: [[
			{type:'numbers'},	//列每行序号
		...
		page: true, //是否分页
		limit: 10, //初始每页显示条数
		height: $(document).height() - $('#entInstitutionTab').offset().top - $('#queryPanel').height() - 25,
		limits: [10, 20, 50],
	...
	*/
    ...
    //新增
	table.on('toolbar(entInstitutionTab)', function(obj){
		if(obj.event === 'addBtn') {
            ...
            layer.open({
			 ...
			 //获取选中的行信息
			 var checkStatus = table.checkStatus('entInstitutionTab');
			 //获取选中行的数据
			 ...
			 if(data.length==0 && rptDate.length==0){
				...
				for(var i = 0; i < data.length; i++){
					str+=checkStatus.data[i].ekaInstitutionBalanceShtId+","
				}
	...
	table.on('tool(entInstitutionTab)', function(obj){
		var data = obj.data;
		...
		if(layEvent === 'delete'){
			layer.confirm('确认要删除该条信息？', function(index){
				$.ajax({
					type : "POST",
					url : "entInstitutionInfo/delInstitution.do",
					data : data,
