...
//	                d1.resolve("渲染完成");
	...
	//展示已知数据
	table.render({
		elem: '#entBalanceTab', //表格所固定的div
		toolbar: '#toolbarHead',  //表格顶部工具栏
		url:'entBalanceInfo/assetsInfoList.do',
    	cols: [[
			{type:'numbers'},	//列每行序号
			{type:'checkbox'},  //列复选框
			{field: 'ehaBalanceSheetBsSgmtId', title: '主键', hide: true},
		...
		page: true, //是否分页
		limit: 10, //初始每页显示条数
		height: $(document).height() - $('#entBalanceTab').offset().top - $('#queryPanel').height() - 25,
		limits: [10, 20, 50],
	...
	*/
    ...
    //新增
	table.on('toolbar(entBalanceTab)', function(obj){
		if(obj.event === 'addBtn') {
            ...
            layer.open({
			 ...
			 //获取选中的行信息
			 var checkStatus = table.checkStatus('entBalanceTab');
			 //获取选中行的数据
			 ...
			 if(data.length==0 && rptDate.length==0){
				...
				for(var i = 0; i < data.length; i++){
					str+=checkStatus.data[i].ehaBalanceSheetBsSgmtId+","
				}
	...
	table.on('tool(entBalanceTab)', function(obj){
		var data = obj.data;
		...
		if(layEvent === 'delete'){
			layer.confirm('确认要删除该条信息？', function(index){
				$.ajax({
					type : "POST",
					url : "entBalanceInfo/delAssetsInfo.do",
					data : data,
