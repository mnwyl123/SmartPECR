	 ...
	 $.when(d0,d1).done(function (d0,d1) {
		 $("#feedBackError").val(feedBackError);
					...
					$.ajax({
						...
						success : function(data) {
							if(data.success == true){
								...
								var EDB_ACCT_BS_INF_SGMT_ID = '{"EDB_ACCT_BS_INF_SGMT_ID":"'+data.result+'"}'
								acctBsInfSgmt =JSON.parse(EDB_ACCT_BS_INF_SGMT_ID);
							}else{
								layer.alert(data.msg);
							}
						},
				...
				$.ajax({
					...
					success : function(result) {
						if(result.success == true){
							...
							//当基本信息中的的担保方式为1-保证时上报相关还款责任人
                            verify.dataValidation('responsibleEditModel','E_RLT_REPYMT_INF',$("#saveResponsibleModelBtn").data("id"),'form',true,false); //数据校验
                            verify.dataValidation('basic','E_ACCT_BS_INF',acctBsInfSgmt.EDB_ACCT_BS_INF_SGMT_ID,'form',true,false); //数据校验
            ...
            form.on('submit(saveMortgageModelBtn)', function(data){
				$.ajax({
					...
					success : function(result) {
						if(result.success == true){
							layer.msg('操作成功', {time:3000});
							var newmccInfoUrl = 'entLoanAcctInfo/getMccInfList.do?edaAcctBsSgmtId='+ dataObj.edaAcctBsSgmtId + '&pageType=edit';
							table.reload('mortgageDataTable', {url:newmccInfoUrl});
							//当基本信息中的担保方式为抵押或质押时上报抵质押物信息段
                            verify.dataValidation('mortgageEditModel','E_MOTGA_CLTAL_CTRCT_INF',$("#saveMortgageModelBtn").data("id"),'form',true,false); //数据校验
                            verify.dataValidation('basic','E_ACCT_BS_INF',acctBsInfSgmt.EDB_ACCT_BS_INF_SGMT_ID,'form',true,false); //数据校验
							verify.dataValidation('base','E_ACCT_BS',dataObj.edaAcctBsSgmtId,'form',true,false);
	                        layer.close(modelIndex);
						}else{
			...
			if (dataObj && acctCredSgmt){
				...
				verify.dataValidation('amount','E_ACCT_CRED',acctCredSgmt.EDE_ACCT_CRED_SGMT_ID,'form',false,false); //数据校验
			}else{
				$("#amountAddBtn").show();
			}
