...
layui.use(['form', 'laydate', 'element', 'layer', 'table', 'laypage','laytpl'], function(){
	...
	require(['static/js/pecr/module/base'],function(base){
	    ...
	    var initEnumerationValue = function () {
			var array = ['ENT_LOAN_ACCT_TYPE','DATA_STATUS_BLXS'];
			var selectData = [];
...
//	                d1.resolve("渲染完成");
	...
	table.render({
		...
		url:'entLoanAcctInfo/acctInfoList.do',
    	cols: [[
			{type:'numbers'},	//列每行序号
			{type:'checkbox'},  //列复选框
			{field: 'edaAcctBsSgmtId', title: '主键', hide: true},
			{field: 'mngmtOrgCode', align:'center', title: '所属机构', hide: true},
			{field: 'name', align:'center', title: '借款人名称'},
            {field: 'idType', align:'center', title: '借款人身份标识类型', templet:function(d){return renderColumnDict(d,'idType',enterDict.arlpCertType)}},
			{field: 'idNum', align:'center', title: '借款人身份标识号码'},
	...
	table.on('toolbar(entLoanAcctTab)', function(obj){
		if(obj.event === 'addBtn') {
            ...
            layer.open({
                ...
                anim: 2,
                resize: false,
				content: 'entLoanAcctInfo/loanAcctInfoEdit.do?addOrUpdate=' + addOrUpdate
            });
