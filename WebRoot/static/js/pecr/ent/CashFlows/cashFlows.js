...
//                    d1.resolve("渲染完成");
	...
	table.render({
		...
		url:'cashFlows/enSecAcctInfList.do',
    	cols: [[
			{type:'numbers'},	//列每行序号
			{type:'checkbox'},  //列复选框
			{field: 'ejaCashFlowsBsSgmtId', title: '主键', hide: true},
//			{field: 'orgCode', align:'center', title: '所属机构', hide: true},
            ...
            layer.open({
			 ...
			 if(data.length==0 && rptDate.length==0){
				...
				for(var i = 0; i < data.length; i++){
					str+=checkStatus.data[i].ejaCashFlowsBsSgmtId+","
				}
	...
	table.on('tool(indvGuarAcctTab)', function(obj){
		...
		if (layEvent === 'editInfo'){
			...
			layer.confirm('确认要删除该条信息？', function(index){
				$.ajax({
					type : "POST",
					url : "cashFlows/delAcctBsSgmt.do",
					data : data,
