...
layui.use(['form', 'laydate', 'element', 'layer', 'table', 'laypage','laytpl'], function(){
	...
	require(['static/js/pecr/module/base'],function(base){
	    ...
	    var initEnumerationValue = function () {
			var array = ['DATA_STATUS_ZHCX','ENT_LOAN_ACCT_TYPE','ENT_CERT_MAIN_TYPE','ENT_LOAN_INF_REC_TYPE'];
			var selectData = [];
			...
			$.ajax({  
				...
				success:function(data){
					for(var i=0;i<data.result.length;i++){
						if (data.result[i].ENT_LOAN_ACCT_TYPE){
							base.renderTpl({
								tplId : "enumerationValueTpl",
								arr : {"list" : data.result[i].ENT_LOAN_ACCT_TYPE},
...
//	                d1.resolve("渲染完成");
	...
	table.render({
		...
		url:'entUpDaccountData/loanList.do',
    	cols: [[
			{type:'numbers'},	//列每行序号
			{type:'checkbox'},  //列复选框
			{field: 'edaAcctBsSgmtId ', title: '主键', hide: true},
//			{field: 'mngmtOrgCode', align:'center', title: '所属机构'},
			...
			{field: 'acctType', align:'center', title: '账户类型', templet:function(d){return renderColumnDict(d,'acctType',enterDict.enLoanAcctType)}},
			...
			{align : "center",field: 'mdfcSgmtCode', title: '待更正段段标',templet: function(d){return renderColumnDict(d, 'mdfcSgmtCode',enterDict.entLoanInfRecType)}},
	...
	//新增
	table.on('toolbar(entLoanAcctTab)', function(obj){
		if(obj.event === 'addBtn') {
			 popIndex = layer.open({
				type: 2,
				title: '获取企业借贷交易上报历史数据列表',
				shadeClose: false,
				shade: [0.3, '#000'],
				area: ['80%', '70%'],
				anim: 2,
				resize: false,
				content: 'entUpDaccountData/goEntLoanSearch.do',
				end: function(){
					tableReload(); 
				}
			});
			...
			//获取选中的行信息
			 var checkStatus = table.checkStatus('entLoanAcctTab');
			 //获取选中行的数据
			...
			for(var i = 0; i < data.length; i++){
				str+=checkStatus.data[i].edaAcctBsSgmtId+","
			}
	...
	table.on('tool(entLoanAcctTab)', function(obj){
		...
		if(layEvent === 'delete'){
			layer.confirm('确认要删除该条信息？', function(index){
				$.ajax({
					type : "POST",
					url : "entLoanAcctInfo/delBasicsInfo.do",
					data : data,
			...
			layer.open({
				...
				anim: 2,
				resize: false,
				content: 'entUpDaccountData/entLoanChangeEdit.do?addOrUpdate=' + addOrUpdate 
				         + "&edaAcctBsSgmtId=" + data.edaAcctBsSgmtId,
				end: function(){
					tableReload();
				}
			});
