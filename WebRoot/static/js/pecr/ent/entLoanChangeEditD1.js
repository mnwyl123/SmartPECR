...
layui.use(['element', 'form','layer','laydate','table','laytpl'], function(){
	...
	require(['static/js/pecr/module/base','static/js/pecr/module/verify','static/js/pecr/module/config'],function(base,verify,config){
		...
		var acctBsSgmt, acctBsInfSgmt, acctCredSgmt, creditorInfSgmt, lbltyInfSgmt,feedBackError;
		if(pageType === 'edit'){
			//基础段
			...
			$.ajax({  
				...
				success:function(data){
					...
					if (data.result[i].ENT_CERT_MAIN_TYPE){
							...
							continue;
						} else if  (data.result[i].ENT_LOAN_REPAY_MODE_D1){
							base.renderTpl({
								tplId : "enumerationValueTpl",
								arr : {"list" : data.result[i].ENT_LOAN_REPAY_MODE_D1},
							...
							continue;
						} else if  (data.result[i].ENT_LOAN_TIME_LIM_CAT){
							base.renderTpl({
								tplId : "enumerationValueTpl",
								arr : {"list" : data.result[i].ENT_LOAN_TIME_LIM_CAT},
								containerId : "loanTimeLimCat"
							});
							continue;
							...
							continue;
						}else if (data.result[i].ENT_LOAN_INF_REC_TYPE){
							base.renderTpl({
								tplId : "enumerationValueTpl",
	 ...
	 $.when(d0,d1).done(function (d0,d1) {
		 $("#feedBackError").val(feedBackError);
				...
				$.ajax({  
					...
					success:function(data){ 
						if (data.success == true){
							...
							baseExistFlag = 'Y';
							dataObj = para; //存储新数据
							verify.dataValidation('base','E_ACCT_BS',$this.data("id"),'form',true,false); //数据校验
						} else {
							layer.alert(data.msg);
						}
					 },
			...
			if (dataObj && acctBsInfSgmt){
				...
				$("#loanTimeLimCat").val(acctBsInfSgmt.LOAN_TIME_LIM_CAT);
			...
			form.on('submit(basicSaveBtn)', function(data){
				...
				para.loanTimeLimCat = $("#loanTimeLimCat").val();
...
//							verify.dataValidation('base','E_ACCT_BS',dataObj.edaAcctBsSgmtId,'form',true,false); //数据校验
						} else {
							layer.alert(data.msg);
						}
					 },
...
//    							$this.hide();
				...
				$.ajax({
					...
					success : function(result) {
						if(result.success == true){
                            ...
                            table.reload('responsibleDataTable', {});
                            layer.close(modelIndex);
                            verify.dataValidation('responsible','E_RLT_REPYMT_INF',$("#saveResponsibleModelBtn").data("id"),'form',true,false); //数据校验
						}else{
			...
			var mccInfoUrl;
			if(pageType === 'edit'){
				mccInfoUrl = 'entLoanAcctInfo/getMccInfList.do?edaAcctBsSgmtId='+ dataObj.edaAcctBsSgmtId 
						+ "&pageType=" + pageType;
				$("#mortgageAddBtn").parent().children('div.layui-colla-content').addClass('layui-show');
			} else {
				mccInfoUrl = 'entLoanAcctInfo/getMccInfList.do?pageType='+ pageType;
			}
            ...
            table.on('tool(mortgageDataTable)', function(obj){
                ...
                if (layEvent === 'edit'){ //编辑
                    ...
                    verify.dataValidation('mortgage','E_MOTGA_CLTAL_CTRCT_INF',data.EDD_MOTGA_CLTAL_CTRCT_INF_ID,'form',false,false); //数据校验
                    modelIndex = layer.open({
                        type: 1,
                        title: "编辑",
                        shade: [0.3, '#000'],
					...
					layer.confirm('确认删除该数据信息?', {anim: 6}, function(index){
						$.ajax({ 
							url      : 'entLoanAcctInfo/mccInfDel.do', 
							data     : {str : JSON.stringify(para)},
							type     : 'post',  
							cache    : false,  
							dataType : 'json',  
            ...
            form.on('submit(saveMortgageModelBtn)', function(data){
				$.ajax({
					type : "post",  //请求方式
					url  : "entLoanAcctInfo/saveMccInf.do",
					...
					success : function(result) {
						if(result.success == true){
                            ...
                            table.reload('mortgageDataTable', {});
                            layer.close(modelIndex);
                            verify.dataValidation('mortgage','E_MOTGA_CLTAL_CTRCT_INF',$("#saveMortgageModelBtn").data("id"),'form',true,false); //数据校验
						}else{
							layer.alert(result.msg);
						}
					},
			...
			if (dataObj && acctCredSgmt){
				...
				verify.dataValidation('amount','PECR_ENT_ACCT_CRED_SGMT',acctCredSgmt.EDE_ACCT_CRED_SGMT_ID,'form',false,false); //数据校验
			}else{
				$("#amountAddBtn").show();
			}
            ...
            form.on('submit(amountSaveBtn)', function(data){
				...
				//提交
				$.ajax({  
					url      : 'entLoanAcctInfo/saveAcctCredSgmt.do', 
					data     : {str:JSON.stringify(para), pageType : $this.data("type")},
					type     : 'post',  
					cache    : false,  
					dataType : 'json',  
					success:function(data){ 
						if (data.success == true){
							...
							$this.data("type","edit");
							verify.dataValidation('amount','E_ACCT_CRED',para.edeAcctCredSgmtId,'form',true,false); //数据校验
						} else {
							layer.alert(data.msg);
						}
					 },
            ...
            $("#amountDelBtn").on("click",function () {
				...
				layer.confirm('确认删除', {anim: 6}, function(index){
                    ...
                    para.edeAcctCredSgmtId = $this.data("id");
					$.ajax({ 
						url      : 'entLoanAcctInfo/acctCredSgmtDel.do', 
						data     : {str : JSON.stringify(para)},
						type     : 'post',  
						cache    : false,  
						dataType : 'json',  
				...
				$.ajax({
					...
					success : function(result) {
						if(result.success == true){
                            ...
                            table.reload('businessDataTable', {});
                            layer.close(modelIndex);
                            verify.dataValidation('business','E_ACCT_SPEC_TRST',$("#saveBusinessModelBtn").data("id"),'form',true,false); //数据校验
						}else{
							layer.alert(result.msg);
						}
					},
