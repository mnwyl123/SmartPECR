		...
		*/
		...
		$.when(d0,d1).done(function (d0,d1) {
			...
			form.on('submit(baseSaveBtn)', function(data){
				...
				//提交
				$.ajax({  
					url	  : 'enCtrctInfoSeo/saveCtrctBsSgmt.do', 
			...
			$("#baseDelBtn").on("click",function () {
				...
				layer.confirm('删除基础段相当于删除该页面所有段信息，是否确认删除', {anim: 6}, function(index){
					...
					//提交
					$.ajax({ 
						url	  : 'enCtrctInfoSeo/deleteOneRecord.do', 
			...
			if(pageType === 'edit'){
				...
				commInfoUrl = 'enCtrctInfoSeo/getRltInfList.do?pageType='+ pageType;
			}
			...
			table.render({
				...
				cols : [[
					...
					{align : "center",field : "CERT_REL_IDTYPE",title : "共同受信人身份标识类型"},
					{align : "center",field : "CERT_REL_ID_NUM",title : "共同受信人身份标识号码"},
					{align : "center",title : "操作",width : 160,toolbar : "#dataTableToolBarTpl"}
				]],
				limit : 5,
				even  : true, //开启隔行背景
				page  : true,
			...
			table.on('tool(commonDataTable)', function(obj){
				...
				if (layEvent === 'edit'){ //编辑
					...
					layer.confirm('确认删除该数据信息?', {anim: 6}, function(index){
						$.ajax({ 
							url	  : 'enCtrctInfoSeo/delCtrctCertRelSgmt.do', 
			...
			form.on('submit(saveCommonModelBtn)', function(data){
				$.ajax({
					type : "POST",  //请求方式
					url  : "enCtrctInfoSeo/saveCtrctCertRelSgmt.do",
			...
			form.on('submit(amountSaveBtn)', function(data){
				...
				//提交
				$.ajax({  
					url	  : 'enCtrctInfoSeo/saveCreditLimSgmt.do', 
			...
			$("#amountDelBtn").on("click",function () {
				...
				layer.confirm('确认删除', {anim: 6}, function(index){
					...
					para.egcCreditLimSgmtId = $this.data("id");
					$.ajax({ 
						url	  : 'enCtrctInfoSeo/delCreditLimSgmt.do', 
						data	 : {str : JSON.stringify(para)},
						type	 : 'post',  
						cache	: false,  
						dataType : 'json',  
