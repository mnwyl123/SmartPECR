...
layui.use(['element', 'form','layer','laydate','table','laytpl'], function(){
    ...
    require(['static/js/pecr/module/base','static/js/pecr/module/config'],function(base,config){
		...
		var enIcdnRltpInfSeo;
		if(pageType === 'edit'){
			enIcdnRltpInfSeo = JSON.parse(document.getElementById("enIcdnRltpInfSeo").value);
		}
		...
		*/
        ...
        var initEnumerationValue = function () {
			var array = ['ENT_CERT_TYPE','ENT_ASSO_TYPE','ENT_ASSO_SIGN'];
			var selectData = [];
        ...
        $.when(d0,d1).done(function (d0,d1) {
            ...
            if (dataObj && enIcdnRltpInfSeo){
                ...
                $("#entName").val(enIcdnRltpInfSeo.ENT_NAME);
                $("#entCertType").val(enIcdnRltpInfSeo.ENT_CERT_TYPE);
                $("#entCertNum").val(enIcdnRltpInfSeo.ENT_CERT_NUM);
                $("#assoEntName").val(enIcdnRltpInfSeo.ASSO_ENT_NAME);
                $("#assoEntCertType").val(enIcdnRltpInfSeo.ASSO_ENT_CERT_TYPE);
                $("#assoEntCertNum").val(enIcdnRltpInfSeo.ASSO_ENT_CERT_NUM);
                $("#assoType").val(enIcdnRltpInfSeo.ASSO_TYPE);
                $("#assoSign").val(enIcdnRltpInfSeo.ASSO_SIGN);
                $("#isFilter").val(enIcdnRltpInfSeo.IS_FILTER);
                $("#rptDate").val(enIcdnRltpInfSeo.RPT_DATE);
                form.render('select');
            }else{
                $("#baseSaveBtn").show();
            }
            ...
            form.on('submit(baseSaveBtn)', function(data){
                var $this = $(this);
				var para = {};
				para.ecaEnIcdnRltpInfId = $this.data("id");
				...
				//提交
				$.ajax({  
					url      : 'enIcdnRltpInfSeo/saveEnIcdnRltpInf.do', 
            ...
            $("#baseDelBtn").on("click",function () {
                ...
                layer.confirm('删除基础段相当于删除该页面所有段信息，是否确认删除', {anim: 6}, function(index){
					...
					$.ajax({ 
						url      : 'enIcdnRltpInfSeo/delEnIcdnRltpInf.do', 
