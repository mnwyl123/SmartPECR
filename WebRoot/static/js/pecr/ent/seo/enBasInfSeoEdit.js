		...
		*/
        ...
        $.when(d0,d1).done(function (d0,d1) {
            ...
            form.on('submit(dcentBsSgmtSub)', function(data){
				...
				//提交
				$.ajax({  
					url      : 'enBasInfSeo/saveEntBsSgmt.do', 
            ...
            $("#dcentBsSgmtDel").on("click",function () {
                ...
                layer.confirm('删除基础段相当于删除该页面所有段信息，是否确认删除', {anim: 6}, function(index){
					...
					//提交
					$.ajax({ 
						url      : 'enBasInfSeo/deleteEntBsSgmt.do', 
            ...
            var rltInfoUrl;
			if(pageType === 'edit'){
				rltInfoUrl = 'enBasInfSeo/getentIdSgmtList.do?eaaBsSgmtId='+ dataObj.eaaBsSgmtId 
						+ "&pageType=" + pageType;
				$("#responsibleAddBtn").parent().children('div.layui-colla-content').addClass('layui-show');
			} else {
				rltInfoUrl = 'enBasInfSeo/getentIdSgmtList.do?pageType='+ pageType;
			}
            ...
            table.on('tool(entIdSgmtTable)', function(obj){
                ...
                if (layEvent === 'edit'){ //编辑
                    ...
                    form.render('select');
                    $("#cancelentIdSgmtModelBtn").data("id",data.eabIdSgmtId);
					modelIndex = layer.open({
                        type: 1,
                        title: "编辑",
                        shade: [0.3, '#000'],
					...
					layer.confirm('确认删除该数据信息?', {anim: 6}, function(index){
						$.ajax({ 
							url      : 'enBasInfSeo/deleteEntIdSgmt.do', 
							data     : {str : JSON.stringify(para)},
							type     : 'post',  
							cache    : false,  
							dataType : 'json',  
            ...
            form.on('submit(saveentIdSgmtModelBtn)', function(data){
				...
				$.ajax({
					type : "POST",  //请求方式
					url  : "enBasInfSeo/saveEntIdSgmt.do",
					data:{str : JSON.stringify(para), pageType : $this.data("type")},
					dataType : 'json',
            ...
            if (dataObj && entFcsInfSgmt){
                ...
                $("#dcentFcsInfSgmtAdd").show();
            }
			
			//点击新增按钮
			$("#dcentFcsInfSgmtAdd").on("click",function () {
				if (dataObj || baseExistFlag === 'Y'){
					var $this = $(this);
					$.ajax({
						type : "POST",  //请求方式
            ...
            form.on('submit(dcentFcsInfSgmtSub)', function(data){
				...
				//提交
				$.ajax({  
					url      : 'enBasInfSeo/saveEntFcsInfSgmt.do', 
					data     : {str:JSON.stringify(para), pageType : $this.data("type")},
					type     : 'post',  
					cache    : false,  
					dataType : 'json',  
            ...
            $("#dcentFcsInfSgmtDel").on("click",function () {
                ...
                layer.confirm('确认删除', {anim: 6}, function(index){
                    ...
                    para.eacFcsInfSgmtId = $this.data("id");
					$.ajax({ 
						url      : 'enBasInfSeo/delAcctBsInfSgmt.do', 
						data     : {str : JSON.stringify(para)},
						type     : 'post',  
						cache    : false,  
						dataType : 'json',  
            ...
            form.on('submit(deentCotaInfSgmtSaveBtn)', function(data){
				...
				//提交
				$.ajax({  
					url      : 'enBasInfSeo/saveEntCotaInfSgmt.do', 
					data     : {str:JSON.stringify(para), pageType : $this.data("type")},
					type     : 'post',  
					cache    : false,  
					dataType : 'json',  
            ...
            $("#deentCotaInfSgmtDelBtn").on("click",function () {
                ...
                layer.confirm('确认删除', {anim: 6}, function(index){
                    ...
                    para.eahCotaInfSgmtId = $this.data("id");
					$.ajax({ 
						url      : 'enBasInfSeo/deleteEntCotaInfSgmt.do', 
						data     : {str : JSON.stringify(para)},
						type     : 'post',  
						cache    : false,  
						dataType : 'json',  
            ...
            var rltInfoUrl;
			if(pageType === 'edit'){
				rltInfoUrl = 'enBasInfSeo/getentMnMmbInfSgmtList.do?eaaBsSgmtId='+ dataObj.eaaBsSgmtId 
						+ "&pageType=" + pageType;
				$("#EntMnMmbInfSgmtAddBtn").parent().children('div.layui-colla-content').addClass('layui-show');
			} else {
				rltInfoUrl = 'enBasInfSeo/getentMnMmbInfSgmtList.do?pageType='+ pageType;
			}
            ...
            table.on('tool(EntMnMmbInfSgmtTable)', function(obj){
                ...
                if (layEvent === 'edit'){ //编辑
                    ...
                    form.render('select');
                    $("#cancelentMnMmbInfSgmtModelBtn").data("id",data.eadMnMmbInfSgmtId);
					modelIndex = layer.open({
                        type: 1,
                        title: "编辑",
                        shade: [0.3, '#000'],
					...
					layer.confirm('确认删除该数据信息?', {anim: 6}, function(index){
						$.ajax({ 
							url      : 'enBasInfSeo/deleteEntMnMmbInfSgmt.do', 
							data     : {str : JSON.stringify(para)},
							type     : 'post',  
							cache    : false,  
							dataType : 'json',  
            ...
            form.on('submit(saveentMnMmbInfSgmtModelBtn)', function(data){
				...
				$.ajax({
					type : "POST",  //请求方式
					url  : "enBasInfSeo/saveEntMnMmbInfSgmt.do",
					data:{str : JSON.stringify(para), pageType : $this.data("type")},
					dataType : 'json',
					success : function(result) {
						if(result.success == true){
            ...
            var mccInfoUrl;
			if(pageType === 'edit'){
				mccInfoUrl = 'enBasInfSeo/getentMnShaHodInfSgmtList.do?eaaBsSgmtId='+ dataObj.eaaBsSgmtId 
						+ "&pageType=" + pageType;
				$("#entMnShaHodInfSgmtAddBtn").parent().children('div.layui-colla-content').addClass('layui-show');
			} else {
				mccInfoUrl = 'enBasInfSeo/getentMnShaHodInfSgmtList.do?pageType='+ pageType;
			}
            ...
            table.on('tool(entMnShaHodInfSgmtTable)', function(obj){
                ...
                console.log(data);
                if (layEvent === 'edit'){ //编辑
                	$("#cancelentIdSgmtModelBtn").data("id",data.eaeMnShaHodInfSgmtId);
					...
					layer.confirm('确认删除该数据信息?', {anim: 6}, function(index){
						$.ajax({ 
							url      : 'enBasInfSeo/deleteEntMnShaHodInfSgmt.do', 
							data     : {str : JSON.stringify(para)},
							type     : 'post',  
							cache    : false,  
							dataType : 'json',  
            ...
            form.on('submit(saveentMnShaHodInfSgmtModelBtn)', function(data){
				...
				$.ajax({
					type : "post",  //请求方式
					url  : "enBasInfSeo/saveEntMnShaHodInfSgmt.do",
					data:{str : JSON.stringify(para), pageType : $this.data("type")},
					dataType : 'json',
					success : function(result) {
						if(result.success == true){
							layer.msg('操作成功', {time:3000});
                            table.reload('entMnShaHodInfSgmtTable', {});
                            layer.close(modelIndex);
						}else{
							layer.alert(result.msg);
						}
            ...
            var mccInfoUrl;
			if(pageType === 'edit'){
				mccInfoUrl = 'enBasInfSeo/getEntActuCotrlInfSgmtList.do?eaaBsSgmtId='+ dataObj.eaaBsSgmtId 
						+ "&pageType=" + pageType;
				$("#entActuCotrlInfSgmtAddBtn").parent().children('div.layui-colla-content').addClass('layui-show');
			} else {
				mccInfoUrl = 'enBasInfSeo/getEntActuCotrlInfSgmtList.do?pageType='+ pageType;
			}
            ...
            table.on('tool(entActuCotrlInfSgmtTable)', function(obj){
                ...
                var layEvent = obj.event;
                if (layEvent === 'edit'){ //编辑
                    $("#cancelentIdSgmtModelBtn").data("id",data.eafActuCotrlInfSgmtId);
                    $("#actuCotrlCertType").val(data.actuCotrlCertType);
                    $("#actuCotrlName").val(data.actuCotrlName);
                    $("#actuCotrlIdType").val(data.actuCotrlIdType);
					...
					layer.confirm('确认删除该数据信息?', {anim: 6}, function(index){
						$.ajax({ 
							url      : 'enBasInfSeo/deleteEntActuCotrlInfSgmt.do', 
							data     : {str : JSON.stringify(para)},
							type     : 'post',  
							cache    : false,  
							dataType : 'json',  
            ...
            form.on('submit(saveentActuCotrlInfSgmtModelBtn)', function(data){
				...
				$.ajax({
					type : "post",  //请求方式
					url  : "enBasInfSeo/saveEntActuCotrlInfSgmt.do",
					data:{str : JSON.stringify(para), pageType : $this.data("type")},
					dataType : 'json',
					success : function(result) {
						if(result.success == true){
            ...
            form.on('submit(dcentSpvsgAthrtyInfSgmtSaveBtn)', function(data){
				...
				//提交
				$.ajax({  
					url      : 'enBasInfSeo/saveEntSpvsgAthrtyInfSgmt.do', 
					data     : {str:JSON.stringify(para), pageType : $this.data("type")},
					type     : 'post',  
					cache    : false,  
					dataType : 'json',  
            ...
            $("#dcentSpvsgAthrtyInfSgmtDelBtn").on("click",function () {
                ...
                layer.confirm('确认删除', {anim: 6}, function(index){
                    ...
                    para.eagSpvsgAthrtyInfSgmtId = $this.data("id");
                    $.ajax({ 
						url      : 'enBasInfSeo/deleteEntSpvsgAthrtyInfSgmt.do',
						data     : {str : JSON.stringify(para)},
						type     : 'post',  
						cache    : false,  
						dataType : 'json',  
