...
layui.use(['element', 'form','layer','laydate','table','laytpl'], function(){
 ...
 //                   d1.resolve("渲染完成");
	...
	//初始化列表
	table.render({
		elem: '#enIcdnRltpInfTab', //表格所固定的div
		toolbar: '#toolbarHead',  //表格顶部工具栏
		url:'enIcdnRltpInf/enIcdnRltpInfList.do',
    	cols: [[
			{type:'numbers'},	//列每行序号
			{type:'checkbox'},  //列复选框
			...
			{field: 'assoSign', align:'center', title: '关联关系有效标志',templet: function(d){return renderColumnDict(d, 'assoSign', enterDict.entAssoSign)}},
//			{field: 'orgName', align:'center', title: '数据来源机构名称'},
		...
		page: true, //是否分页
		limit: 10, //初始每页显示条数
		height: $(document).height() - $('#enIcdnRltpInfTab').offset().top - $('#queryPanel').height() - 25,
		limits: [10, 20, 50],
	 ...
	 * 点击新增按钮
	 */
	table.on('toolbar(enIcdnRltpInfTab)', function(obj){
		if(obj.event === 'addBtn') {
            ...
            layer.open({
			 ...
			 //获取选中的行信息
			 var checkStatus = table.checkStatus('enIcdnRltpInfTab');
			 //获取选中行的数据
			 ...
			 if(data.length==0 && rptDate.length==0){
				...
				for(var i = 0; i < data.length; i++){
					str+=checkStatus.data[i].ecaEnIcdnRltpInfId+","
				}
				...
				layer.confirm('确定要强制提交选中的数据吗？', {icon: 3, title: '提示信息'}, function (index){
					$.ajax({
						...
						success : function(result) { //请求成功
							...
							table.reload('enIcdnRltpInfTab'/*'enBasInfTab'*/, {
								where:{
									rptDate   :   $('#rptDate').val(), 
									entName      :   $('#entName').val(), 
									entCertType      :   $('#entCertType').val(), 
	 ...
	 * 工具栏操作
	 */
	table.on('tool(enIcdnRltpInfTab)', function(obj){
		var data = obj.data;
