...
layui.use(['element', 'form','layer','laydate','table'], function(){
	...
	table.render({
		...
		url:'incomeAndExpenseStatementDlt/List.do',
    	cols: [[
			{type:'numbers'},	//列每行序号
			{type:'checkbox'},  //列复选框
	...
	table.on('toolbar(indvGuarAcctTab)', function(obj){
		if(obj.event === 'addBtn') {
            ...
            layer.open({
                ...
                anim: 2,
                resize: false,
				content: 'incomeAndExpenseStatementDlt/guarAcctInfoEdit.do?addOrUpdate=' + addOrUpdate
            });
		}
	});
	...
	table.on('tool(indvGuarAcctTab)', function(obj){
		...
		if (layEvent === 'editInfo'){
			...
			layer.confirm('确认要删除该条信息？', function(index){
				$.ajax({
					type : "POST",
					url : "incomeAndExpenseStatementDlt/delAcctBsSgmt.do",
					data : data,
