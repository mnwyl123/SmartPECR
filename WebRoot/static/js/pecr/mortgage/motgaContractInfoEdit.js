...
layui.use(['element', 'form','layer','laydate','table','laytpl'], function(){
    ...
    require(['static/js/pecr/module/base','static/js/pecr/module/verify','static/js/pecr/module/config'],function(base,verify,config){
		...
		*/
		 ...
		 * 监听合同类型，展示不同的信息段
		 */
		form.on('select(guarType)', function(data){
        ...
        $.when(d0,d1).done(function (d0,d1) {
            ...
            if (dataObj && motgaCltalBsSgmt){
                ...
                form.render('select');
                verify.dataValidation('base','MOTGA_CLTAL_CTRCT_BS',dataObj.efaMotgaCltalCtrctBsId,'form',false,false); //校验错误信息回显
            }else{
                $("#baseAddBtn").show();
            }
            ...
            form.on('submit(baseSaveBtn)', function(data){
				...
				//提交
				$.ajax({  
					url      : 'motgaInfo/saveMotgaCltalSgmt.do', 
					...
					success:function(data){ 
						if (data.success){
							...
							baseExistFlag = 'Y';
							dataObj = para; //存储新数据
							verify.dataValidation('base','MOTGA_CLTAL_CTRCT_BS',$this.data("id"),'form',true,false); //数据校验
						} else {
							layer.alert(data.msg);
						}
					 },
            ...
            $("#baseDelBtn").on("click",function () {
                ...
                layer.confirm('删除基础段相当于删除该页面所有段信息，是否确认删除', {anim: 6}, function(index){
					...
					//提交
					$.ajax({ 
						url      : 'motgaInfo/delMotgaCltalSgmt.do', 
            ...
            if (dataObj && motgaCltalInfSgmt){
				...
				form.render('select');
				verify.dataValidation('basic','MOTGA_CLTAL_BS_INF',motgaCltalInfSgmt.EFB_MOTGA_CLTAL_BS_INF_SGMT_ID,'form',false,false); //校验错误信息回显
            }else{
                $("#basicAddBtn").show();
            }
					...
					$.ajax({
						...
						success : function(data) {
							if(data.success == true){
								...
								//20210302,基础段信息以及后续的信息段校验做准备
								var EFB_MOTGA_CLTAL_BS_INF_SGMT_ID ='{"EFB_MOTGA_CLTAL_BS_INF_SGMT_ID":"'+data.result+'"}'
								motgaCltalInfSgmt = JSON.parse(EFB_MOTGA_CLTAL_BS_INF_SGMT_ID);
							}else{
								layer.alert(data.msg);
							}
						},
            ...
            form.on('submit(basicSaveBtn)', function(data){
				...
				//提交
				$.ajax({  
					url      : 'motgaInfo/saveMotgaCltalInfo.do', 
					data     : {str:JSON.stringify(para), pageType : $this.data("type")},
					type     : 'post',  
					cache    : false,  
					dataType : 'json',  
					success:function(data){ 
						if (data.success == true){
							...
							$this.data("type","edit");
							verify.dataValidation('basic','MOTGA_CLTAL_BS_INF',$this.data("id"),'form',true,false);
							verify.dataValidation('base','MOTGA_CLTAL_CTRCT_BS',dataObj.efaMotgaCltalCtrctBsId,'form',true,false);
						} else {
							layer.alert(data.msg);
						}
					 },
            ...
            $("#basicDelBtn").on("click",function () {
                ...
                layer.confirm('确认删除', {anim: 6}, function(index){
                    ...
                    para.efbMotgaCltalBsInfSgmtId = $this.data("id");
					$.ajax({ 
						url      : 'motgaInfo/delMotgaCltalInfo.do', 
						data     : {str : JSON.stringify(para)},
						type     : 'post',  
						cache    : false,  
						dataType : 'json',  
            ...
            var rltInfoUrl;
			if(pageType === 'edit'){
				rltInfoUrl = 'motgaInfo/getComRecInfSgmtList.do?efaMotgaCltalCtrctBsId='+ dataObj.efaMotgaCltalCtrctBsId 
						+ "&pageType=" + pageType;
				$("#debtorAddBtn").parent().children('div.layui-colla-content').addClass('layui-show');
			} else {
				rltInfoUrl = 'motgaInfo/getComRecInfSgmtList.do?pageType='+ pageType;
			}
			...
			table.render({
				...
				cols : [[
					...
					{align:"center",field:"INFO_ID_TYPE",title:"身份类别", templet:function(d){return renderColumnDict(d,'INFO_ID_TYPE',staticDict.infoIdType)}},
					{align:"center",field:"GUAR_NAME",title:"其他债务人名称"},
					{align:"center",field:"GUAR_CERT_TYPE",title:"其他债务人身份标识类型", templet:function(d){return renderColumnDictComsharHodIdType(d,'GUAR_CERT_TYPE',staticDict.idType,staticDict.entCertMainType,'INFO_ID_TYPE')}},
            ...
            table.on('tool(debtorDataTable)', function(obj){
                ...
                if (layEvent === 'edit'){ //编辑
                    ...
                    verify.dataValidation('debtor','MOTGA_COM_REC_INF',data.EFC_COM_REC_INF_SGMT_ID,'form',false,false); //校验错误信息回显
					modelIndex = layer.open({
                        type: 1,
                        title: "编辑",
                        shade: [0.3, '#000'],
					...
					layer.confirm('确认删除该数据信息?', {anim: 6}, function(index){
						$.ajax({ 
							url      : 'motgaInfo/delComRecInfSgmt.do', 
							data     : {str : JSON.stringify(para)},
							type     : 'post',  
							cache    : false,  
							dataType : 'json',  
            ...
            form.on('submit(saveDebtorModelBtn)', function(data){
				$.ajax({
					type : "POST",  //请求方式
					url  : "motgaInfo/saveComRecInfSgmt.do",
					...
					success : function(result) {
						if(result.success == true){
                            ...
                            table.reload('debtorDataTable', {});
                            layer.close(modelIndex);
                            //module字段放置弹出框的div命名ID
                            verify.dataValidation('debtorEditModel','MOTGA_COM_REC_INF',$("#saveDebtorModelBtn").data("id"),'form',true,false);
                            verify.dataValidation('basic','MOTGA_CLTAL_BS_INF',motgaCltalInfSgmt.EFB_MOTGA_CLTAL_BS_INF_SGMT_ID,'form',true,false);
							verify.dataValidation('base','MOTGA_CLTAL_CTRCT_BS',dataObj.efaMotgaCltalCtrctBsId,'form',true,false);
						}else{
            ...
            var proptUrl;
			if(pageType === 'edit'){
				proptUrl = 'motgaInfo/getMotgaProptInfList.do?efaMotgaCltalCtrctBsId='+ dataObj.efaMotgaCltalCtrctBsId 
						+ "&pageType=" + pageType;
				$("#pawnAddBtn").parent().children('div.layui-colla-content').addClass('layui-show');
			} else {
				proptUrl = 'motgaInfo/getMotgaProptInfList.do?pageType='+ pageType;
			}
			...
			table.render({
				...
				cols : [[
					...
					{align:"center",field:"PLE_TYPE",title:"抵押物种类", templet:function(d){return renderColumnDict(d,'PLE_TYPE',staticDict.ccPleType)}},
					{align:"center",field:"MOTGA_PROPT_ID_TYPE",title:"抵押物识别号类型", templet:function(d){return renderColumnDict(d,'MOTGA_PROPT_ID_TYPE',staticDict.ccMotgaProptIdtype)}},
					...
					{align:"center",field:"PLE_CY",title:"币种", templet:function(d){return renderColumnDict(d,'PLE_CY',staticDict.ccy)}},
					{align:"center",field:"VAL_ORG_TYPE",title:"评估机构类型", templet:function(d){return renderColumnDict(d,'VAL_ORG_TYPE',staticDict.ccValOrgType)}},
					{align:"center",field:"VAL_DATE",title : "抵押物评估日期"},
					{align:"center",field:"PLEDGOR_TYPE",title:"抵押人身份类别", templet:function(d){return renderColumnDict(d,'PLEDGOR_TYPE',staticDict.infoIdType)}},
					{align:"center",field:"PLEDGOR_NAME",title:"抵押人名称"},
					{align:"center",field:"PLEOR_CERT_TYPE",title:"抵押人身份标识类型", templet:function(d){return renderColumnDictComsharHodIdType(d,'PLEOR_CERT_TYPE',staticDict.idType,staticDict.entCertMainType,'PLEDGOR_TYPE')}},
            ...
            table.on('tool(pawnDataTable)', function(obj){
                ...
                if (layEvent === 'edit'){ //编辑
                    ...
                    verify.dataValidation('pawn','MOTGA_PROPT_INF',data.EFD_MOTGA_PROPT_INF_SGMT_ID,'form',false,false); //校验错误信息回显
					modelIndex = layer.open({
                        type: 1,
                        title: "编辑",
                        shade: [0.3, '#000'],
					...
					layer.confirm('确认删除该数据信息?', {anim: 6}, function(index){
						$.ajax({ 
							url      : 'motgaInfo/delMotgaProptInf.do', 
							data     : {str : JSON.stringify(para)},
							type     : 'post',  
							cache    : false,  
							dataType : 'json',  
            ...
            form.on('submit(savePawnModelBtn)', function(data){
				$.ajax({
					type : "post",  //请求方式
					url  : "motgaInfo/saveMotgaProptInf.do",
					...
					success : function(result) {
						if(result.success == true){
                            ...
                            verify.dataValidation('pawnEditModel','MOTGA_PROPT_INF',$("#savePawnModelBtn").data("id"),'form',true,false);
                            verify.dataValidation('basic','MOTGA_CLTAL_BS_INF',motgaCltalInfSgmt.EFB_MOTGA_CLTAL_BS_INF_SGMT_ID,'form',true,false);
							verify.dataValidation('base','MOTGA_CLTAL_CTRCT_BS',dataObj.efaMotgaCltalCtrctBsId,'form',true,false);
						}else{
							layer.alert(result.msg);
			...
			var ctrctInfoUrl;
			if(pageType === 'edit'){
				ctrctInfoUrl = 'motgaInfo/getCltalInfList.do?efaMotgaCltalCtrctBsId='+ dataObj.efaMotgaCltalCtrctBsId 
						+ "&pageType=" + pageType;
				$("#qualityAddBtn").parent().children('div.layui-colla-content').addClass('layui-show');
			} else {
				ctrctInfoUrl = 'motgaInfo/getCltalInfList.do?pageType='+ pageType;
			}
			...
			table.render({
				...
				cols : [[
					...
					{align:"center",field:"IMP_TYPE",title:"质物种类", templet:function(d){return renderColumnDict(d,'IMP_TYPE',staticDict.ccImpType)}},
					{align:"center",field:"IMP_VAL",title:"质物价值"},
					{align:"center",field:"IMP_CY",title:"币种", templet:function(d){return renderColumnDict(d,'IMP_CY',staticDict.ccy)}},
					{align:"center",field:"IPPC",title:"出质人身份类别", templet:function(d){return renderColumnDict(d,'IPPC',staticDict.infoIdType)}},
					{align:"center",field:"PAWN_NAME",title:"出质人名称"},
					{align:"center",field:"PAWN_CERT_TYPE",title:"出质人身份标识类型", templet:function(d){return renderColumnDictComsharHodIdType(d,'PAWN_CERT_TYPE',staticDict.idType,staticDict.entCertMainType,'IPPC')}},
            ...
            table.on('tool(qualityDataTable)', function(obj){
                ...
                if (layEvent === 'edit'){ //编辑
                    ...
                    verify.dataValidation('quality','MOTGA_CLTAL_INF',data.EFE_CLTAL_INF_SGMT_ID,'form',false,false); //校验错误信息回显
					modelIndex = layer.open({
                        type: 1,
                        title: "编辑",
                        shade: [0.3, '#000'],
				...
				$.ajax({
					...
					success : function(result) {
						if(result.success == true){
                            ...
                            verify.dataValidation('qualityEditModel','MOTGA_CLTAL_INF',$("#saveQualityModelBtn").data("id"),'form',true,false);
                            verify.dataValidation('basic','MOTGA_CLTAL_BS_INF',motgaCltalInfSgmt.EFB_MOTGA_CLTAL_BS_INF_SGMT_ID,'form',true,false);
							verify.dataValidation('base','MOTGA_CLTAL_CTRCT_BS',dataObj.efaMotgaCltalCtrctBsId,'form',true,false);
						}else{
							layer.alert(result.msg);
