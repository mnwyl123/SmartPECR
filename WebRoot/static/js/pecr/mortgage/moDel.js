...
layui.use(['form', 'laydate','layer', 'element', 'table','laypage','laytpl'], function()
{
  ...
  require(['static/js/pecr/module/base','static/js/pecr/module/verify'],function(base,verify){
        ...
        table.render({
			...
			,url:'moDel/motgaContractInfoList.do' //url
			,cols: [[ 
				{type:'numbers'}	//列每行序号
				,{type:'checkbox'} //列复选框
        ...
        //日期输入框  按css将所有的date组件渲染
        		...
        		if(obj.event === 'del'){
        			layer.confirm('是否确定删除该条数据？', function(index){
        				$.ajax({
        					...
        					type : "POST",
        					//请求地址
        					url : "moDel/delMotgaContractInfo.do",
        					//数据，json字符串
        					data : data,
        					//请求成功
        			...
        			$("#infRecType").val(data.infRecType);
        			 $("#exampleSubmit").data("id",data.efzMocId);
        			form.val('exampleForm', data);
        			verify.dataValidation('dialog','MOC_ENT_DEL',data.efzMocId,'form',false,false); //数据校验结果回显
        			form.render('select');
        		}
        	...
        	table.on('toolbar(acc)', function (obj) {
        		switch (obj.event) {
        			case 'addBtn':
        				...
        				$("#infRecType").val('');
        				$(".errorFormBox").html("");
                		form.val('exampleForm', {'infRecType':'', 'lastUpdateTime':'', 'delRecCode':''});
                		$.ajax({
      						type : "POST",  //请求方式
      						url  : "generateUUID/id.do",
      						success : function(data) {
      							if(data.success == true){
      								$("#efzMocId").val(data.result);
      							}else{
        			...
        			case 'submitBtn':
        				...
        				for(var i = 0; i < data.length; i++){
        					str+=checkStatus.data[i].efzMocId+","
        				}
        	...
        	form.on('submit(exampleSubmit)', function(data){
        		...
        		$.ajax({
        			...
        			type : "POST",
        			//请求地址
        			url : "moDel/addOrUpdateUser.do?addOrUpdate="+addOrUpdate,
        			//数据，json字符串
       			     data : para,
        			//请求成功
        			success : function(result) {
        				if(result.success == true){
        					verify.dataValidation('dialog','MOC_ENT_DEL',$("#efzMocId").val(),'form',true,false,true); //数据校验
        					layer.msg('操作成功');
        					table.reload('acc', 
        							{method : 'post',
            						 where:{
