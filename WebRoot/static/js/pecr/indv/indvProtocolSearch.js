	...
	form.render();//每个页面都有
      ...
      //初始化列表
    	table.render({
    		...
    		url:'indvProtocol/getHistIndivProtocolLists.do',
    		where:{mdfcSgmtCode : $("#mdfcSgmtCode").val()},
        	cols: [[
    			{type:'numbers'},	//列每行序号
    			{type:'checkbox'},  //列复选框
    			{field: 'PCA_CTRCT_BS_SGMT_ID', title: '主键', hide: true},
    			 //{align : "center",field : "CONTRACT_CODE",title : "授信标识码"},
    			{align : "center",field : "INF_REC_TYPE",title : "信息记录类型", templet:function(d){return renderColumnDict(d,'INF_REC_TYPE',staticDict.infRecTypePOE)}},
                 ...
                 {align : "center",field : "RPT_DATE_CODE",title : "报告时点说明代码", templet:function(d){return renderColumnDict(d,'RPT_DATE_CODE',staticDict.perCreditRptDateCode)}},
    	...
    	table.on('tool(dataTable)', function(obj){
    		...
    		if (layEvent === 'checked'){
    			...
    			$.ajax({
					type : "POST",
					url : 'indvProtocol/selectIndvProtocolChangeEdit.do',
					data : {pcaCtrctBsSgmtId:data.PCA_CTRCT_BS_SGMT_ID,mdfcSgmtCode:data.MDFC_SGMT_CODE},
