      ...
      //日期输入框  按css将所有的date组件渲染
		...
		*/
        ...
        var initEnumerationValue = function () {
                var array = ['PER_GUAR_ACCT_TYPE','PER_GUAR_RPT_DATE_CODE','ID_TYPE','PER_GUAR_BUSI_LINES','CURRENCY','PER_GUAR_GUAR_MODE','PER_GUAR_OTH_REPY_GUAR_WAY','CERT_TYPE','PER_GUAR_ARLP_TYPE','PER_GUAR_ACCT_STATUS','FIVE_CATE','COMP_ADV_FLAG','PER_GUAR_UPD_INF_REC_TYPE'];
                var selectData = [];
        ...
        $.when(d0,d1).done(function (d0,d1) {
            ...
            form.on('submit(baseSaveBtn)', function(data){
            	$("#mdfcSgmtCode,#idType,#acctType,#orgCode,#isFilter").removeAttr("disabled");
            	if(updateFlag ==='N'){
                var $this = $(this);
                var para = {};
                     ...
                     para.orgName = $("#orgCode").next().find(".layui-this").val();
            ...
            if (dataObj && acctInfSgmt){
				...
				$("#basicSaveBtn").parent().show().children('div.layui-colla-content').addClass('layui-show');
				form.render('select');
...
//    				                verify.dataValidation('base','I_GUAR_ACCT_BS',dataObj.pdaGuarAcctBsSgmtId,'form',true,false); //数据校验
            ...
            $("#basicDelBtn").on("click",function () {
                  ...
                  layer.confirm('确认删除', {anim: 6}, function(index){
                      ...
                      para.pdbGuarAcctInfSgmtId = $this.data("id");
  					$.ajax({ 
  						url      : 'indvGuarAcctInfo/delacctInfSgmt.do', 
  						data     : {str : JSON.stringify(para)},
  						type     : 'post',  
  						cache    : false,  
  						dataType : 'json',  
