...
layui.use(['element', 'form','layer','laydate','table','laypage','laytpl'], function(){
	...
	require(['static/js/pecr/module/base'],function (base){
		...
		var acctBsSgmt, acctBsInfSgmt, acctCredSgmt, ocInfoSgmt, amblgInfoSgmt, acctDbtInfSgmt, specPrdSgmt;
		if(pageType === 'edit'){
			//基础段
	...
	form.on('select(arlpType)', function(data){
	 ...
	 $.when(d0,d1).done(function (d0,d1) {
            ...
            form.on('submit(stagingSaveBtn)', function(data){
				...
				//提交
				$.ajax({  
					url      : 'indvLoanAcctInfoSeo/saveSpecPrdSgmt.do', 
					data     : {str:JSON.stringify(para), pageType : $this.data("type")},
					type     : 'post',  
					cache    : false,  
					dataType : 'json',  
            ...
            $("#stagingDelBtn").on("click",function () {
                ...
                layer.confirm('确认删除', {anim: 6}, function(index){
                    ...
                    para.pbhSpecPrdSgmtId = $this.data("id");
					$.ajax({ 
						url      : 'indvLoanAcctInfoSeo/specPrdSgmtDel.do',
						data     : {str : JSON.stringify(para)},
						type     : 'post',  
						cache    : false,  
						dataType : 'json',  
