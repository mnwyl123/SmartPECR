	...
	form.on('select(arlpType)', function(data){
	 ...
	 $.when(d0,d1).done(function (d0,d1) {
			...
			if (dataObj && acctBsSgmt){
				...
				form.render('select');
//				verify.dataValidation('base','PBA_ACCT_BS_SGMT',dataObj.pbaAcctBsSgmtId,'form',true); //数据校验
			}else{
				$("#baseAddBtn").show();
			}
			//点击新增按钮
				...
				$.ajax({  
					...
					success:function(data){ 
						if (data.success == true){
							...
							baseExistFlag = 'Y';
							dataObj = para; //存储新数据
//							verify.dataValidation('base','PECR_INDV_ACCT_BS_SGMT',dataObj.pbaAcctBsSgmtId,'form',true); //数据校验
						} else {
							layer.alert(data.msg);
						}
					 },
				...
				$.ajax({  
					...
					success:function(data){ 
						if (data.success == true){
							...
							$this.data("type","edit");
							dataObj = para; //存储新数据
							//verify.dataValidation('base','PECR_INDV_ACCT_BS_INF_SGMT',dataObj.pbaAcctBsSgmtId,'form',true); //数据校验
						} else {
							layer.alert(data.msg);
						}
					 },
