		...
		*/
        ...
        var initEnumerationValue = function () {
			var array = ['ID_TYPE','PAA_RPT_DATE_CODE','PER_CUSTOMER_TYPE','SEX','W_C_A_R_N_CODES','EDU_LEVEL','ACA_DEGREE','EMP_STATUS'
				,'CPN_TYPE','A_DIVISION','OCCUPATION','TITLE','TECH_TITLE','RESI_STATUS','MARI_STATUS'];
			var selectData = [];
        ...
        $.when(d0,d1).done(function (d0,d1) {
            ...
            form.on('submit(baseSaveBtn)', function(data){
				...
				//提交
				$.ajax({  
					url      : 'indvBasicsInfoSeo/updatePcBasBsSgmt.do', 
            ...
            table.on('tool(responsibleDataTable)', function(obj){
                ...
                if (layEvent === 'edit'){ //编辑
                    ...
                    form.render('select');
                    $("#saveResponsibleModelBtn").data("id",data.pabIdSgmtId);
                    verify.dataValidation('saveResponsible','I_ID', $("#saveResponsibleModelBtn").data("id"),'form',true,false); //数据校验
					modelIndex = layer.open({
                        type: 1,
                        title: "编辑",
                        shade: [0.3, '#000'],
            ...
            form.on('submit(saveResponsibleModelBtn)', function(data){
				...
				$.ajax({
					...
					data : {rltData  :  JSON.stringify(data.field),
					        ...
					        pabIdSgmtId : $("#saveResponsibleModelBtn").data("id")
							},
					dataType : 'json',
            ...
            if (dataObj && basFcsInfoSgmt){
				...
				$.ajax({
					...
					success:function(data){
						...
						$("#busiDtilLines").val(basFcsInfoSgmt.HH_DIST);
						form.render('select');
					}
				});
