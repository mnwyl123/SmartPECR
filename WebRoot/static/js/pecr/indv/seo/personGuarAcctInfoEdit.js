		...
		*/
        ...
        $.when(d0,d1).done(function (d0,d1) {
            ...
            form.on('submit(baseSaveBtn)', function(data){
				...
				//提交
				$.ajax({  
					url      : 'indvGuarAcctInfoSeo/saveAcctBsSgmt.do', 
            ...
            $("#baseDelBtn").on("click",function () {
                ...
                layer.confirm('删除基础段相当于删除该页面所有段信息，是否确认删除', {anim: 6}, function(index){
					...
					//提交
					$.ajax({ 
						url      : 'indvGuarAcctInfoSeo/delAcctBsSgmt.do', 
            ...
            form.on('submit(basicSaveBtn)', function(data){
				...
				//提交
				$.ajax({  
					url      : 'indvGuarAcctInfoSeo/saveAcctInfoSgmt.do', 
					data     : {str:JSON.stringify(para), pageType : $this.data("type")},
					type     : 'post',  
					cache    : false,  
					dataType : 'json',  
            ...
            $("#basicDelBtn").on("click",function () {
                ...
                layer.confirm('确认删除', {anim: 6}, function(index){
                    ...
                    para.pdbGuarAcctInfSgmtId = $this.data("id");
					$.ajax({ 
						url      : 'indvGuarAcctInfoSeo/delAcctBsInfSgmt.do', 
						data     : {str : JSON.stringify(para)},
						type     : 'post',  
						cache    : false,  
						dataType : 'json',  
            ...
            var rltInfoUrl;
			if(pageType === 'edit'){
				rltInfoUrl = 'indvGuarAcctInfoSeo/getRltInfList.do?pdaGuarAcctBsSgmtId='+ dataObj.pdaGuarAcctBsSgmtId 
						+ "&pageType=" + pageType;
				$("#responsibleAddBtn").parent().children('div.layui-colla-content').addClass('layui-show');
			} else {
				rltInfoUrl = 'indvGuarAcctInfoSeo/getRltInfList.do?pageType='+ pageType;
			}
            ...
            table.on('tool(responsibleDataTable)', function(obj){
                ...
                if (layEvent === 'edit'){ //编辑
					...
					layer.confirm('确认删除该数据信息?', {anim: 6}, function(index){
						$.ajax({ 
							url      : 'indvGuarAcctInfoSeo/rltRepymtInfDel.do', 
							data     : {str : JSON.stringify(para)},
							type     : 'post',  
							cache    : false,  
							dataType : 'json',  
            ...
            form.on('submit(saveResponsibleModelBtn)', function(data){
				$.ajax({
					type : "POST",  //请求方式
					url  : "indvGuarAcctInfoSeo/saveRltRepymtInf.do",
            ...
            var mccInfoUrl;
			if(pageType === 'edit'){
				mccInfoUrl = 'indvGuarAcctInfoSeo/getMccInfList.do?pdaGuarAcctBsSgmtId='+ dataObj.pdaGuarAcctBsSgmtId 
						+ "&pageType=" + pageType;
				$("#mortgageAddBtn").parent().children('div.layui-colla-content').addClass('layui-show');
			} else {
				mccInfoUrl = 'indvGuarAcctInfoSeo/getMccInfList.do?pageType='+ pageType;
			}
            ...
            table.on('tool(mortgageDataTable)', function(obj){
                ...
                if (layEvent === 'edit'){ //编辑
					...
					layer.confirm('确认删除该数据信息?', {anim: 6}, function(index){
						$.ajax({ 
							url      : 'indvGuarAcctInfoSeo/mccInfDel.do', 
							data     : {str : JSON.stringify(para)},
							type     : 'post',  
							cache    : false,  
							dataType : 'json',  
            ...
            form.on('submit(saveMortgageModelBtn)', function(data){
				$.ajax({
					type : "post",  //请求方式
					url  : "indvGuarAcctInfoSeo/saveMccInf.do",
            ...
            form.on('submit(insuranceSaveBtn)', function(data){
				...
				//提交
				$.ajax({  
					url      : 'indvGuarAcctInfoSeo/saveRepymtInfSgmt.do', 
					data     : {str:JSON.stringify(para), pageType : $this.data("type")},
					type     : 'post',  
					cache    : false,  
					dataType : 'json',  
            ...
            $("#insuranceDelBtn").on("click",function () {
                ...
                layer.confirm('确认删除', {anim: 6}, function(index){
                    ...
                    para.pdeGrRepymtInfSgmtId = $this.data("id");
                    $.ajax({ 
						url      : 'indvGuarAcctInfoSeo/delRepymtInfSgmt.do',
						data     : {str : JSON.stringify(para)},
						type     : 'post',  
						cache    : false,  
						dataType : 'json',  
