		...
		*/
        ...
        var initEnumerationValue = function () {
			var array = ['PER_LOAN_RPT_DATE_CODE','ID_TYPE', 'PER_LOAN_BUSI_LINES', 'CURRENCY', 'A_DIVISION', 
						  ...
						  'RPY_STATUS_R3', 'PER_LOAN_CHAN_TRAN_TYPE','PER_LOAN_UPD_INF_REC_TYPE'];
			var selectData = [];
	 ...
	 $.when(d0,d1).done(function (d0,d1) {
		 $("#feedBackError").val(feedBackError);
			...
			form.on('submit(dcAcctBsInfSgmtSub)', function(data){
				...
				para.openDate = new Date($("#openDate").val()).getTime();
				para.cy = $("#cy").val();
				para.acctCredLine = $("#acctCredLine").val();
				para.loanAmt = '';
				para.flag = '';
				para.dueDate = new Date($("#dueDate").val()).getTime();
				para.repayMode = '';
				para.repayFreqcy = '';
				para.repayPrd = '';
...
//            				verify.dataValidation('base','I_ACCT_BS', dataObj.pbaAcctBsSgmtId,'form',true,false); //数据校验
            ...
            form.on('submit(behaviorSaveBtn)', function(data){
				...
				para.settDate = new Date($("#settDate").val()).getTime();
				...
				para.fiveCateAdjDate = new Date($("#fiveCateAdjDate").val()).getTime();
				...
				para.latRpyDate = new Date($("#latRpyDate").val()).getTime();
				para.closeDate = new Date($("#closeDate").val()).getTime();
				//数据校验缺失，后期添加
				//提交
				$.ajax({  
            ...
            form.on('submit(noBehaviorSaveBtn)', function(data){
				...
				para.nomonthFiveCateAdjDate = new Date($("#nomonthFiveCateAdjDate").val()).getTime();
				...
				para.nomonthLatRpyDate = new Date($("#nomonthLatRpyDate").val()).getTime();
				para.nomonthCloseDate = new Date($("#nomonthCloseDate").val()).getTime();
				//数据校验缺失，后期添加
				//提交
				$.ajax({  
