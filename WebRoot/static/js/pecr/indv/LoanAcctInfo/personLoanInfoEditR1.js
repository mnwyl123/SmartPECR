		...
		*/
        ...
        var initEnumerationValue = function () {
			var array = ['PER_LOAN_RPT_DATE_CODE','ID_TYPE', 'PER_LOAN_BUSI_LINES', 'CURRENCY',
						'PER_LOAN_REPAY_MODE_R1','PER_LOAN_REPAY_FREQCY', 'A_DIVISION', 'PER_LOAN_GUAR_MODE',
						'PER_LOAN_OTH_REPY_GUAR_WAY', 'ASSET_TRAND_FLAG', 'PER_LOAN_FUND_SOU', 'PER_LOAN_FORM',
						'CERT_TYPE', 'PER_LOAN_ARLP_TYPE', 'PER_LOAN_ACCT_STATUS_R1', 'FIVE_CATE',
						'RPY_STATUS_R1', 'PER_LOAN_CHAN_TRAN_TYPE'];
			var selectData = [];
				...
				success:function(data){
					for(var i=0;i<data.result.length;i++){
						if (data.result[i].PER_LOAN_RPT_DATE_CODE){
							...
							continue;
						} else if  (data.result[i].PER_LOAN_REPAY_MODE_R1){
							base.renderTpl({
								tplId : "enumerationValueTpl",
								arr : {"list" : data.result[i].PER_LOAN_REPAY_MODE_R1},
							...
							continue;
						} else if (data.result[i].PER_LOAN_ACCT_STATUS_R1){
							base.renderTpl({
								tplId : "enumerationValueTpl",
								arr : {"list" : data.result[i].PER_LOAN_ACCT_STATUS_R1},
							...
							continue;
						} else if  (data.result[i].RPY_STATUS_R1){
							base.renderTpl({
								tplId : "enumerationValueTpl",
								arr : {"list" : data.result[i].RPY_STATUS_R1},
	 ...
	 $.when(d0,d1).done(function (d0,d1) {
		 $("#feedBackError").val(feedBackError);
			...
			form.on('submit(dcAcctBsInfSgmtSub)', function(data){
				...
				para.cy = $("#cy").val();
				para.acctCredLine = $("#acctCredLine").val(); //与D1有差异，D1不包含该要素
				para.loanAmt = '';
                para.flag = '';
            ...
            form.on('submit(amountSaveBtn)', function(data){
				...
				$.ajax({  
					...
					success:function(data){ 
						if (data.success == true){
							//对于R1/R4账户，授信额度信息段在账户开立和月结日首次上报存量账户时属于必报，与基本信息段没有关系。
