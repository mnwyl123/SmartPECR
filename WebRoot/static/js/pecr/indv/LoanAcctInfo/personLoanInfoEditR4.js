		...
		*/
        ...
        var initEnumerationValue = function () {
			var array = ['PER_LOAN_RPT_DATE_CODE','ID_TYPE', 'PER_LOAN_BUSI_LINES', 'CURRENCY', 
						'PER_LOAN_REPAY_MODE_R4','PER_LOAN_REPAY_FREQCY', 'A_DIVISION', 'PER_LOAN_GUAR_MODE',
						...
						'RPY_STATUS_D1_R4', 'PER_LOAN_CHAN_TRAN_TYPE'];
			var selectData = [];
				...
				success:function(data){
					for(var i=0;i<data.result.length;i++){
						if (data.result[i].PER_LOAN_RPT_DATE_CODE){
							...
							continue;
						} else if  (data.result[i].PER_LOAN_REPAY_MODE_R4){
							base.renderTpl({
								tplId : "enumerationValueTpl",
								arr : {"list" : data.result[i].PER_LOAN_REPAY_MODE_R4},
				...
				$.ajax({
					...
					success : function(result) {
						if(result.success == true){
							...
							//只有当基本信息中担保方式为3-保证和5-组合（含保证）时才上报相关还款责任人
