...
layui.use(['element', 'form','layer','laydate','table','laypage','laytpl'], function(){
	...
	require(['static/js/pecr/module/base','static/js/pecr/module/verify'],function(base,verify){
		...
		var acctBsSgmt, acctBsInfSgmt, acctCredSgmt, ocInfoSgmt, amblgInfoSgmt, acctDbtInfSgmt, specPrdSgmt,feedBackError;
		if(pageType === 'edit'){
			//基础段
			...
			//大额专项分期信息段
			specPrdSgmt = JSON.parse(parent.document.getElementById("specPrdSgmt").value);
			//反馈错误信息
		...
		*/
        ...
        var initEnumerationValue = function () {
			var array = ['PER_LOAN_RPT_DATE_CODE','ID_TYPE', 'PER_LOAN_BUSI_LINES', 'CURRENCY', 'A_DIVISION', 
						 ...
						 'RPY_STATUS_R2', 'PER_LOAN_CHAN_TRAN_TYPE'];
			var selectData = [];
				...
				success:function(data){
					for(var i=0;i<data.result.length;i++){
						if (data.result[i].PER_LOAN_RPT_DATE_CODE){
							...
							continue;
						} else if  (data.result[i].RPY_STATUS_R2){
							base.renderTpl({
								tplId : "enumerationValueTpl",
								arr : {"list" : data.result[i].RPY_STATUS_R2},
	...
	form.on('select(orgCode)', function(data){
	 ...
	 $.when(d0,d1).done(function (d0,d1) {
		 $("#feedBackError").val(feedBackError);
					...
					$.ajax({
						...
						success : function(data) {
							if(data.success == true){
								...
								//为了联合相关还款责任人段、抵质押物信息段和授信额度信息段的js校验
								var PBB_ACCT_BS_INF_SGMT_ID = '{"PBB_ACCT_BS_INF_SGMT_ID":"'+data.result+'"}'
								acctBsInfSgmt = JSON.parse(PBB_ACCT_BS_INF_SGMT_ID);
							}else{
								layer.alert(data.msg);
							}
						},
            ...
            table.on('tool(dcRltRepymtInfSgmtTable)', function(obj){
                ...
                if (layEvent === 'edit'){ //编辑
	                ...
	                verify.dataValidation('dcRltRepymtInfSgmt','I_LOAN_RLT_REPYMT_INF',  $("#saveResponsibleModelBtn").data("id"),'form',false,false); //数据校验
				...
				$.ajax({
					...
					success : function(result) {
						if(result.success == true){
							var newrltInfoUrl = 'indvLoanAcctInfo/getRltRepymtInfList.do?pbaAcctBsSgmtId='+ dataObj.pbaAcctBsSgmtId + '&pageType=edit';
							table.reload('dcRltRepymtInfSgmtTable', {url:newrltInfoUrl});
							//只有当基本信息中的担保方式为3保证和5组合时才上报相关还款责任段
        	                verify.dataValidation('responsibleEditModel','I_LOAN_RLT_REPYMT_INF',pbcRltRepymtInfSgmtId,'form',true,false); //数据校验
        	                verify.dataValidation('basic','I_ACCT_BS_INF', acctBsInfSgmt.PBB_ACCT_BS_INF_SGMT_ID,'form',true,false); //数据校验
            				verify.dataValidation('base','I_ACCT_BS', dataObj.pbaAcctBsSgmtId,'form',true,false);
			...
			var mccInfoUrl;
			if(pageType === 'edit'){
				mccInfoUrl = 'indvLoanAcctInfo/getMccInfList.do?pbaAcctBsSgmtId='+ dataObj.pbaAcctBsSgmtId 
						+ "&pageType=" + pageType;
				$("#mccAddBtn").parent().children('div.layui-colla-content').addClass('layui-show');
			} else {
				mccInfoUrl = 'indvLoanAcctInfo/getMccInfList.do?pageType='+ pageType;
			}
            ...
            table.on('tool(mccInfoTable)', function(obj){
                ...
                if (layEvent === 'edit'){ //编辑
	                ...
	                verify.dataValidation('mcc','I_MCC_INF',$("#saveMortgageModelBtn").data("id"),'form',false,false); //数据校验
					...
					layer.confirm('确认删除该数据信息?', {anim: 6}, function(index){
						$.ajax({ 
							url      : 'indvLoanAcctInfo/mccInfDel.do', 
							data     : {str : JSON.stringify(para)},
							type     : 'post',  
							cache    : false,  
							dataType : 'json',  
            ...
            form.on('submit(saveMortgageModelBtn)', function(data){
				...
				$.ajax({
					type : "post",  //请求方式
					url  : "indvLoanAcctInfo/saveMccInf.do",
					...
					success : function(result) {
						if(result.success == true){
							var newmccInfoUrl = 'indvLoanAcctInfo/getMccInfList.do?pbaAcctBsSgmtId='+ dataObj.pbaAcctBsSgmtId + '&pageType=edit';
							table.reload('mccInfoTable', {url:newmccInfoUrl});
							//当基本信息段中担保方式为抵押和质押时上报抵质押物信息段
        	                verify.dataValidation('mortgageEditModel','I_MCC_INF',pbdMccInfSgmtId,'form',true,false); //数据校验
        	                verify.dataValidation('basic','I_ACCT_BS_INF', acctBsInfSgmt.PBB_ACCT_BS_INF_SGMT_ID,'form',true,false); //数据校验
            				verify.dataValidation('base','I_ACCT_BS', dataObj.pbaAcctBsSgmtId,'form',true,false);
							layer.msg('操作成功', {time:3000});
                            layer.close(modelIndex);
			...
			if (dataObj && acctCredSgmt){
                ...
                verify.dataValidation('amount','I_ACCT_CRED',acctCredSgmt.PBE_ACCT_CRED_SGMT_ID,'form',false,false); //数据校验
            ...
            form.on('submit(amountSaveBtn)', function(data){
				...
				//提交
				$.ajax({  
					url      : 'indvLoanAcctInfo/saveAcctCredSgmt.do', 
					data     : {str:JSON.stringify(para), pageType : $this.data("type")},
					type     : 'post',  
					cache    : false,  
					dataType : 'json',  
					success:function(data){ 
						if (data.success == true){
			                verify.dataValidation('amount','I_ACCT_CRED',$this.data("id"),'form',true,false); //数据校验
            				verify.dataValidation('base','I_ACCT_BS', dataObj.pbaAcctBsSgmtId,'form',true,false);
							layer.msg("操作成功");
							if($this.data("type") === "add"){
								$("#amountDelBtn").show();
            ...
            $("#amountDelBtn").on("click",function () {
				...
				layer.confirm('确认删除', {anim: 6}, function(index){
                    ...
                    para.pbeAcctCredSgmtId = $this.data("id");
					$.ajax({ 
						url      : 'indvLoanAcctInfo/acctCredSgmtDel.do', 
						data     : {str : JSON.stringify(para)},
						type     : 'post',  
						cache    : false,  
						dataType : 'json',  
            ...
            if (dataObj && amblgInfoSgmt){
				...
				$("#pridAcctBal").val(amblgInfoSgmt.PRID_ACCT_BAL);
				$("#usedAmt").val(amblgInfoSgmt.USED_AMT);
				$("#notIsuBal").val(amblgInfoSgmt.NOT_ISU_BAL);
				$("#remRepPrd").val(amblgInfoSgmt.REM_REP_PRD);
				...
				$("#rpyPrct").val(amblgInfoSgmt.RPY_PRCT);
				$("#overdPrd").val(amblgInfoSgmt.OVERD_PRD);
				$("#totOverd").val(amblgInfoSgmt.TOT_OVERD);
				$("#curRpyAmt").val(amblgInfoSgmt.CUR_RPY_AMT);
				...
				form.render('select');
                verify.dataValidation('behavior','I_AMBLG_INF',amblgInfoSgmt.PBG_AMBLG_INF_SGMT_ID,'form',false,false); //数据校验
            ...
            form.on('submit(behaviorSaveBtn)', function(data){
				...
				para.pridAcctBal = $("#pridAcctBal").val();
				para.usedAmt = $("#usedAmt").val();
				para.notIsuBal = $("#notIsuBal").val();
				para.remRepPrd = $("#remRepPrd").val();
				para.fiveCate = $("#fiveCate").val();
				para.fiveCateAdjDate =$("#fiveCateAdjDate").val();
				para.rpyStatus = $("#rpyStatus").val();
				para.rpyPrct = $("#rpyPrct").val();
				para.overdPrd = $("#overdPrd").val();
				...
				para.oved91180Princ = '';
				para.ovedPrinc180 = '';
				para.ovedrawBaOve180 = '';
				para.curRpyAmt = $("#curRpyAmt").val();
				...
				//提交
				$.ajax({  
					url      : 'indvLoanAcctInfo/saveAMBlgInfSgmt.do', 
					data     : {str:JSON.stringify(para), pageType : $this.data("type")},
					type     : 'post',  
					cache    : false,  
					dataType : 'json',  
					success:function(data){ 
						if (data.success == true){
			                verify.dataValidation('behavior','I_AMBLG_INF',$this.data("id"),'form',true,false); //数据校验
			                verify.dataValidation('base','I_ACCT_BS', dataObj.pbaAcctBsSgmtId,'form',true,false);
							layer.msg("操作成功");
							if($this.data("type") === "add"){
								$("#behaviorDelBtn").show();
            ...
            $("#behaviorDelBtn").on("click",function () {
                ...
                layer.confirm('确认删除该数据信息?', {anim: 6}, function(index){
                    ...
                    para.pbgAmblgInfSgmtId = $this.data("id");
					$.ajax({ 
						url      : 'indvLoanAcctInfo/amblgInfSgmtDel.do', 
						data     : {str : JSON.stringify(para)},
						type     : 'post',  
						cache    : false,  
						dataType : 'json',  
            ...
            if (dataObj && acctDbtInfSgmt){
				...
				$("#nomonthRemRepPrd").val(acctDbtInfSgmt.NOMONTH_REM_REP_PRD);
				$("#nomonthRpyStatus").val(acctDbtInfSgmt.NOMONTH_RPY_STATUS);
				$("#nomonthOverdPrd").val(acctDbtInfSgmt.NOMONTH_OVERD_PRD);
				$("#nomonthTotOverd").val(acctDbtInfSgmt.NOMONTH_TOT_OVERD);
            ...
            form.on('submit(noBehaviorSaveBtn)', function(data){
				...
				para.nomonthRemRepPrd = $("#nomonthRemRepPrd").val();
				para.nomonthRpyStatus = $("#nomonthRpyStatus").val();
				para.nomonthOverdPrd = $("#nomonthOverdPrd").val();
				para.nomonthTotOverd = $("#nomonthTotOverd").val();
			...
			/****************************************大额专项分期信息段开始****************************************/
            //判断是新增还是编辑
            if (dataObj && specPrdSgmt){
				$("#specLine").val(specPrdSgmt.SPEC_LINE);
				$("#specEfctDate").val(specPrdSgmt.SPEC_EFCT_DATE);
				$("#specEndDate").val(specPrdSgmt.SPEC_END_DATE);
				$("#usedInstAmt").val(specPrdSgmt.USED_INST_AMT);
				$("#stagingSaveBtn").show().data("id",specPrdSgmt.PBH_SPEC_PRD_SGMT_ID).data("type","edit");
                $("#stagingDelBtn").show().data("id",specPrdSgmt.PBH_SPEC_PRD_SGMT_ID);
				$("#stagingSaveBtn").parent().children('div.layui-colla-content').addClass('layui-show');
				form.render('select');
                verify.dataValidation('staging','I_SPEC_PRD', specPrdSgmt.PBH_SPEC_PRD_SGMT_ID,'form',false,false); //数据校验

            }else{
                $("#stagingAddBtn").show();
            }
            //点击新增按钮
            $("#stagingAddBtn").on("click",function () {
                if (dataObj || baseExistFlag === 'Y'){
                    var $this = $(this);
					$.ajax({
						type : "POST",  //请求方式
						...
						success : function(data) {
							if(data.success == true){
								$this.hide();
								$("#stagingSaveBtn").show().data("id",data.result).data("type","add");
							    $("#stagingDelBtn").data("id",data.result);
								$this.parent().children('div.layui-colla-content').addClass('layui-show');
							}else{
								layer.alert(data.msg);
							}
            ...
            //点击保存按钮
            form.on('submit(stagingSaveBtn)', function(data){
                var $this = $(this);
				var para = {};
				para.pbaAcctBsSgmtId = dataObj.pbaAcctBsSgmtId;
				para.pbhSpecPrdSgmtId = $this.data("id");
				para.specLine = $("#specLine").val();
				para.specEfctDate =$("#specEfctDate").val();
				para.specEndDate = $("#specEndDate").val();
				para.usedInstAmt = $("#usedInstAmt").val();
				
				//数据校验缺失，后期添加
				//提交
				$.ajax({  
					url      : 'indvLoanAcctInfo/saveSpecPrdSgmt.do', 
					data     : {str:JSON.stringify(para), pageType : $this.data("type")},
					type     : 'post',  
					cache    : false,  
					dataType : 'json',  
					success:function(data){ 
						if (data.success == true){
			                verify.dataValidation('staging','I_SPEC_PRD',$this.data("id"),'form',true,false); //数据校验
			                verify.dataValidation('base','I_ACCT_BS', dataObj.pbaAcctBsSgmtId,'form',true,false);
							layer.msg("操作成功");
							if($this.data("type") === "add"){
								$("#stagingDelBtn").show();
            ...
            //点击删除按钮
            $("#stagingDelBtn").on("click",function () {
                var $this = $(this);
                layer.confirm('确认删除', {anim: 6}, function(index){
                    var para = {};
                    para.pbaAcctBsSgmtId = dataObj.pbaAcctBsSgmtId;
                    para.pbhSpecPrdSgmtId = $this.data("id");
					$.ajax({ 
						url      : 'indvLoanAcctInfo/specPrdSgmtDel.do',
						data     : {str : JSON.stringify(para)},
						type     : 'post',  
						cache    : false,  
						dataType : 'json',  
            ...
            table.on('tool(businessDataTable)', function(obj){
                ...
                if (layEvent === 'edit'){ //编辑
	                ...
	                verify.dataValidation('business','I_AS_TRST_DSPN', $("#saveBusinessModelBtn").data("id"),'form',false,false); //数据校验
            ...
            form.on('submit(saveBusinessModelBtn)', function(data){
				...
				$.ajax({
					...
					success : function(result) {
						if(result.success == true){
							var newbusInfoUrl = 'indvLoanAcctInfo/getDspnSgmtList.do?pbaAcctBsSgmtId='+ dataObj.pbaAcctBsSgmtId + '&pageType=edit';
							table.reload('businessDataTable', {url:newbusInfoUrl});
			                verify.dataValidation('businessEditModel','I_AS_TRST_DSPN', $("#saveBusinessModelBtn").data("id"),'form',true,false); //数据校验
			                verify.dataValidation('base','I_ACCT_BS', dataObj.pbaAcctBsSgmtId,'form',true,false);
							layer.msg('操作成功', {time:3000});
                            layer.close(modelIndex);
						}else{
            ...
            if (base.getrequest('addOrUpdate',true) === 'detail'){
                ...
                if (dataObj.businessStates === 'RKCG'){ //入库成功状态
                    ...
                    $("#basicSegmentDelBtn,#behaviorSegmentDelBtn,#noBehaviorSegmentDelBtn,#stagingSegmentDelBtn,#businessSegmentDelBtn").show().css("visibility","visible").removeClass("hide");
                    $("#baseSegmentChangeBtn,#basicSegmentChangeBtn,#responsibleSegmentChangeBtn,#mortgageSegmentChangeBtn, #amountSegmentChangeBtn,#behaviorSegmentChangeBtn,#noBehaviorSegmentChangeBtn,#stagingSegmentChangeBtn,#businessSegmentChangeBtn").show().css("visibility","visible").removeClass("hide");
                }
         ...
         * 点击按段删除按钮
         */
        $("#basicSegmentDelBtn,#behaviorSegmentDelBtn,#noBehaviorSegmentDelBtn,#stagingSegmentDelBtn,#businessSegmentDelBtn").on("click",function () {
         ...
         * 点击按段更正按钮
         */
        $("#baseSegmentChangeBtn,#basicSegmentChangeBtn,#responsibleSegmentChangeBtn,#mortgageSegmentChangeBtn,#amountSegmentChangeBtn,#behaviorSegmentChangeBtn,#noBehaviorSegmentChangeBtn,#stagingSegmentChangeBtn,#businessSegmentChangeBtn").on("click",function () {
