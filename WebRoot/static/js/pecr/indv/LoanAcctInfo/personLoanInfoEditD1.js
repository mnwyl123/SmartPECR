...
layui.use(['element', 'form','layer','laydate','table','laytpl'], function(){
	...
	require(['static/js/pecr/module/base','static/js/pecr/module/verify'],function(base,verify){
		...
		var acctBsSgmt, acctBsInfSgmt, acctCredSgmt, ocInfoSgmt, amblgInfoSgmt, acctDbtInfSgmt,feedBackError;
		if(pageType === 'edit'){
			//基础段
		...
		*/
        ...
        var initEnumerationValue = function () {
			var array = ['PER_LOAN_RPT_DATE_CODE','ID_TYPE', 'PER_LOAN_BUSI_LINES', 'CURRENCY', 'SUB_LOAN_FLAG',
						'PER_LOAN_REPAY_MODE_D1','PER_LOAN_REPAY_FREQCY', 'A_DIVISION', 'PER_LOAN_GUAR_MODE',
						'PER_LOAN_OTH_REPY_GUAR_WAY', 'ASSET_TRAND_FLAG', 'PER_LOAN_FUND_SOU', 'PER_LOAN_FORM',
						'CERT_TYPE', 'PER_LOAN_ARLP_TYPE', 'PER_LOAN_ACCT_STATUS_D1_R4', 'FIVE_CATE',
						'RPY_STATUS_D1_R4', 'PER_LOAN_CHAN_TRAN_TYPE','FIRST_HOU_LOAN_FLAG'];
			var selectData = [];
				...
				success:function(data){
					for(var i=0;i<data.result.length;i++){
						if (data.result[i].PER_LOAN_RPT_DATE_CODE){
							...
							continue;
						} else if (data.result[i].SUB_LOAN_FLAG){
							base.renderTpl({
								tplId : "enumerationValueTpl",
								arr : {"list" : data.result[i].SUB_LOAN_FLAG},
								containerId : "flag"
							});
							continue;
						} else if  (data.result[i].PER_LOAN_REPAY_MODE_D1){
							base.renderTpl({
								tplId : "enumerationValueTpl",
								arr : {"list" : data.result[i].PER_LOAN_REPAY_MODE_D1},
								containerId : "repayMode"
							});
							continue;
						} else if (data.result[i].PER_LOAN_REPAY_FREQCY){
							base.renderTpl({
								tplId : "enumerationValueTpl",
								arr : {"list" : data.result[i].PER_LOAN_REPAY_FREQCY},
							...
							continue;
						} else if (data.result[i].PER_LOAN_ACCT_STATUS_D1_R4){
							base.renderTpl({
								tplId : "enumerationValueTpl",
								arr : {"list" : data.result[i].PER_LOAN_ACCT_STATUS_D1_R4},
							...
							continue;
						} else if  (data.result[i].RPY_STATUS_D1_R4){
							base.renderTpl({
								tplId : "enumerationValueTpl",
								arr : {"list" : data.result[i].RPY_STATUS_D1_R4},
							...
							continue;
						} else if (data.result[i].FIRST_HOU_LOAN_FLAG){
							base.renderTpl({
								tplId : "enumerationValueTpl",
								arr : {"list" : data.result[i].FIRST_HOU_LOAN_FLAG},
								containerId : "firstHouLoanFlag"
							});
							continue;
						}
					}
	...
	form.on('select(orgCode)', function(data){
	 ...
	 $.when(d0,d1).done(function (d0,d1) {
		 //判断反馈信息是否为空，不为空时展示错误反馈信息
			 $("#feedBackError").parent().parent().children('div.layui-colla-content').addClass('layui-show');
			...
			if (dataObj && acctBsInfSgmt){
				...
				//20210105,add
				$("#loanConCode").val(acctBsInfSgmt.LOAN_CON_CODE);
				$("#firstHouLoanFlag").val(acctBsInfSgmt.FIRST_HOU_LOAN_FLAG);
			...
			form.on('submit(basicSaveBtn)', function(data){
				...
				//20210105,新增
				para.loanConCode = $("#loanConCode").val();
				para.firstHouLoanFlag = $("#firstHouLoanFlag").val();
				//数据校验缺失，后期添加
            ...
            form.on('submit(saveMortgageModelBtn)', function(data){
				...
				$.ajax({
					...
					success : function(result) {
						if(result.success == true){
							newmccInfoUrl = 'indvLoanAcctInfo/getMccInfList.do?pbaAcctBsSgmtId='+ dataObj.pbaAcctBsSgmtId + '&pageType=edit';
			...
			if (dataObj && acctCredSgmt){
				...
				$("#amountSaveBtn").parent().children('div.layui-colla-content').addClass('layui-show');//同级下的div 添加css
				 form.render('select');
                verify.dataValidation('amount','I_ACCT_CRED',acctBsInfSgmt.PBE_ACCT_CRED_SGMT_ID,'form',false,false); //数据校验
            ...
            if (dataObj && amblgInfoSgmt){
				...
				$("#overdPrinc").val(amblgInfoSgmt.OVERD_PRINC);
				$("#oved3160Princ").val(amblgInfoSgmt.OVED31_60_PRINC);
				$("#oved6190Princ").val(amblgInfoSgmt.OVED61_90_PRINC);
				$("#oved91180Princ").val(amblgInfoSgmt.OVED91_180_PRINC);
				$("#ovedPrinc180").val(amblgInfoSgmt.oved_princ180);
            ...
            form.on('submit(behaviorSaveBtn)', function(data){
				...
				para.oved3160Princ = $("#oved3160Princ").val();
				para.oved6190Princ = $("#oved6190Princ").val();
				para.oved91180Princ = $("#oved91180Princ").val();
				para.ovedPrinc180 = $("#ovedPrinc180").val();
