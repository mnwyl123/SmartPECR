	...
	require(['static/js/pecr/module/base','static/js/pecr/module/verify'],function(base,verify){
		...
		*/
        ...
        var initEnumerationValue = function () {
			var array = ['PER_LOAN_RPT_DATE_CODE_C1','ID_TYPE', 'PER_LOAN_BUSI_LINES', 'CURRENCY',
						'CERT_TYPE', 'PER_LOAN_ARLP_TYPE', 'PER_LOAN_ACCT_STATUS_C1', 'PER_LOAN_CHAN_TRAN_TYPE',
						'PER_LOAN_ORIG_DBT_CATE', 'INIT_RPY_STS'];
			var selectData = [];
			...
			$.ajax({  
				...
				success:function(data){
					for(var i=0;i<data.result.length;i++){
						if (data.result[i].PER_LOAN_RPT_DATE_CODE_C1){
							base.renderTpl({
								tplId : "enumerationValueTpl",
								arr : {"list" : data.result[i].PER_LOAN_RPT_DATE_CODE_C1},
				...
				$.ajax({
					...
					success : function(result) {
						if(result.success == true){
        	                ...
        	                verify.dataValidation('businessEditModel','I_AS_TRST_DSPN',pbjAsTrstDspnSgmtId,'form',true,false); //数据校验
            				verify.dataValidation('base','I_ACCT_BS', dataObj.pbaAcctBsSgmtId,'form',true,false);
							layer.msg('操作成功', {time:3000});
                            layer.close(modelIndex);
						}else{
