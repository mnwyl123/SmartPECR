		...
		*/
        ...
        var initEnumerationValue = function () {
			var array = ['PER_LOAN_RPT_DATE_CODE','ID_TYPE', 'PER_LOAN_BUSI_LINES', 'CURRENCY', 'A_DIVISION', 
						 ...
						 'RPY_STATUS_R2', 'PER_LOAN_CHAN_TRAN_TYPE','PER_LOAN_UPD_INF_REC_TYPE'];
			var selectData = [];
...
//            				verify.dataValidation('base','I_ACCT_BS', dataObj.pbaAcctBsSgmtId,'form',true,false); //数据校验
				...
				$.ajax({
					...
					success : function(result) {
						if(result.success == true){
        	                ...
        	                verify.dataValidation('mcc','I_MCC_INF',pbdMccInfSgmtId,'form',true,false); //数据校验
            ...
            form.on('submit(saveBusinessModelBtn)', function(data){
				...
				$.ajax({
					...
					success : function(result) {
						if(result.success == true){
			                verify.dataValidation('business','I_AS_TRST_DSPN', $("#saveBusinessModelBtn").data("id"),'form',true,false); //数据校验
