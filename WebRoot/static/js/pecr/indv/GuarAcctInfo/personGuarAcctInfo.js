...
layui.use(['form', 'laydate', 'element', 'layer', 'table', 'laypage','laytpl'], function(){
    ...
    var initEnumerationValue = function () {
		var array = ['DATA_STATUS_BLXS','PER_GUAR_ACCT_TYPE'];
		var selectData = [];
	...
	table.render({
		...
		url:'indvGuarAcctInfo/acctInfoList.do',
    	cols: [[
			{type:'numbers'},	//列每行序号
			{type:'checkbox'},  //列复选框
...
//			{field: 'mngmtOrgCode', align:'center', title: '所属机构'},
	...
	$("#queryBtn").on('click', function(){
		table.reload('indvGuarAcctTab', {
			...
			where: {
			  ...
			  dataStatusBlxs : $('#dataStatusBlxs').val(),
	...
	table.on('toolbar(indvGuarAcctTab)', function(obj){
		if(obj.event === 'addBtn') {
			...
			//获取选中的行信息
			 var checkStatus = table.checkStatus('indvGuarAcctTab');
			 //获取选中行的数据
			...
			for(var i = 0; i < data.length; i++){
				str+=checkStatus.data[i].pdaGuarAcctBsSgmtId+","
			}
	...
	table.on('tool(indvGuarAcctTab)', function(obj){
		...
		if (layEvent === 'editInfo'){
			...
			layer.confirm('确认要删除该条信息？', function(index){
				$.ajax({
					type : "POST",
					url : "indvGuarAcctInfo/delBasicsInfo.do",
					data : data,
