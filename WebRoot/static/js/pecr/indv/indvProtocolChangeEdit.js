		...
		*/
        ...
        var initEnumerationValue = function () {
                var array = ['PER_CREDIT_RPT_DATE_CODE','ID_TYPE','CERT_TYPE','PER_CREDIT_LIM_TYPE',
                	'CREDIT_LIM_TYPE','CREDIT_CON_STATUS','CURRENCY','PER_CREDIT_UPD_INF_REC_TYPE'];
                var selectData = [];
...
//								$this.hide();
            ...
            form.on('submit(saveCommonModelBtn)', function(data){
                ...
                var $this = $(this);
                var para = {};
                para.pcbCtrctCertRelSgmtId =$("#saveCommonModelBtn").data("id");
                para.pcaCtrctBsSgmtId =dataObj.pcaCtrctBsSgmtId;
                para.brerType =  $("#brerType").val();
				para.certRelName =  $("#certRelName").val();
				para.certRelIdType =  $("#certRelIdType").val();
				para.certRelIdNum =  $("#certRelIdNum").val();
				$.ajax({
					type : "POST",  //请求方式
					url  : "inCtrctInf/saveCertRelSgmt.do",
					data : {commData  :  JSON.stringify(para),
