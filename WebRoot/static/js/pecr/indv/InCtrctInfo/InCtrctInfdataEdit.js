		...
		*/
		...
		$.when(d0,d1).done(function (d0,d1) {
			...
			if (dataObj && CtrctBsSgmt){
				...
				form.render('select');
				verify.dataValidation('base','I_CTRCT_BS',dataObj.pcaCtrctBsSgmtId,'form',false,false); //数据校验
			}else{
				$("#baseAddBtn").show();
			}
			//点击新增按钮
            ...
            form.on('submit(baseSaveBtn)', function(data){
				...
				//提交
				$.ajax({  
					url      : 'inCtrctInf/saveCtrctBsSgmt.do', 
					...
					success:function(data){ 
						if (data.success == true){
							...
							baseExistFlag = 'Y';
							dataObj = para; //存储新数据
							verify.dataValidation('base','I_CTRCT_BS', $this.data("id"),'form',true,false); //数据校验
			...
			$("#baseDelBtn").on("click",function () {
				...
				layer.confirm('删除基础段相当于删除该页面所有段信息，是否确认删除', {anim: 6}, function(index){
					...
					//提交
					$.ajax({ 
						url      : 'inCtrctInf/delCtrctBsSgmt.do', 
			...
			var commInfoUrl;
			if(pageType === 'edit'){
				commInfoUrl = 'inCtrctInf/getCtrctCertRelSgmtList.do?pcaCtrctBsSgmtId='+ dataObj.pcaCtrctBsSgmtId 
						+ "&pageType=" + pageType;
				$("#commonAddBtn").parent().children('div.layui-colla-content').addClass('layui-show');
			} else {
				commInfoUrl = 'inCtrctInf/getCtrctCertRelSgmtList.do?pageType='+ pageType;
			}
			...
			table.render({
				...
				cols : [[
					...
					{align:"center",field:"BRER_TYPE",title : "共同受信人身份类别",templet:function(d){return renderColumnDict(d,'BRER_TYPE',staticDict.infoIdType)}},
					{align:"center",field:"CERT_REL_NAME",title : "共同受信人名称"},
					{align:"center",field:"CERT_REL_ID_TYPE",title : "共同受信人身份标识类型",templet:function(d){return renderColumnDictComsharHodIdType(d,'CERT_REL_ID_TYPE',staticDict.idType,staticDict.entCertMainType,'BRER_TYPE')}},
            ...
            $("#commonAddBtn").on("click",function () {
                if (dataObj || baseExistFlag === 'Y'){
					...
					$("#cancelCommonModelBtn").data("id",dataObj.pcaCtrctBsSgmtId);
 //                   $("#saveCommonModelBtn").data("id","");
            ...
            table.on('tool(commonDataTable)', function(obj){
                ...
                if (layEvent === 'edit'){ //编辑
					...
					verify.dataValidation('common','I_CTRCT_CERT_REL', data.PCB_CTRCT_CERT_REL_SGMT_ID,'form',false,false); //数据校验
					...
					layer.confirm('确认删除该数据信息?', {anim: 6}, function(index){
						$.ajax({ 
							url      : 'inCtrctInf/delCtrctCertRelSgmt.do',
							data     : {str : JSON.stringify(para)},
							type     : 'post',  
							cache    : false,  
							dataType : 'json',  
            ...
            form.on('submit(saveCommonModelBtn)', function(data){
				...
				$.ajax({
					type : "POST",  //请求方式
					url  : "inCtrctInf/saveCertRelSgmt.do",
					...
					success : function(result) {
						if(result.success == true){
							...
							var newcommInfoUrl = 'inCtrctInf/getCtrctCertRelSgmtList.do?pcaCtrctBsSgmtId='+ dataObj.pcaCtrctBsSgmtId + '&pageType=edit';
							table.reload('commonDataTable', {url:newcommInfoUrl});
                            layer.close(modelIndex);
                            verify.dataValidation('common','I_CTRCT_CERT_REL', $("#saveCommonModelBtn").data("id"),'form',true,false); //数据校验
                            verify.dataValidation('base','I_CTRCT_BS', dataObj.pcaCtrctBsSgmtId,'form',true,false); //数据校验
						}else{
            ...
            if (dataObj && CreditLimSgmt){
				...
				form.render('select');
				verify.dataValidation('amount','I_CREDIT_LIM',CreditLimSgmt.PCC_CREDIT_LIM_SGMT_ID,'form',false,false); //数据校验
            ...
            form.on('submit(amountSaveBtn)', function(data){
				...
				//提交
				$.ajax({  
					url      : 'inCtrctInf/saveCreditLimSgmt.do', 
					data     : {str:JSON.stringify(para), pageType : $this.data("type")},
					type     : 'post',  
					cache    : false,  
					dataType : 'json',  
					success:function(data){ 
						if (data.success == true){
							...
							$this.data("type","edit");
							verify.dataValidation('amount','I_CREDIT_LIM',$this.data("id"),'form',true,false); //数据校验
							verify.dataValidation('base','I_CTRCT_BS',dataObj.pcaCtrctBsSgmtId,'form',true,false);
						} else {
							layer.alert(data.msg);
						}
					 },
            ...
            $("#amountDelBtn").on("click",function () {
                ...
                layer.confirm('确认删除', {anim: 6}, function(index){
                    ...
                    para.pccCreditLimSgmtId = $this.data("id");
					$.ajax({ 
						url      : 'inCtrctInf/delCreditLimSgmt.do',
						data     : {str : JSON.stringify(para)},
						type     : 'post',  
						cache    : false,  
						dataType : 'json',  
