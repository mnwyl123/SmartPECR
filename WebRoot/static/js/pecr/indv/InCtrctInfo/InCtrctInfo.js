...
layui.use(['form', 'laydate', 'element', 'layer', 'table', 'laypage','laytpl'], function(){
	...
	//展示已知数据
	table.render({
		elem: '#inCtrctInfTab', //表格所固定的div
		toolbar: '#toolbarHead',  //表格顶部工具栏
		url:'inCtrctInf/dataCtrctBsSgmtList.do',
    	cols: [[
			{type:'numbers'},	//列每行序号
			{type:'checkbox'}, //列复选框
			{field: 'pcaCtrctBsSgmtId', title: '主键', hide: true},
//			{field: 'mngmtOrgCode', align:'center', title: '所属机构'},
			{field: 'name', align:'center', title: '授信人姓名'},
			{field: 'idType', align:'center', title: '身份证件标识类别',templet:function(d){return renderColumnDict(d,'idType',staticDict.idType)}},
			...
			{field: 'rptDateCode', align:'center',title: '报告时点(状态)',templet:function(d){return renderColumnDict(d,'rptDateCode',staticDict.perCreditRptDateCode)}},
		...
		page: true, //是否分页
		limit: 10, //初始每页显示条数
		height: $(document).height() - $('#inCtrctInfTab').offset().top - $('#queryPanel').height() - 25,
		limits: [10, 20, 50],
	...
	table.on('toolbar(inCtrctInfTab)', function(obj){
		if(obj.event === 'addBtn') {
            ...
            layer.open({
                ...
                anim: 2,
                resize: false,
				content: 'inCtrctInf/inCtrctInfEdit.do?addOrUpdate=' + addOrUpdate
            });
		}else if(obj.event === 'submitBtn') {
			//获取选中的行信息
			 var checkStatus = table.checkStatus('inCtrctInfTab');
			 //获取选中行的数据
	...
	table.on('tool(inCtrctInfTab)', function(obj){
		var data = obj.data;
