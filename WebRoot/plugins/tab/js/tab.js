...
function TabView(f) {
    ...
    this.tabContainer = null;
   	var c = '<table class="tab_item"  id="{0}" border="0" cellpadding="0" cellspacing="0"><tr style="height:28px;"><td class="tab_item2 tab_title">{1}</td><td class="tab_item2"><div class="iconfont icon-cuohao" style="font-size:8px;color:#595959;"></div></td></tr></table>';
    ...
    if (g.position == "bottom") {
       c = '<table class="tab_item_bottom"  id="{0}" border="0" cellpadding="0" cellspacing="0"><tr style="height:28px;"><td class="tab_item2_bottom tab_title">{1}</td><td class="tab_item2_bottom"><div class="iconfont icon-cuohao tab_close_bottom" style="font-size:8px;color:#595959;"></div></td></tr></table>';
			...
			if($(n).attr("id") == "tab1_index1"){ //针对主页面的样式，事件做特殊处理
			  $(p).click(function () {
                if (a.current != null) {
					...
					$(a.current).find(".tab_item2" + b).parent().parent().parent().removeClass("tabactive");
				 }
                l.addClass("tab_item2_selected" + b);
				l.parent().parent().parent().addClass("tabactive1");
                a.current = this;
			   ...
			   $(p).mouseover(function () {
					...
					$(l).parent().parent().parent().addClass("tab_mouse_over");
					k.addClass("tab_item3_mouseover" + b);
					//主页样式不变
		            $("#tab1_index1").css("background-color","#ffffff");
              }).mouseout(function () {
            		m.removeClass("tab_item1_mouseover" + b);
            		l.removeClass("tab_item2_mouseover" + b);
            		l.parent().parent().parent().css("background-color","#ffffff");
            		$(l).parent().parent().parent().removeClass("tab_mouse_over");
            		$(a.current).css("background-color","#ffffff");//当前click的tab样式不变
                ...
                if (a.current != null) {
                    ...
                    $(a.current).find(".tab_item2" + b).removeClass("tab_item2_selected" + b);
					$(a.current).find(".tab_item2" + b).parent().parent().parent().removeClass("tabactive");
					$(a.current).find(".tab_item2" + b).parent().parent().parent().removeClass("tabactive1");
                    $(a.current).find(".tab_item3" + b).removeClass("tab_item3_selected" + b);
                    $(a.current).find(".icon-cuohao").addClass("tab_close_noselected");
					$(a.current).parent().parent().parent().find("table").css("background-color","#ffffff");
					$(a.current).parent().parent().parent().find("table");
                }
                /* */
                ...
                l.addClass("tab_item2_selected" + b);
				$(l).parent().parent().parent().addClass("tabactive");
            	...
            	if(3 == e.which) {
            		$("#context-menu").attr("tabId",$(this).attr('id'));
            	}
            });
    ...
    this.add = function (n) {
        ...
        if ($("#" + m.id).length > 0) {
           ...
           // this.tabContainer.append(l);
            ...
            $("#" + m.id).attr("data-toggle","context").attr("data-target","#context-menu");
            $("#" + m.id).contextmenu();
            this.activate(m.id)
        }
    };
