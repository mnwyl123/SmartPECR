 ...
 */
@Controller
@RequestMapping("/entInstitutionInfoSeo")
public class EntInstitutionBalanceSeoController extends BaseController{

	@Autowired
	private EntInstitutionBalanceSeoManager entInstitutionBalanceService;
	/**
	 * 数据列表界面
	 * @return
	 */
	...
	public String dataPage() {
		return "pecr/ent/seo/entInstitutionSeo/entInstitutionSeoList";
	}
	...
	public Map<String, Object> dataList(Page page){
		...
		try {
			...
			page.setPd(pd);
			List<PageData> dataList = entInstitutionBalanceService.dataList(page);
	...
	public ModelAndView listHistPage(Page page) { 
		...
		PageData pd = this.getPageData();
		mv.addObject("pd", pd);
		mv.setViewName("pecr/ent/seo/entInstitutionSeo/entInstitutionSeoHist");
		return mv;
	}
	...
	public Map<String, Object> dataHistList(Page page){
		...
		try {
			...
			page.setShowCount(limit);
			page.setPd(pd);
			List<EntInstitutionBalance> dataList = entInstitutionBalanceService.dataHistList(page);
			resultMap.put("code", 0);
			resultMap.put("msg","");
			resultMap.put("count", page.getTotalResult());
			resultMap.put("data", dataList);
	...
	public Map<String, Object> updatedate(Page page){
		...
		try {
			...
			entInstitutionBalanceService.updateStatus(map);
	...
	public Map<String, Object> backdate(Page page){
		...
		try {
			...
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("COMPANY", company);
			map.put("baseId", pd.getString("ekaInstitutionBalanceShtId"));
			map.put("reportMessageId", pd.getString("reportMessageId"));
			entInstitutionBalanceService.backdate(map);
			entInstitutionBalanceService.updateStatus(map);
			entInstitutionBalanceService.deletedate(map);
			resultMap.put("success", true);
		}catch(Exception e) {
			logger.error(e.getMessage());
			resultMap.put("success", false);
