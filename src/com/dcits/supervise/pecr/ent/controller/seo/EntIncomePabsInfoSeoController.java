 ...
 */
@Controller
@RequestMapping("/entIncomeInfoSeo")
public class EntIncomePabsInfoSeoController extends BaseController{

	
	@Autowired
	private EntIncomePabsInfoSeoManager entIncomePabsInfoService;
	...
	public String dataPage() {
		return "pecr/ent/seo/entIncomeSeo/entIncomeSeoList";
	}
	...
	public Map<String, Object> dataList(Page page){
		...
		try {
			...
			page.setPd(pd);
			List<PageData> dataList = entIncomePabsInfoService.dataList(page);
	...
	public ModelAndView listHistPage(Page page) { 
		...
		PageData pd = this.getPageData();
		mv.addObject("pd", pd);
		mv.setViewName("pecr/ent/seo/entIncomeSeo/entIncomeSeoHist");
		return mv;
	}
	...
	public Map<String, Object> dataHistList(Page page){
		...
		try {
			...
			page.setShowCount(limit);
			page.setPd(pd);
			List<EntIncomeStatement> dataList = entIncomePabsInfoService.dataHistList(page);
			resultMap.put("code", 0);
			resultMap.put("msg","");
			resultMap.put("count", page.getTotalResult());
			resultMap.put("data", dataList);
	...
	public Map<String, Object> updatedate(Page page){
		...
		try {
			...
			entIncomePabsInfoService.updateStatus(map);
	...
	public Map<String, Object> backdate(Page page){
		...
		try {
			...
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("COMPANY", company);
			map.put("baseId", pd.getString("eiaIncomeStatementProfitId"));
			map.put("reportMessageId", pd.getString("reportMessageId"));
			entIncomePabsInfoService.backdate(map);
			entIncomePabsInfoService.updateStatus(map);
			entIncomePabsInfoService.deletedate(map);
			resultMap.put("success", true);
		}catch(Exception e) {
			logger.error(e.getMessage());
			resultMap.put("success", false);
