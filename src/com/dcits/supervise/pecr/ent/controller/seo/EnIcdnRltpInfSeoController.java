...
@Controller
@RequestMapping("/enIcdnRltpInfSeo")
public class EnIcdnRltpInfSeoController extends BaseController{

	@Autowired
	private EnIcdnRltpInfSeoManager EnIcdnRltpInfService;
	...
	public String dataPage() {
		return "pecr/ent/seo/enIcdnRltpInf/enIcdnRltpInfSeoList";
	}
	...
	public Map<String, Object> dataList(Page page){
		...
		try {
			...
			page.setPd(pd);
			List<PageData> dataList = EnIcdnRltpInfService.dataList(page);
	...
	public ModelAndView listHistPage(Page page) { 
		...
		PageData pd = this.getPageData();
		mv.addObject("pd", pd);
		mv.setViewName("pecr/ent/seo/enIcdnRltpInf/enIcdnRltpInfSeoHist");
		return mv;
	}
	...
	public Map<String, Object> dataHistList(Page page){
		...
		try {
			...
			page.setShowCount(limit);
			page.setPd(pd);
			List<EnIcdnRltpInf> dataList = EnIcdnRltpInfService.dataHistList(page);
			resultMap.put("code", 0);
			resultMap.put("msg","");
			resultMap.put("count", page.getTotalResult());
			resultMap.put("data", dataList);
	...
	public ModelAndView dataDetail(Page page) { 
		...
		try {
			...
			mv.addObject("enIcdnRltpInfSeo", JSONUtils.toJSONString(dataList));
			mv.addObject("pd", pd);
		}catch(Exception e) {
			logger.error(e.getMessage());
		}
		mv.setViewName("pecr/ent/seo/enIcdnRltpInf/enIcdnRltpInfSeoEdit");
		return mv;
	}
	...
	public Map<String, Object> updatedate(Page page){
		...
		try {
			...
			EnIcdnRltpInfService.updateStatus(map);
	...
	public Map<String, Object> backdate(Page page){
		...
		try {
			...
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("COMPANY", company);
			map.put("baseId", pd.getString("ecaEnIcdnRltpInfId"));
			map.put("reportMessageId", pd.getString("reportMessageId"));
			EnIcdnRltpInfService.backdate(map);
			EnIcdnRltpInfService.updateStatus(map);
			EnIcdnRltpInfService.deletedate(map);
			resultMap.put("success", true);
		}catch(Exception e) {
			logger.error(e.getMessage());
			resultMap.put("success", false);
