...
public class ImportExportData {
	...
	private static Logger logger = Logger.getLogger(ImportExportData.class);
	    ...
	    public ImportExportData(File file,LoanAcctInfoManager loanAcctInfoService,String COMPANY){  
	        this.file = file; 
	        this.loanAcctInfoService = loanAcctInfoService;
	    	 ...
	    	 setHandler(new SimpleSheetContentsHandler(){
			        ...
			        public void endRow(int rowNum) {
			            ...
			            if(listData.size()>=1000){
			            	//调用数据入库方法
			            	try {
								InsertData(listData,fields,loanAcctInfoService,COMPANY);
							} catch (Exception e) {
								logger.error("Excel批量导入，个人借贷按段更正月度表现信息出错！",e);
	    ...
	    public void InsertData(List<List<String>> listData,List<String> fields,LoanAcctInfoManager loanAcctInfoService,String COMPANY) throws Exception{
	    	//组织插入数据
			...
			//入库
			loanAcctInfoService.saveAmblgChangeListInfo(dataMapList);
	    }
}
