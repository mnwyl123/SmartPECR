...
public class FeedBackReportPartFail422 extends FeedBackReportPartFailService{

	private ReportFeedBackMessageMapper reportFeedBackMessageMapper;
	
	private static final Logger logger = LoggerFactory.getLogger(FeedBackReportPartFail422.class);
	
	public FeedBackReportPartFail422(ReportFeedBackMessageMapper reportFeedBackMessageMapper){
	...
	String selectIdByJSONObject(Document reportDocument) throws Exception {
		...
		Element bas = rootElt.element("EnCtrctInfMdfc");
		Element sgmt = bas.element("BsSgmt");
		String table_name = ReportType.getReportTypeBaseTable(ReportType.REPORT_422);
		String pk = ReportType.getReportTypePrimaryKey(ReportType.REPORT_422);
		...
		list.add(new ConditionColumAndValue("CONTRACT_CODE",sgmt.element("ModRecCode").getStringValue()));
