...
public class FeedBackReportPartFail640 extends FeedBackReportPartFailService{

	private ReportFeedBackMessageMapper reportFeedBackMessageMapper;
	
	private static final Logger logger = LoggerFactory.getLogger(FeedBackReportPartFail640.class);
	
	public FeedBackReportPartFail640(ReportFeedBackMessageMapper reportFeedBackMessageMapper){
	...
	String selectIdByJSONObject(Document reportDocument) throws Exception {
		...
		Element bas = rootElt.element("InstitutionBalanceSheet");
		Element sgmt = bas.element("InstitutionBalanceSheetBsSgmt");
		String table_name = ReportType.getReportTypeBaseTable(ReportType.REPORT_640);
		String pk = ReportType.getReportTypePrimaryKey(ReportType.REPORT_640);
