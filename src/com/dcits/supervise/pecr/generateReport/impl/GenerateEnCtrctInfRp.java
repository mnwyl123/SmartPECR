 ...
 *
 */
public class GenerateEnCtrctInfRp extends AbstractGenerateReport {

	private final static Logger logger = Logger.getLogger(GenerateEnCtrctInfRp.class);

	public Properties properties;
	
	public ReportMessageManager reportMessageService;

	private static String TABLE_NAME = "E_CTRCT_BS";

	private static String PACKET_KEY = "EGA_CTRCT_BS_SGMT_ID";
	
	private int count =0;
	
	private String CCRC_ORG_CODE = "";

	public GenerateEnCtrctInfRp(Properties properties, ReportMessageManager reportMessageService) {
	...
	public Map<String, Object> generateBody(NeedReportDetail needReportDetail, String name,
		...
		//信息记录类型
		reportCount.put("INF_REC_TYPE", PecrConstantUtil.ENT_EN_CTRCT_INF);
		...
		for (int f = 0; f < forNum; f++) {
			...
			reportCount.put("INF_REC_TYPE", PecrConstantUtil.ENT_EN_CTRCT_INF);
			//获取组织企业授信协议报文所需要的数据
			list = reportMessageService.queryEnCtrctInfRpData(reportCount);
			
			// 存放报文名和对应数据ID的map
			List<String> paaid = new ArrayList<>();
			List<Map<String,String>> pkColnumList = new ArrayList<Map<String,String>>();
			for(int i=0;i<list.size();i++){
				Map<String, Object> enCtrctInfEntity = list.get(i);
				String pk = enCtrctInfEntity.get("EGA_CTRCT_BS_SGMT_ID").toString();
				paaid.add(pk);
			...
			// 获取共同授信人的数据信息（通过order by 字段EGA_CTRCT_BS_SGMT_ID
			// 将相同的连续放在一起，其外获取每一种相同EGA_CTRCT_BS_SGMT_ID的条数也返回到当前数据中 ）
			// 依据返回结果，将相同EGA_CTRCT_BS_SGMT_ID的数据放入一个list中，在将EGA_CTRCT_BS_SGMT_ID的值作为key形成一个map，方便其他标识段的生成和减少频繁数据库的访问
			List<Map<String, Object>> idlist = reportMessageService.queryEntBsIDSgmtRpData(paaid);
			//放置共同授信人信息数据
			...
			if(f%fileForNum == 0){
				String headStr=generateRpHead(PecrConstantUtil.ENT_EN_CTRCT_INF, numTotal[forTop++], "  "+CCRC_ORG_CODE);
				sbf.append(headStr);
				sbf.append("\r\n");
			}
			for (Map<String, Object> indvEntity : list) {
				...
				Element InCtrctInf = root.addElement("EnCtrctInf");
				/*** 企业授信信息 --- 基础段 ****/
				...
				if (map.get(indvEntity.get("EGA_CTRCT_BS_SGMT_ID").toString()) != null) {
					...
					writeCtrctCertRelSgmt(CtrctCertRelSgmt, map.get(indvEntity.get("EGA_CTRCT_BS_SGMT_ID").toString()));
				}
				/*** 企业授信信息 --- -记录额度段-- ****/
			...
			if(f%fileForNum == 0){
				...
				serialNumMap.put("inRefType", PecrConstantUtil.ENT_EN_CTRCT_INF);
				...
				reportName = generateRpName(PecrConstantUtil.ENT_EN_CTRCT_INF,reportNum, "0"+CCRC_ORG_CODE);
			...
			if((f+1)%fileForNum == 0||f==(forNum-1)){
				...
				rabme = AbstractGenerateReport.generateRpMessage(rpt_date, rabme, PecrConstantUtil.ENT_EN_CTRCT_INF, reportName, 
		...
		logger.info("XML格式BODY内容生成执行完成--------------企业授信协议信息生成完毕");
		return reportMap;
	}
	...
	public static void writeCtrctCertRelSgmt(Element CtrctCertRelSgmt, List<Map<String, Object>> borrower) {
		...
		for (Map<String, Object> map : borrower) {
			...
			CtrctCertRel.addElement("CertRelIDType").setText(
					map.get("CERT_REL_IDTYPE") != null ? map.get("CERT_REL_IDTYPE").toString() : "");
	...
	public static void writeCreditLimSgmt(Element CreditLimSgmt, Map<String, Object> indvEntity) {
		...
		CreditLimSgmt.addElement("Cy").setText(indvEntity.get("CY") != null ? indvEntity.get("CY").toString() : "");
