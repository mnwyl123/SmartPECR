 ...
 *
 */
public class GenerateInSecAcctMdfcRp extends AbstractGenerateReport {

	private final static Logger logger = Logger.getLogger(GenerateInSecAcctMdfcRp.class);
	...
	public GenerateInSecAcctMdfcRp(Properties properties, ReportMessageManager reportMessageService) {
	...
	public Map<String, Object> generateBody(NeedReportDetail needReportDetail, String name,
		...
		table_name = "I_GUAR_ACCT_BS";
		PACKET_KEY = "PDA_GUAR_ACCT_BS_SGMT_ID";
		// 获取要生成的报文总条数
		...
		for (int f = 0; f < forNum; f++) {
			...
			reportCount.put("company", needReportDetail.getCompany());
			reportCount.put("INF_REC_TYPE", infRecType);
			List<Map<String, Object>> list = reportMessageService.dataGuarAcctBsSgmtlist(reportCount);
			// 存放报文名和对应数据ID的map
			...
			List<Map<String, Object>> rltlist = reportMessageService.getRLTRepymtRpMsgByFactor(paaid);
			//放置相关还款责任人信息
			...
			//一个文件只能有一个报文头
			if(f%fileForNum == 0){
				String headStr=generateRpHead(infRecType, numTotal[forTop++], "  "+CCRC_ORG_CODE);
				sbf.append(headStr);
				sbf.append("\r\n");
			}
			for (Map<String, Object> indvEntity : list) {
				...
				Element InSecAcctMdfc = root.addElement("InSecAcctMdfc");
				/*** 个人担保更正段请求信息 --- 基础段 ****/
				Element BsSgmt = InSecAcctMdfc.addElement("BsSgmt");
				writeBsSgmt(BsSgmt, indvEntity);
				/*** 个人担保更正段请求信息 --- -变更段-- ****/
				Element MdfcSgmt = InSecAcctMdfc.addElement("MdfcSgmt");
				switch (indvEntity.get("MDFC_SGMT_CODE").toString()) {
				case PecrConstantUtil.IN_SEC_ACCT_MDFC_B:
					/*** 个人担保基本信息 --- 基础段 ****/
					Element GuarAcctBsSgmt = MdfcSgmt.addElement("GuarAcctBsSgmt");
					writeGuarAcctBsSgmt(GuarAcctBsSgmt, indvEntity);
					break;
				case PecrConstantUtil.IN_SEC_ACCT_MDFC_C:
					/*** 个人担保基本信息 --- 基本信息段 ****/
					Element GuarAcctBsInfSgmt = MdfcSgmt.addElement("GuarAcctBsInfSgmt");
					GenerateInSecAcctInfRp.writeGuarAcctBsInfSgmt(GuarAcctBsInfSgmt, indvEntity);
					break;
				case PecrConstantUtil.IN_SEC_ACCT_MDFC_D:
					/*** 个人担保在保责任信息 --- --- ****/
					Element GuarRltRepymtInfSgmt = MdfcSgmt.addElement("GuarRltRepymtInfSgmt");
					GenerateInSecAcctInfRp.writeRltRepymtInfSgmt(GuarRltRepymtInfSgmt, indvEntity);
					break;
				case PecrConstantUtil.IN_SEC_ACCT_MDFC_E:
					/*** 个人担保还款人信息 --- --- ****/
					if (rltmap.get(indvEntity.get("PDA_GUAR_ACCT_BS_SGMT_ID").toString()) != null) {
						...
						GenerateInSecAcctInfRp.writeRltSgmt(RltRepymtInfSgmt,
								rltmap.get(indvEntity.get("PDA_GUAR_ACCT_BS_SGMT_ID").toString()));
					}
					break;
				/*case PecrConstantUtil.IN_SEC_ACCT_MDFC_F:
					*//*** 个人担保抵质押信息 --- --- ****//*
					if (mccmap.get(indvEntity.get("PDA_GUAR_ACCT_BS_SGMT_ID").toString()) != null) {
						...
						GenerateInSecAcctInfRp.writeMccSgmt(GuarMotgaCltalCtrctInfSgmt,
		...
		logger.info("XML格式BODY内容生成执行完成 -----------wzy-------"+infRecType+"-----个人担保账户更正段-----生成完毕");
		return reportMap;
	}
