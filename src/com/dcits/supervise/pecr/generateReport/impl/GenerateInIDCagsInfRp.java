 ...
 *
 */
public class GenerateInIDCagsInfRp extends AbstractGenerateReport {

	private final static Logger logger = Logger.getLogger(GenerateInIDCagsInfRp.class);
	...
	public GenerateInIDCagsInfRp(Properties properties, ReportMessageManager reportMessageService) {
	...
	public Map<String, Object> generateBody(NeedReportDetail needReportDetail, String name,
		...
		case PecrConstantUtil.IN_ACCT_ID_CAGS_INF:
			PACKET_KEY = "PBW_IN_ACCT_ID_CAGS_INF_ID";
			element = "InAcctIDCagsInf";
			table_name = "I_IN_ACCT_ID_CAGS_INF";
			break;
		case PecrConstantUtil.IN_CTRCT_ID_CAGS_INF:
			PACKET_KEY = "PCW_IN_CTRCTID_CAGS_INF_ID";
			element = "InCtrctIDCagsInf";
			table_name = "I_IN_CTRCTID_CAGS_INF";
			break;
		case PecrConstantUtil.IN_SEC_ACCT_ID_CAGS_INF:
			PACKET_KEY = "PDW_INSECACCTID_CAGSINF_ID";
			element = "InSecAcctIDCagsInf";
			table_name = "I_INSECACCTID_CAGSINF";
			break;
		}
		// 获取要生成的报文总条数
		...
		for (int f = 0; f < forNum; f++) {
			...
			switch (infRecType) {
			case PecrConstantUtil.IN_ACCT_ID_CAGS_INF:
				list = reportMessageService.queryInIDCagsInfRpData1(reportCount);
				break;
			case PecrConstantUtil.IN_CTRCT_ID_CAGS_INF:
				list = reportMessageService.queryInIDCagsInfRpData2(reportCount);
				break;
			case PecrConstantUtil.IN_SEC_ACCT_ID_CAGS_INF:
				list = reportMessageService.queryInIDCagsInfRpData3(reportCount);
				break;
			}
		...
		logger.info("XML格式BODY内容生成执行完成 -----------wzy-------"+infRecType+"-----个人标识变更-----生成完毕");
		return reportMap;
	}
	...
	public void updateBusinessState(NeedReportDetail needReportDetail) throws Exception {
		if (needReportDetail.getBusinessState() != null && needReportDetail.getBusinessState() != "") {
			...
			case PecrConstantUtil.BUSINESS_STATUS_30:
				...
				reportMessageService.updatenInIDCagsInfRpDataStatus(conditionMap);
				break;
	...
	public void writeInDlt(Element element, Map<String, Object> indvEntity) {
		...
		element.addElement("OdBnesCode")
				.setText(indvEntity.get("OD_BNES_CODE") != null ? indvEntity.get("OD_BNES_CODE").toString() : "");
		element.addElement("NwBnesCode")
				.setText(indvEntity.get("NW_BNES_CODE") != null ? indvEntity.get("NW_BNES_CODE").toString() : "");
	}
