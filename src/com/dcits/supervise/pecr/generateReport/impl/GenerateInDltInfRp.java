 ...
 *
 */
public class GenerateInDltInfRp extends AbstractGenerateReport {

	private final static Logger logger = Logger.getLogger(GenerateInDltInfRp.class);
	...
	public GenerateInDltInfRp(Properties properties, ReportMessageManager reportMessageService) {
	...
	public Map<String, Object> generateBody(NeedReportDetail needReportDetail, String name,
		...
		/***** 记录数 *****/
		switch (needReportDetail.getInfRecType()) {
		case PecrConstantUtil.IN_ACCT_DEL:
			PACKET_KEY = "PBY_IN_ACCT_DEL_ID";
			element = "InAcctDel";
			table_name = "I_IN_ACCT_DEL";
			break;
		case PecrConstantUtil.IN_CTRCT_DEL:
			PACKET_KEY = "PCY_IN_CTRCT_DEL_ID";
			element = "InCtrctDel";
			table_name = "I_IN_CTRCT_DEL";
			break;
		case PecrConstantUtil.IN_SEC_ACCT_DEL:
			PACKET_KEY = "PDY_IN_SEC_ACCT_DEL_ID";
			element = "InSecAcctDel";
			table_name = "I_IN_SEC_ACCT_DEL";
			break;
		}
		...
		for (int f = 0; f < forNum; f++) {
			...
			switch (infRecType) {
			case PecrConstantUtil.IN_ACCT_DEL:
				list = reportMessageService.queryInDltRpData1(reportCount);
				break;
			case PecrConstantUtil.IN_CTRCT_DEL:
				list = reportMessageService.queryInDltRpData2(reportCount);
				break;
			case PecrConstantUtil.IN_SEC_ACCT_DEL:
				list = reportMessageService.queryInDltRpData3(reportCount);
				break;
			}
			// 存放报文名和对应数据ID的map
			...
			for (Map<String, Object> indvEntity : list) {
				...
				writeInDlt(InCtrctInf, indvEntity);
		...
		logger.info("XML格式BODY内容生成执行完成 -----------wzy-------"+infRecType+"-----个人按段删除-----生成完毕");
		return reportMap;
	}
	...
	public void updateBusinessState(NeedReportDetail needReportDetail) throws Exception {
		if (needReportDetail.getBusinessState() != null && needReportDetail.getBusinessState() != "") {
			...
			case PecrConstantUtil.BUSINESS_STATUS_30:
				...
				reportMessageService.updateInDltInfRpDataStatus(conditionMap);
				break;
	...
	public void writeInDlt(Element element, Map<String, Object> indvEntity) {
		...
		element.addElement("DelSgmtCode")
				.setText(indvEntity.get("DEL_SGMT_CODE") != null ? indvEntity.get("DEL_SGMT_CODE").toString() : "");
		element.addElement("DelStartDate")
				.setText(indvEntity.get("DEL_START_DATE") != null ? indvEntity.get("DEL_START_DATE").toString() : "");
		element.addElement("DelEndDate")
				.setText(indvEntity.get("DEL_END_DATE") != null ? indvEntity.get("DEL_END_DATE").toString() : "");
	}
