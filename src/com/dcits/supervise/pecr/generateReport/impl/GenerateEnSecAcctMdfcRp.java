 ...
 *
 */
public class GenerateEnSecAcctMdfcRp extends AbstractGenerateReport {

	private final static Logger logger = Logger.getLogger(GenerateEnSecAcctMdfcRp.class);
	...
	public GenerateEnSecAcctMdfcRp(Properties properties, ReportMessageManager reportMessageService) {
	...
	public Map<String, Object> generateBody(NeedReportDetail needReportDetail, String name,
		...
		for (int f = 0; f < forNum ; f++) {
			...
			reportCount.put("company", needReportDetail.getCompany());
			reportCount.put("INF_REC_TYPE", infRecType);
			List<Map<String, Object>> list = reportMessageService.getGuarAcctBsSgmtByFactor(reportCount);
			// 存放报文名和对应数据ID的map
			...
			for (Map<String, Object> indvEntity : list) {
				...
				Element EnSecAcctMdfc = root.addElement("EnSecAcctMdfc");
				/*** 个人授信更正段请求信息 --- 基础段 ****/
				Element BsSgmt = EnSecAcctMdfc.addElement("BsSgmt");
				writeBsSgmt(BsSgmt, indvEntity);
				/*** 个人授信更正段请求信息 --- -变更段-- ****/
				Element MdfcSgmt = EnSecAcctMdfc.addElement("MdfcSgmt");
				switch (indvEntity.get("MDFC_SGMT_CODE").toString()) {
				case PecrConstantUtil.EN_SEC_ACCT_MDFC_B:
					/*** 企业担保账户信息 基础段 ****/
					Element GuarAcctBsSgmt = MdfcSgmt.addElement("GuarAcctBsSgmt");
					writeGuarAcctBsSgmt(GuarAcctBsSgmt, indvEntity);
					break;
				case PecrConstantUtil.EN_SEC_ACCT_MDFC_C:
					/*** 企业担保账户信息基本信息段 ****/
					Element GuarAcctBsInfSgmt = MdfcSgmt.addElement("GuarAcctBsInfSgmt");
					GenerateEnSecAcctInfRp.writeGuarAcctBsInfSgmt(GuarAcctBsInfSgmt, indvEntity);
					break;
				case PecrConstantUtil.EN_SEC_ACCT_MDFC_D:
					/*** 企业担保更正段标识——D-在保责任信息段 ****/
					Element GuarRltRepymtInfSgmt = MdfcSgmt.addElement("GuarRltRepymtInfSgmt");
					GenerateEnSecAcctInfRp.writeGuarRltRepymtInfSgmt(GuarRltRepymtInfSgmt, indvEntity);
					break;
				case PecrConstantUtil.EN_SEC_ACCT_MDFC_E:
					/*** 企业担保账户信息相关还款责任人段 ****/
					if (rltmap.get(indvEntity.get("EEA_GUAR_ACCT_BS_SGMT_ID").toString()) != null) {
						...
						GenerateEnSecAcctInfRp.writeRltRepymtInfSgmt(RltRepymtInfSgmt,
								rltmap.get(indvEntity.get("EEA_GUAR_ACCT_BS_SGMT_ID").toString()));
					}
					break;
				/*case PecrConstantUtil.EN_SEC_ACCT_MDFC_F:
				 * if (mccmap.get(indvEntity.get("EEA_GUAR_ACCT_BS_SGMT_ID").toString()) != null) {
						Element GuarMotgaCltalCtrctInfSgmt = MdfcSgmt.addElement("GuarMotgaCltalCtrctInfSgmt");
						GenerateEnSecAcctInfRp.writeGuarMotgaCltalCtrctInfSgmt(GuarMotgaCltalCtrctInfSgmt,
								mccmap.get(indvEntity.get("EEA_GUAR_ACCT_BS_SGMT_ID").toString()));
					}
					break;*/
				case PecrConstantUtil.EN_SEC_ACCT_MDFC_G:
					/*** 企业担保账户信息授信额度信息段 ****/
					Element GuarAcctCredSgmt = MdfcSgmt.addElement("GuarAcctCredSgmt");
					GenerateEnSecAcctInfRp.writeGuarAcctCredSgmt(GuarAcctCredSgmt, indvEntity);
					break;
				default:
					break;
				}
