 ...
 *
 */
public class GenerateEnIcdnRltpInfrpRp extends AbstractGenerateReport {

	private final static Logger logger = Logger.getLogger(GenerateEnIcdnRltpInfrpRp.class);

	public Properties properties;

	public ReportMessageManager reportMessageService;

	private static String TABLE_NAME = "E_ICDN_RLTP_INF";

	private static String PACKET_KEY = "ECA_EN_ICDN_RLTP_INF_ID";
	
	private int count = 0;
	
	private String CCRC_ORG_CODE = "";

	public GenerateEnIcdnRltpInfrpRp(Properties properties, ReportMessageManager reportMessageService) {
	...
	public Map<String, Object> generateBody(NeedReportDetail needReportDetail, String name,
		...
		reportCount.put("INF_REC_TYPE",PecrConstantUtil.ENT_EN_ICDN_RLTP_INF);
		...
		for (int f = 0; f < forNum; f++) {
			...
			reportCount.put("INF_REC_TYPE",PecrConstantUtil.ENT_EN_ICDN_RLTP_INF);
			//获取拼装报文的企业关联关系数据。
			list = reportMessageService.getEnIcdnRltpInfByFactor(reportCount);
			...
			for(int i=0;i<list.size();i++){
				Map<String, Object> enIcdnRltpInfEntity = list.get(i);
				String pk = enIcdnRltpInfEntity.get("ECA_EN_ICDN_RLTP_INF_ID").toString();
				pdaid.add(pk);
			...
			if(f%fileForNum == 0){
				sbf.append(generateRpHead(PecrConstantUtil.ENT_EN_ICDN_RLTP_INF, numTotal[forTop++], "  "+CCRC_ORG_CODE));
				sbf.append("\r\n");
			}
			...
			for (Map<String, Object> indvEntity : list) {
				...
				Element EnIcdnRltpInf = root.addElement("EnIcdnRltpInf");
				writeEnIcdnRltpInf(EnIcdnRltpInf, indvEntity);
			...
			if(f%fileForNum == 0){
				...
				serialNumMap.put("inRefType", PecrConstantUtil.ENT_EN_ICDN_RLTP_INF);
				...
				reportName = generateRpName(PecrConstantUtil.ENT_EN_ICDN_RLTP_INF,reportNum, "0"+CCRC_ORG_CODE);
			...
			if((f+1)%fileForNum == 0||f==(forNum-1)){
				...
				rabme =  AbstractGenerateReport.generateRpMessage(rpt_date, rabme, PecrConstantUtil.ENT_EN_ICDN_RLTP_INF, reportName, 
		...
		logger.info("XML格式BODY内容生成执行完成 -----------企业关联关系生成完毕");
		return reportMap;
	}
	...
	public void updateBusinessState(NeedReportDetail needReportDetail) throws Exception {
		if (needReportDetail.getBusinessState() != null && needReportDetail.getBusinessState() != "") {
			...
			case PecrConstantUtil.BUSINESS_STATUS_30:
				...
				reportMessageService.updateEnIcdnRltpInfDataStatus(conditionMap);
				break;
	...
	public void writeEnIcdnRltpInf(Element EnIcdnRltpInf, Map<String, Object> indvEntity) {
		EnIcdnRltpInf.addElement("InfRecType")
				.setText(indvEntity.get("INF_REC_TYPE") != null ? indvEntity.get("INF_REC_TYPE").toString() : "");
		EnIcdnRltpInf.addElement("EntName")
				.setText(indvEntity.get("ENT_NAME") != null ? indvEntity.get("ENT_NAME").toString() : "");
		EnIcdnRltpInf.addElement("EntCertType")
				.setText(indvEntity.get("ENT_CERT_TYPE") != null ? indvEntity.get("ENT_CERT_TYPE").toString() : "");
		EnIcdnRltpInf.addElement("EntCertNum")
				.setText(indvEntity.get("ENT_CERT_NUM") != null ? indvEntity.get("ENT_CERT_NUM").toString() : "");
		EnIcdnRltpInf.addElement("AssoEntName")
				.setText(indvEntity.get("ASSO_ENT_NAME") != null ? indvEntity.get("ASSO_ENT_NAME").toString() : "");
		EnIcdnRltpInf.addElement("AssoEntCertType").setText(
				indvEntity.get("ASSO_ENT_CERT_TYPE") != null ? indvEntity.get("ASSO_ENT_CERT_TYPE").toString() : "");
		EnIcdnRltpInf.addElement("AssoEntCertNum").setText(
				indvEntity.get("ASSO_ENT_CERT_NUM") != null ? indvEntity.get("ASSO_ENT_CERT_NUM").toString() : "");
		EnIcdnRltpInf.addElement("AssoType")
				.setText(indvEntity.get("ASSO_TYPE") != null ? indvEntity.get("ASSO_TYPE").toString() : "");
		EnIcdnRltpInf.addElement("AssoSign")
				.setText(indvEntity.get("ASSO_SIGN") != null ? indvEntity.get("ASSO_SIGN").toString() : "");
		EnIcdnRltpInf.addElement("RptDate")
