 ...
 *
 */
public class GenerateMoCEntDltInfRp extends AbstractGenerateReport {

	private final static Logger logger = Logger.getLogger(GenerateMoCEntDltInfRp.class);
	...
	public GenerateMoCEntDltInfRp(Properties properties, ReportMessageManager reportMessageService) {
	...
	public Map<String, Object> generateBody(NeedReportDetail needReportDetail, String name,
		...
		PACKET_KEY = "EFZ_MOC_ENT_DEL_ID";
		table_name = "MOC_ENT_DEL";
		// 获取要生成的报文总条数
		...
		for (int f = 0; f < forNum; f++) {
			...
			reportCount.put("company", needReportDetail.getCompany());
			reportCount.put("INF_REC_TYPE", infRecType);
			List<Map<String, Object>> list = reportMessageService.queryMoCEntDltInfRpData(reportCount);
			...
			for (Map<String, Object> indvEntity : list) {
				...
				Element InCtrctInf = root.addElement("MoCEntDel");
				writeMoCEntDlt(InCtrctInf, indvEntity);
		...
		logger.info("XML格式BODY内容生成执行完成 -----------wzy-------"+infRecType+"-----抵质押合同模块整笔删除-----生成完毕");
		return reportMap;
	}
	...
	public void updateBusinessState(NeedReportDetail needReportDetail) throws Exception {
		if (needReportDetail.getBusinessState() != null && needReportDetail.getBusinessState() != "") {
			...
			case PecrConstantUtil.BUSINESS_STATUS_30:
				...
				reportMessageService.updatenMoCEntDltInfRpDataStatus(conditionMap);
				break;
	...
	public void writeMoCEntDlt(Element element, Map<String, Object> indvEntity) {
