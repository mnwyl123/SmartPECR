 ...
 *
 */
public class GenerateInSpcEvtDscInfRp extends AbstractGenerateReport {

	private final static Logger logger = Logger.getLogger(GenerateInSpcEvtDscInfRp.class);

	public Properties properties;
	
	public ReportMessageManager reportMessageService;

	private String table_name = "I_IN_SPC_EVT_DSC_INF";

	private String PACKET_KEY = "PBK_IN_SPC_EVT_DSC_INF_ID";
	
	private int count = 0;

	private String CCRC_ORG_CODE = "";

	public GenerateInSpcEvtDscInfRp(Properties properties, ReportMessageManager reportMessageService) {
	...
	public Map<String, Object> generateBody(NeedReportDetail needReportDetail, String name,
		...
		//信息记录类型
		reportCount.put("INF_REC_TYPE", PecrConstantUtil.IN_SPC_EVT_DSC_INF);
		...
		for (int f = 0; f < forNum; f++) {
			...
			reportCount.put("INF_REC_TYPE", PecrConstantUtil.IN_SPC_EVT_DSC_INF);
			//获取组织报文的数据
			list = reportMessageService.queryInSpcEvtDscInfRpData(reportCount);
			...
			// 一个报文一个报文头
			if(f%fileForNum == 0){
				sbf.append(generateRpHead(PecrConstantUtil.IN_SPC_EVT_DSC_INF, numTotal[forTop++], "  "+CCRC_ORG_CODE));
				sbf.append("\r\n");
			}
			for (Map<String, Object> Entity : list) {
				...
				/*** 个人借贷特殊事件说明信息 ***/
				Element InSpcEvtDscInf = root.addElement("InSpcEvtDscInf");
				writeInSpcEvtDscInf(InSpcEvtDscInf, Entity);
			...
			if(f%fileForNum == 0){
				...
				serialNumMap.put("inRefType", PecrConstantUtil.IN_SPC_EVT_DSC_INF);
				...
				reportName = generateRpName(PecrConstantUtil.IN_SPC_EVT_DSC_INF,reportNum, "0"+CCRC_ORG_CODE);
			...
			if((f+1)%fileForNum == 0||f==(forNum-1)){
				...
				rabme =  AbstractGenerateReport.generateRpMessage(rpt_date, rabme, PecrConstantUtil.IN_SPC_EVT_DSC_INF, reportName, 
		...
		logger.info("XML格式BODY内容生成执行完成 -----------借贷账户特殊事件说明信息生成完毕");
		return reportMap;
	}
	...
	public void updateBusinessState(NeedReportDetail needReportDetail) throws Exception {
		if (needReportDetail.getBusinessState() != null && needReportDetail.getBusinessState() != "") {
			...
			case PecrConstantUtil.BUSINESS_STATUS_30:
				...
				reportMessageService.updateInSpcEvtDscInfRpDataStatus(conditionMap);
				break;
	...
	public void writeInSpcEvtDscInf(Element InSpcEvtDscInf, Map<String, Object> InIDEfctInfM) {
		/*** 个人证件有效信息记录 --- ***/
		InSpcEvtDscInf.addElement("InfRecType")
				.setText(InIDEfctInfM.get("INF_REC_TYPE") != null ? InIDEfctInfM.get("INF_REC_TYPE").toString() : "");
		InSpcEvtDscInf.addElement("AcctCode")
				.setText(InIDEfctInfM.get("ACCT_CODE") != null ? InIDEfctInfM.get("ACCT_CODE").toString() : "");
		InSpcEvtDscInf.addElement("OpetnType")
				.setText(InIDEfctInfM.get("OPETN_TYPE") != null ? InIDEfctInfM.get("OPETN_TYPE").toString() : "");
		InSpcEvtDscInf.addElement("Month")
				.setText(InIDEfctInfM.get("MONTH") != null ? InIDEfctInfM.get("MONTH").toString() : "");
		InSpcEvtDscInf.addElement("Flag")
				.setText(InIDEfctInfM.get("FLAG") != null ? InIDEfctInfM.get("FLAG").toString() : "");
		InSpcEvtDscInf.addElement("RptDate")
