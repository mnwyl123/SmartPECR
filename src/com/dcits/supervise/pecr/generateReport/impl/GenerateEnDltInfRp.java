 ...
 *
 */
public class GenerateEnDltInfRp extends AbstractGenerateReport {

	private final static Logger logger = Logger.getLogger(GenerateEnDltInfRp.class);
	...
	public GenerateEnDltInfRp(Properties properties, ReportMessageManager reportMessageService) {
	...
	public Map<String, Object> generateBody(NeedReportDetail needReportDetail, String name,
		...
		for (int f = 0; f < forNum ; f++) {
			...
			switch (infRecType) {
			case PecrConstantUtil.EN_ACCT_INF_DEL:
				list = reportMessageService.queryEnDltRpData1(reportCount);
				break;
			case PecrConstantUtil.EN_CTRCT_INF_DEL:
				list = reportMessageService.queryEnDltRpData2(reportCount);
				break;
			case PecrConstantUtil.EN_SEC_ACCT_DEL:
				list = reportMessageService.queryEnDltRpData3(reportCount);
				break;
			}
			// 存放报文名和对应数据ID的map
		...
		logger.info("XML格式BODY内容生成执行完成 -----------企业按段删除报文生成完毕");
		return reportMap;
	}
	...
	public void updateBusinessState(NeedReportDetail needReportDetail) throws Exception {
		if (needReportDetail.getBusinessState() != null && needReportDetail.getBusinessState() != "") {
			...
			case PecrConstantUtil.BUSINESS_STATUS_30:
				...
				reportMessageService.updateEnDltInfRpDataStatus(conditionMap);
				break;
