 ...
 *
 */
public class GenerateInEntDltInfRp extends AbstractGenerateReport {

	private final static Logger logger = Logger.getLogger(GenerateInEntDltInfRp.class);
	...
	public GenerateInEntDltInfRp(Properties properties, ReportMessageManager reportMessageService) {
	...
	public Map<String, Object> generateBody(NeedReportDetail needReportDetail, String name,
		...
		switch (infRecType) {
			case PecrConstantUtil.IN_BS_INF_DLT:
		      PACKET_KEY = "PAY_IN_BS_INF_DLT_ID";
		      element = "InBsInfDlt";
		      table_name = "I_IN_BS_INF_DLT";
		      break;
		    case PecrConstantUtil.IN_ID_EFCT_INF_DLT:
		      PACKET_KEY = "PAZ_IN_ID_EFCT_INF_DLT_ID";
		      element = "InIDEfctInfDlt";
		      table_name = "I_IN_ID_EFCT_INF_DLT";
		      break;
		    case PecrConstantUtil.IN_ACCT_ENT_DEL:
		      PACKET_KEY = "PBZ_IN_ACCT_ENT_DEL_ID";
		      element = "InAcctEntDel";
		      table_name = "I_IN_ACCT_ENT_DEL";
		      break;
		    case PecrConstantUtil.IN_CTRCT_ENT_DEL:
		      PACKET_KEY = "PCZ_IN_CTRCT_ENT_DEL_ID";
		      element = "InCtrctEntDel";
		      table_name = "I_IN_CTRCT_ENT_DEL";
		      break;
		    case PecrConstantUtil.IN_SEC_ACCT_ENT_DEL:
		      PACKET_KEY = "PDZ_IN_SEC_ACCT_ENT_DEL_ID";
		      element = "InSecAcctEntDel";
		      table_name = "I_IN_SEC_ACCT_ENT_DEL";
		      break;
		    }
		// 获取要生成的报文总条数
		...
		for (int f = 0; f < forNum; f++) {
			...
			switch (infRecType) {
			case PecrConstantUtil.IN_BS_INF_DLT:
				list = reportMessageService.queryInEntDltRpData1(reportCount);
				break;
			case PecrConstantUtil.IN_ID_EFCT_INF_DLT:
				list = reportMessageService.queryInEntDltRpData2(reportCount);
				break;
			case PecrConstantUtil.IN_ACCT_ENT_DEL:
				list = reportMessageService.queryInEntDltRpData3(reportCount);
				break;
			case PecrConstantUtil.IN_CTRCT_ENT_DEL:
				list = reportMessageService.queryInEntDltRpData4(reportCount);
				break;
			case PecrConstantUtil.IN_SEC_ACCT_ENT_DEL:
				list = reportMessageService.queryInEntDltRpData5(reportCount);
				break;
			}
		...
		logger.info("XML格式BODY内容生成执行完成 -----------wzy-------"+infRecType+"-----个人整笔删除-----生成完毕");
		return reportMap;
	}
	...
	public void updateBusinessState(NeedReportDetail needReportDetail) throws Exception {
		if (needReportDetail.getBusinessState() != null && needReportDetail.getBusinessState() != "") {
			...
			case PecrConstantUtil.BUSINESS_STATUS_30:
				...
				reportMessageService.updatenInEntDltInfRpDataStatus(conditionMap);
				break;
	...
	public void writeInEntDlt1(Element element, Map<String, Object> indvEntity) {
		...
		element.addElement("Name").setText(indvEntity.get("NAME") != null ? indvEntity.get("NAME").toString() : "");
		element.addElement("IDType")
				.setText(indvEntity.get("ID_TYPE") != null ? indvEntity.get("ID_TYPE").toString() : "");
		element.addElement("IDNum")
