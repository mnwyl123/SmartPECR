 ...
 *
 */
public class GenerateEnSecAcctInfRp extends AbstractGenerateReport {

	private final static Logger logger = Logger.getLogger(GenerateEnSecAcctInfRp.class);

	public Properties properties;

	public ReportMessageManager reportMessageService;

	private static String TABLE_NAME = "E_GUAR_ACCT_BS";

	private static String PACKET_KEY = "EEA_GUAR_ACCT_BS_SGMT_ID";
	
	private int count = 0;
	
	private String CCRC_ORG_CODE = "";

	public GenerateEnSecAcctInfRp(Properties properties, ReportMessageManager reportMessageService) {
	...
	public Map<String, Object> generateBody(NeedReportDetail needReportDetail, String name,
		...
		//信息记录类型
		reportCount.put("INF_REC_TYPE", PecrConstantUtil.ENT_GUAR_ACCT_BS_SGMT);
		...
		for (int f = 0; f < forNum; f++) {
			...
			reportCount.put("company", needReportDetail.getCompany());
			reportCount.put("INF_REC_TYPE", PecrConstantUtil.ENT_GUAR_ACCT_BS_SGMT);
			list = reportMessageService.getGuarAcctBsSgmtByFactor(reportCount);
			...
			for(int i=0;i<list.size();i++){
				Map<String, Object> enSecAcctInfEntity = list.get(i);
				String pk = enSecAcctInfEntity.get("EEA_GUAR_ACCT_BS_SGMT_ID").toString();
				pdaid.add(pk);
			...
			// 获取相关还款责任人段数据（通过order by 字段EEA_GUAR_ACCT_BS_SGMT_ID
			// 将相同的连续放在一起，其外获取每一种相同EEA_GUAR_ACCT_BS_SGMT_ID的条数也返回到当前数据中 ）
			// 依据返回结果，将相同EEA_GUAR_ACCT_BS_SGMT_ID的数据放入一个list中，在将EEA_GUAR_ACCT_BS_SGMT_ID的值作为key形成一个map，方便其他标识段的生成和减少频繁数据库的访问
			List<Map<String, Object>> rltlist = reportMessageService.getGuarRltRepymtInfSgmtByFactor(pdaid);
			//放置相关还款责任人
			...
			if(rltlist!=null){
				for (int i = 0; i < rltlist.size();) {
					...
					String tempName = rltlist.get(i).get("EEA_GUAR_ACCT_BS_SGMT_ID").toString();
			...
			// 获取抵质押物信息段数据（通过order by 字段EEA_GUAR_ACCT_BS_SGMT_ID
			// 将相同的连续放在一起，其外获取每一种相同EEA_GUAR_ACCT_BS_SGMT_ID的条数也返回到当前数据中 ）
			// 依据返回结果，将相同EEA_GUAR_ACCT_BS_SGMT_ID的数据放入一个list中，在将EEA_GUAR_ACCT_BS_SGMT_ID的值作为key形成一个map，方便其他标识段的生成和减少频繁数据库的访问
			/*List<Map<String, Object>> mcclist = reportMessageService.getGuarMotgaCltalCtrctInfSgmtByFactor(pdaid);
			//放置抵质押物信息段
			...
			if(mcclist!=null){
				for (int i = 0; i < mcclist.size();) {
					...
					String tempName = mcclist.get(i).get("EEA_GUAR_ACCT_BS_SGMT_ID").toString();
					List<Map<String, Object>> s = new ArrayList<Map<String, Object>>();
					for (int j = i; j < (i + tempNum);) {
						s.add(mcclist.get(j));
						j++;
			...
			if(f%fileForNum == 0){
				String headStr = generateRpHead(PecrConstantUtil.ENT_GUAR_ACCT_BS_SGMT, numTotal[forTop++], "  "+CCRC_ORG_CODE);
				sbf.append(headStr);
				sbf.append("\r\n");
			}
			...
			for (Map<String, Object> indvEntity : list) {
				...
				Element EnSecAcctInf = root.addElement("EnSecAcctInf");
				/*** 企业担保账户信息 基础段 ****/
				Element GuarAcctBsSgmt = EnSecAcctInf.addElement("GuarAcctBsSgmt");
				writeGuarAcctBsSgmt(GuarAcctBsSgmt, indvEntity);
				/*** 企业担保账户信息基本信息段 ****/
				if (indvEntity.get("BUSI_LINES") != null && !indvEntity.get("BUSI_LINES").toString().equals("")) {
					Element GuarAcctBsInfSgmt = EnSecAcctInf.addElement("GuarAcctBsInfSgmt");
					writeGuarAcctBsInfSgmt(GuarAcctBsInfSgmt, indvEntity);
				}
				/*** 企业担保账户信息相关还款责任人 ****/
				if (rltmap.get(indvEntity.get("EEA_GUAR_ACCT_BS_SGMT_ID").toString()) != null) {
					Element RltRepymtInfSgmt = EnSecAcctInf.addElement("RltRepymtInfSgmt");
					writeRltRepymtInfSgmt(RltRepymtInfSgmt,
							rltmap.get(indvEntity.get("EEA_GUAR_ACCT_BS_SGMT_ID").toString()));
				}
				/*** 企业担保账户信息抵质押物信息段 ****/
				// 抵质押物信息段作为担保账户的预留段，暂时不需要报送
				/*if (mccmap.get(indvEntity.get("EEA_GUAR_ACCT_BS_SGMT_ID").toString()) != null) {
					Element GuarMotgaCltalCtrctInfSgmt = EnSecAcctInf.addElement("GuarMotgaCltalCtrctInfSgmt");
					writeGuarMotgaCltalCtrctInfSgmt(GuarMotgaCltalCtrctInfSgmt,
							mccmap.get(indvEntity.get("EEA_GUAR_ACCT_BS_SGMT_ID").toString()));
				}*/
				/*** 企业担保账户信息授信额度信息段 ****/
				if (indvEntity.get("MCC") != null && !indvEntity.get("MCC").toString().equals("")) {
					Element GuarAcctCredSgmt = EnSecAcctInf.addElement("GuarAcctCredSgmt");
					writeGuarAcctCredSgmt(GuarAcctCredSgmt, indvEntity);
				}

				/*** 企业担保账户信息在保责任信息段 ****/
				//当报送时点为其他信息变化时不上报在保责任系信息段
				if(indvEntity.get("RPT_DATE_CODE")!=null && !"50".equals(indvEntity.get("RPT_DATE_CODE").toString())){
					Element GuarRltRepymtInfSgmt = EnSecAcctInf.addElement("GuarRltRepymtInfSgmt");
					writeGuarRltRepymtInfSgmt(GuarRltRepymtInfSgmt, indvEntity);
				}
			...
			if(f%fileForNum == 0){
				...
				serialNumMap.put("inRefType", PecrConstantUtil.ENT_GUAR_ACCT_BS_SGMT);
				...
				reportName = generateRpName(PecrConstantUtil.ENT_GUAR_ACCT_BS_SGMT,reportNum, "0"+CCRC_ORG_CODE);
			...
			if((f+1)%fileForNum == 0||f==(forNum-1)){
				...
				rabme = AbstractGenerateReport.generateRpMessage(rpt_date,rabme, PecrConstantUtil.ENT_GUAR_ACCT_BS_SGMT, reportName, Float.valueOf(toalNum),
		...
		logger.info("XML格式BODY内容生成执行完成-----------wzy 企业担保信息生成完毕");
		return reportMap;
	}
	...
	public void updateBusinessState(NeedReportDetail needReportDetail) throws Exception {
		if (needReportDetail.getBusinessState() != null && needReportDetail.getBusinessState() != "") {
			...
			case PecrConstantUtil.BUSINESS_STATUS_30:
				...
				reportMessageService.updateGuarAcctBsSgmtDataStatus(conditionMap);
				break;
	...
	public static void writeGuarAcctBsInfSgmt(Element GuarAcctBsInfSgmt, Map<String, Object> indvEntity) {
		...
		GuarAcctBsInfSgmt.addElement("GuarAmt")
				.setText(indvEntity.get("GUAR_AMT") != null ? indvEntity.get("GUAR_AMT").toString() : "");
		...
		GuarAcctBsInfSgmt.addElement("OthRepyGuarWay").setText(
				indvEntity.get("OTH_REPY_GUARWAY") != null ? indvEntity.get("OTH_REPY_GUARWAY").toString() : "");
		GuarAcctBsInfSgmt.addElement("SecDep")
				.setText(indvEntity.get("SEC_DEP") != null ? indvEntity.get("SEC_DEP").toString() : "");
		GuarAcctBsInfSgmt.addElement("CtrctTxtCode")
				.setText(indvEntity.get("CTRCT_TXT_CODE") != null ? indvEntity.get("CTRCT_TXT_CODE").toString() : "");
	}
	...
	public static void writeRltRepymtInfSgmt(Element RltRepymtInfSgmt, List<Map<String, Object>> borrower) {
		...
		for (Map<String, Object> map : borrower) {
			if (map.get("INFO_ID_TYPE") != null && !map.get("INFO_ID_TYPE").toString().equals("")) {
				...
				// 韦宗英 为反担保人添加联保标志
				if (map.get("ARLP_TYPE").toString().equals("2")) {
					...
					RltRepymtInf.addElement("ArlpAmt").setText("");
					RltRepymtInf.addElement("WartySign").setText("");
					RltRepymtInf.addElement("MaxGuarMcc").setText("");
				}
	 ...
	 * @param borrower
	 */
	public static void writeGuarMotgaCltalCtrctInfSgmt(Element GuarMotgaCltalCtrctInfSgmt,
			List<Map<String, Object>> borrower) {
	 ...
	 * @param indvEntity
	 */
	public static void writeGuarAcctCredSgmt(Element GuarAcctCredSgmt, Map<String, Object> indvEntity) {
		GuarAcctCredSgmt.addElement("Mcc")
				.setText(indvEntity.get("MCC") != null ? indvEntity.get("MCC").toString() : "");
	}
	 ...
	 * @param indvEntity
	 */
	public static void writeGuarRltRepymtInfSgmt(Element GuarRltRepymtInfSgmt, Map<String, Object> indvEntity) {
