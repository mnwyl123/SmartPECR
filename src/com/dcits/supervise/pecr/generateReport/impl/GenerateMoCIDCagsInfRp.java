 ...
 *
 */
public class GenerateMoCIDCagsInfRp extends AbstractGenerateReport {

	private final static Logger logger = Logger.getLogger(GenerateMoCIDCagsInfRp.class);
	...
	public GenerateMoCIDCagsInfRp(Properties properties, ReportMessageManager reportMessageService) {
	...
	public Map<String, Object> generateBody(NeedReportDetail needReportDetail, String name, ReportFeedBackMessageService reportFeedBackMessageService) throws Exception {
		...
		PACKET_KEY = "EFW_MOC_ID_CAGS_INF_ID";
		table_name = "MOC_ID_CAGS_INF";
		// 获取要生成的报文总条数
		...
		for (int f = 0; f < forNum; f++) {
			...
			reportCount.put("company", needReportDetail.getCompany());
			reportCount.put("INF_REC_TYPE", infRecType);
			List<Map<String, Object>> list = reportMessageService.queryMoCIDCagsInfRpData(reportCount);
			...
			for (Map<String, Object> indvEntity : list) {
				...
				Element InCtrctInf = root.addElement("MoCIDCagsInf");
				writeMoCDlt(InCtrctInf, indvEntity);
		...
		logger.info("XML格式BODY内容生成执行完成 -----------wzy-------"+infRecType+"-----抵质押合同模块标识变更-----生成完毕");
		return reportMap;
	}
	...
	public void updateBusinessState(NeedReportDetail needReportDetail) throws Exception {
		if (needReportDetail.getBusinessState() != null && needReportDetail.getBusinessState() != "") {
			...
			case PecrConstantUtil.BUSINESS_STATUS_30:
				...
				reportMessageService.updatenMoCIDCagsInfRpDataStatus(conditionMap);
				break;
	...
	public void writeMoCDlt(Element element, Map<String, Object> indvEntity) {
