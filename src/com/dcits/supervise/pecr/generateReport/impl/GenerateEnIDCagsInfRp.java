 ...
 *
 */
public class GenerateEnIDCagsInfRp extends AbstractGenerateReport {

	private final static Logger logger = Logger.getLogger(GenerateEnIDCagsInfRp.class);
	...
	public GenerateEnIDCagsInfRp(Properties properties, ReportMessageManager reportMessageService) {
	...
	public Map<String, Object> generateBody(NeedReportDetail needReportDetail, String name,
		...
		case PecrConstantUtil.EN_ACCT_INF_ID_CAGS_INF:
			PACKET_KEY = "EDW_EN_ACCT_INF_ID_CAGS_INF_ID";
			element = "EnAcctInfIDCagsInf";
			table_name = "E_ACCT_INF_ID_CAGS_INF";
			break;
		case PecrConstantUtil.EN_CTRCT_INF_ID_CAGS_INF:
			PACKET_KEY = "EGW_EN_ACCT_INF_ID_CAGS_INF_ID";
			element = "EnCtrctInfIDCagsInf";
			table_name = "E_CR_ACCT_ID_CAGS_INF";
			break;
		case PecrConstantUtil.EN_SEC_ACCT_ID_CAGS_INF:
			PACKET_KEY = "EEW_EN_SEC_ACCT_ID_CAGS_INF_ID";
			element = "EnSecAcctIDCagsInf";
			table_name = "E_SEC_ACCT_ID_CAGS_INF";
			break;
		}
		...
		for (int f = 0; f < forNum ; f++) {
			...
			switch (infRecType) {
			case PecrConstantUtil.EN_ACCT_INF_ID_CAGS_INF:
				list = reportMessageService.queryEnIDCagsInfRpData1(reportCount);
				break;
			case PecrConstantUtil.EN_CTRCT_INF_ID_CAGS_INF:
				list = reportMessageService.queryEnIDCagsInfRpData2(reportCount);
				break;
			case PecrConstantUtil.EN_SEC_ACCT_ID_CAGS_INF:
				list = reportMessageService.queryEnIDCagsInfRpData3(reportCount);
				break;
			}
			// 存放报文名和对应数据ID的map
		...
		logger.info("XML格式BODY内容生成执行完成 -----------企业标识码变更类生成完毕");
		return reportMap;
	}
	...
	public void updateBusinessState(NeedReportDetail needReportDetail) throws Exception {
		if (needReportDetail.getBusinessState() != null && needReportDetail.getBusinessState() != "") {
			...
			case PecrConstantUtil.BUSINESS_STATUS_30:
				...
				reportMessageService.updatenEnIDCagsInfRpDataStatus(conditionMap);
				break;
