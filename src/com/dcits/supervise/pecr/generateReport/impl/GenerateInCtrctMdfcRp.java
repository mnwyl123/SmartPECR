 ...
 *
 */
public class GenerateInCtrctMdfcRp extends AbstractGenerateReport {

	private final static Logger logger = Logger.getLogger(GenerateInCtrctMdfcRp.class);
	...
	public GenerateInCtrctMdfcRp(Properties properties, ReportMessageManager reportMessageService) {
	...
	public Map<String, Object> generateBody(NeedReportDetail needReportDetail, String name,
		...
		table_name = "I_CTRCT_BS";
		PACKET_KEY = "PCA_CTRCT_BS_SGMT_ID";
		// 获取要生成的报文总条数
		...
		for (int f = 0; f < forNum; f++) {
			...
			reportCount.put("company", needReportDetail.getCompany());
			reportCount.put("INF_REC_TYPE", infRecType);
			List<Map<String, Object>> list = reportMessageService.queryInCtrctInfRpData(reportCount);
			// 存放报文名和对应数据ID的map
			...
			if(relList!=null){
				for (int i = 0; i < relList.size();) {
					...
					String tempName = relList.get(i).get(PACKET_KEY).toString();
			...
			for (Map<String, Object> indvEntity : list) {
				...
				Element InCtrctMdfc = root.addElement("InCtrctMdfc");
				/*** 个人授信更正段请求信息 --- 基础段 ****/
				Element BsSgmt = InCtrctMdfc.addElement("BsSgmt");
				writeBsSgmt(BsSgmt, indvEntity);
				/*** 个人授信更正段请求信息 --- -变更段-- ****/
				Element MdfcSgmt = InCtrctMdfc.addElement("MdfcSgmt");
				switch (indvEntity.get("MDFC_SGMT_CODE").toString()) {
				case PecrConstantUtil.IN_CTRCT_MDFC_B:
					/*** 个人授信信息 --- 基础段 ****/
					Element CtrctBsSgmt = MdfcSgmt.addElement("CtrctBsSgmt");
					writeCtrctBsSgmt(CtrctBsSgmt, indvEntity);
					break;
				case PecrConstantUtil.IN_CTRCT_MDFC_C:
					/*** 个人授信信息 --- -共同授信人段-- ****/
					if (map.get(indvEntity.get("PCA_CTRCT_BS_SGMT_ID").toString()) != null) {
						...
						GenerateCtrctBsSgmtRp.writeCtrctCertRelSgmt(CtrctCertRelSgmt,
								map.get(indvEntity.get("PCA_CTRCT_BS_SGMT_ID").toString()));
					}
					break;
				case PecrConstantUtil.IN_CTRCT_MDFC_D:
					/*** 个人授信信息 --- -记录额度段-- ****/
					Element CreditLimSgmt = MdfcSgmt.addElement("CreditLimSgmt");
					GenerateCtrctBsSgmtRp.writeCreditLimSgmt(CreditLimSgmt, indvEntity);
					break;
				default:
					break;
				}
		...
		logger.info("XML格式BODY内容生成执行完成 -----------wzy-------"+infRecType+"-----个人授信协议按段更正-----生成完毕");
		return reportMap;
	}
